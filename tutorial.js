...
export function tickTutorial(g,dt){if(!tutorialActive(g))return;const t=g.tutorial,p=g.player;
 ...
 else if(t.step===4){const e=g.enemies.find(e=>e.tutorial);if(!e)return;if(e.cast){e.cast.remaining-=dt;if(e.cast.remaining<=0){const avoided=distance(p,e.cast)>=e.cast.radius||p.invulnerable>0;e.cast=null;if(avoided&&t.dash){advance(g);return;}t.dash=false;t.clock=D.castPause;t.tries=(t.tries||0)+1;if(t.tries>=D.maxDodgeTries){g.toast(D.giveUp);advance(g);return;}if(t.tries>1)g.toast(D.retry);}}else if((t.clock-=dt)<=0){e.cast={name:D.cast,total:D.castTime,remaining:D.castTime,ground:true,x:p.x,y:p.y,radius:D.radiusAttack};t.dash=false;}}
