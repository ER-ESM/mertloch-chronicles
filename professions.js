...
/** Ansprechbares in Reichweite (oder unter dem Mauspunkt): Lehrer und Fundstellen. Die Station selbst ist kein Ziel – hergestellt wird im Berufefenster. */
export function professionTarget(g,point=null){if(g.instance||g.tutorial&&!g.tutorial.completed)return null;const l=professionWorld(g.world),p=point||g.player;return [...l.teachers,...l.nodes].filter(n=>Math.hypot(n.x-p.x,n.y-(point?p.y+(n.type==='professionTeacher'?14:4):p.y))<(point?(n.type==='professionTeacher'?20:22):R.range)).sort((a,b)=>Math.hypot(a.x-p.x,a.y-p.y)-Math.hypot(b.x-p.x,b.y-p.y))[0]||null;}
...
export function mountProfessions(host){
 ...
 function finish(c){const game=g();if(professionReason(game,c.site)){game.toast(T.cancelled);return;}const snapshot={...game.save(),savedAt:Date.now()};
  ...
  if(phase)plan.professions.solo[c.site.id]={cycle:phase.cycle,opened:phase.opened||Date.now(),claimed:phase.cycle};try{apply(game,{...snapshot,...plan},doneText(c.action));}catch{game.toast(T.full);}
 }
 async function start(action,retried=false){const game=g(),online=mode();if(game.professionCast||game.professionCommit||beginning)return false;const site=actionSite(professionWorld(game.world),action);if(action.kind==='gather')action={...action,target:site?.kind};const error=professionReason(game,site)||(moving(game)&&!site?.anywhere?T.moving:'')||professionPlan(game.save(),action).error;if(error){game.toast(error);return false;}
  ...
  if(!site.anywhere){game.dismount();game.player.moving=false;game.player.vx=game.player.vy=0;}const c={action,site,x:game.player.x,y:game.player.y,cycle:phase?.cycle,id:crypto.randomUUID(),name:actionName(action),total:actionDuration(action),remaining:actionDuration(action),online:game.professionOnline?online:null};
  game.professionFinish=finish;game.professionCancel=()=>{if(c.online)c.online.profession({op:'cancel'}).catch(()=>{});};
  if(c.online){beginning=true;try{const r=await online.profession({op:'begin',id:c.id,action,save:{...game.save(),savedAt:Date.now()}});if(r.stale&&r.server?.save){host.pull(r.server.save);/* Stand war nur kurz hinter dem Server (Vorgang direkt davor): nachladen und einmal selbst neu starten, statt den Klick zu verschlucken */if(!retried){beginning=false;setTimeout(()=>start(action,true),0);}return false;}if(r.error){game.toast(r.error);return false;}c.token=r.token;if(game!==g()||professionReason(game,site)||!site.anywhere&&(moving(game)||Math.hypot(game.player.x-c.x,game.player.y-c.y)>.5)){game.professionCancel();return false;}}catch{game.toast(T.offline);return false;}finally{beginning=false;}}
