...
Browser-Rollenspiel ohne Abhängigkeiten (ES-Module, Canvas, Node-Server). `npm test` muss vor jedem Commit grün sein. Kein GitHub Pages, keine GitHub Actions (E-63). Das Spiel läuft nur auf dem eigenen Server unter https://mertloch.esm-consultant.de/ (siehe README); nach dem Push `node scripts/server-refresh.mjs`.
