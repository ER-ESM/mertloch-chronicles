// Clanbuch: genau ein Buchfenster mit sieben Reitern. Overlays (Gespräch, Beute, Tod, Anlage) liegen daneben.
...
const widths={
inspection:360,detail:390,mobile:390,install:360,touchhelp:340,talents:820,activity:430,bag:600,person:820,book:760,quest:640,base:640,map:760,menu:620,clan:820,guide:620,admin:620,loot:296,dialog:440,memory:600,memoryart:800,death:420};
...
export class PopupWindows{
 ...
 clamp(w){const {el}=w;if(document.body.classList.contains('touch-mode')){const landscape=innerWidth>innerHeight;el.style.width=Math.min(widths[w.id]||360,innerWidth-24)+'px';el.style.left=(innerWidth-el.offsetWidth)/2+'px';el.style.top=landscape?'12px':'calc(104px + env(safe-area-inset-top, 0px))';return;}el.style.maxWidth=Math.max(240,innerWidth-18)+'px';if(isBook(w.id))el.style.width=Math.min(widthFor(w.id),innerWidth-18)+'px';el.style.left=Math.max(5,Math.min(el.offsetLeft,innerWidth-el.offsetWidth-5))+'px';el.style.top=Math.max(5,Math.min(el.offsetTop,Math.max(5,innerHeight-150-el.offsetHeight)))+'px';}
