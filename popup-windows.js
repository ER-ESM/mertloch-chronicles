...
// bag 412: sechs 56er-Plätze mit 48er-Symbol (icon-steps.css) brauchen 356 px Inhalt.
const widths={
professions:900,trainer:520,mounts:820,companions:780,
shop:920,inspection:360,detail:390,mobile:390,install:360,touchhelp:340,talents:760,activity:430,bag:412,person:440,book:400,quest:420,base:420,map:760,menu:220,settings:820,clan:470,guide:720,admin:620,loot:296,dialog:440,memory:600,memoryart:800,death:420,dungeonEntry:430,journal:560, volker:400};
...
const GRID_OVERLAY={dialog:'person',settings:'center',dungeonEntry:'center',journal:'center',volker:'center'};
...
export class PopupWindows{
 ...
 placeGrid(w){const a=this.dockArea(),slot=this.slots(a),f=this.frame(a,slot),where=GRID_OVERLAY[w.id],width=Math.min(widths[w.id]||440,a.width-2*EDGE);
  ...
  /* Runde 5b (Grafik-Endliste 4): die Einstellungen (feste Höhe, fenster-r4c/ui-chrome) enden über den sichtbaren Leisten – reicht der Platz
     unter der gemeinsamen Oberkante nicht, rückt das Fenster nach oben (höchstens bis 56 px), statt die Leiste zu schneiden. */
  let top=f.top;if(w.id==='settings'){const r=w.el.getBoundingClientRect(),h=r.height||Math.min(620,innerHeight-170),x0=r.width?r.left:left,x1=x0+(r.width||width),floor=visibleBars(a.height).reduce((b,v)=>x0<v.right&&x1>v.left?Math.min(b,v.top-GAP):b,a.bottom);top=Math.max(56,Math.min(f.top,floor-h));}
  Object.assign(w.el.style,{left:Math.round(left)+'px',top:Math.round(top)+'px',width:Math.round(width)+'px',maxWidth:'',minWidth:'',maxHeight:Math.round(f.height)+'px',height:where==='center'||CONTENT_HEIGHT.has(w.id)?'':Math.round(f.height)+'px'});}
 ...
 clamp(w){const {el}=w;if(touch()){
  ...
  /* Runde 4c: Hochkant ist das Spielmenü modal und darf über die ruhende Touch-Steuerung reichen (fenster-r4c.css blendet sie aus) –
     auf sehr kleinen Schirmen (≤ 600 px hoch, iPhone SE) gilt das für jedes Fenster, sonst bliebe für Figur/Talente kaum Höhe; der Spielerrahmen ist
     bei offenem Fenster verborgen, darum beginnen die Fenster 14 statt 28 px unter der Safe Area. */const portrait=innerHeight>=innerWidth,modal=w.id==='settings'/* Runde 5b: Einstellungen sind modal (auch quer) – die Steuerung ruht, das Fenster nutzt die ganze Breite */||portrait&&(w.id==='menu'||innerHeight<=600)||!portrait&&w.id==='menu'&&innerHeight<=340/* Runde 5b: sehr flach quer (568×320) passt das Spielmenü nur modal */,wide=!portrait&&w.id==='menu'&&innerHeight<=340;
  const r=touchPopupBounds({width:innerWidth,height:innerHeight,safe,controls:modal?[]:controls.map(c=>c.getBoundingClientRect()),hud:wide?[]:[...document.querySelectorAll('#gameShell>.player-panel')].map(c=>c.getBoundingClientRect()),preferredWidth:w.id==='map'?innerWidth:w.id==='settings'&&!portrait?720:TOUCH_WIDTHS[w.id]||widths[w.id]||360,fill:CHILD.has(w.id)||['shop','map','talents'].includes(w.id),topInset:(innerWidth>innerHeight?['shop']:['shop','talents','map','person','menu','bag','book','guide','quest','settings']).includes(w.id)?(portrait&&w.id!=='shop'?14:28):undefined});
  ...
  /* Runde 5b: Einstellungen füllen am Handy die freie Höhe (Seiten statt Scrollen, options-ui.js settingsWindow) – schlägt die alte feste Höhe */
  if(w.id==='settings')el.style.setProperty('height',r.maxHeight+'px','important');return;
