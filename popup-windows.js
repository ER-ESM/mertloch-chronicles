...
export class PopupWindows{
 ...
 clamp(w){const {el}=w;if(touch()){
  ...
  /* Runde 4c: Hochkant ist das Spielmenü modal und darf über die ruhende Touch-Steuerung reichen (fenster-r4c.css blendet sie aus) –
     ...
     bei offenem Fenster verborgen, darum beginnen die Fenster 14 statt 28 px unter der Safe Area. */const portrait=innerHeight>=innerWidth,modal=portrait&&(w.id==='menu'||innerHeight<=600);
  const r=touchPopupBounds({width:innerWidth,height:innerHeight,safe,controls:modal?[]:controls.map(c=>c.getBoundingClientRect()),hud:[...document.querySelectorAll('#gameShell>.player-panel')].map(c=>c.getBoundingClientRect()),preferredWidth:w.id==='map'?innerWidth:TOUCH_WIDTHS[w.id]||widths[w.id]||360,fill:CHILD.has(w.id)||['shop','map','talents'].includes(w.id),topInset:(innerWidth>innerHeight?['shop']:['shop','talents','map','person','menu','bag','book','guide','quest']).includes(w.id)?(portrait&&w.id!=='shop'?14:28):undefined});
