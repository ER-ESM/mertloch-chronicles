...
export function bossCasts(bossId){
 const def=DUNGEON_BOSSES[bossId];if(!def)return [];const sets=[def.castSet,...(def.phases||[]).map(p=>p.castSet).filter(Boolean)],seen=new Set(),out=[];
 for(const s of sets){const set=DUNGEON_CASTS[s];if(!set)continue;for(const type of set.cycle){if(seen.has(type))continue;seen.add(type);out.push({set:s,type,cast:set.casts[type]});}}
 return out;
}
/** Merkmale aller Fähigkeiten eines Bosses (Phasen-Helfer zählen als „summon“). */
export function bossTraits(bossId){const def=DUNGEON_BOSSES[bossId];const out=new Set();for(const c of bossCasts(bossId))for(const t of castTraits(c.cast))out.add(t);if(def?.phases?.some(p=>p.summon))out.add('summon');return out;}
...
export function journalPanel(g,bossId=null,dungeonId='schloss-bigb'){
 ...
 const tabs=dg.bosses.map(b=>{const d=DUNGEON_BOSSES[b.id],on=b.id===bossId,label=d?d.name:J.unknown;return `<button type="button" class="dj-tab${on?' on':''}${d?'':' dj-unknown'}" data-dj-boss="${b.id}" aria-pressed="${on}" ${d?'':'disabled'} data-tooltip-label="${esc(label)}" data-tooltip-note="${esc(b.rare?U.map.boss+' · ★':b.optional?U.map.boss+' · optional':U.map.boss)}">${d?`<canvas class="dj-face" width="72" height="72" data-dj-portrait="${b.id}" aria-hidden="true"></canvas>`:dicon('skull',22,'dim')}${b.rare?dicon('star',12,'dj-star'):''}</button>`;}).join('');
 const casts=bossCasts(bossId).map(c=>({...c,d:describeCast(c.set,c.type,{interrupt})}));
 const phases=(def.phases||[]).map((p,i)=>{const icons=[p.summon?dicon('trait-summon',18):'',p.castSet?dicon('trait-hit',18):''].join('');const note=[p.summon?J.phaseAdds(p.summon.count):'',p.castSet?J.phaseCycle:''].filter(Boolean).join(' · ')||J.phaseLine;return `<span class="dj-phase" data-tooltip-label="${esc(U.alerts.phase(i+2)+' · '+J.phaseAt(Math.round(p.at*100)))}" data-tooltip-note="${esc(note)}"><b>${Math.round(p.at*100)} %</b>${icons}</span>`;}).join('');
 const summon=def.phases?.find(p=>p.summon);
 const abilities=casts.map(({type,d})=>`<button type="button" class="dj-ability" data-dj-cast="${type}" data-tooltip-label="${esc(d.name)}" data-tooltip-note="${abilityNote(d)}">${dicon(d.icon,28)}<span><b>${esc(d.name)}</b><small>${esc(d.hint)}</small></span><i>${d.traits.slice(1).map(t=>dicon(t.icon,14)).join('')}</i></button>`).join('')
  +(summon?`<button type="button" class="dj-ability" data-dj-cast="summon" data-tooltip-label="${esc(U.traits.summon.name)}" data-tooltip-note="${esc(U.traits.summon.tip)}">${dicon('trait-summon',28)}<span><b>${esc(U.traits.summon.name)}</b><small>${esc(U.traits.summon.answer)}</small></span><i></i></button>`:'');
 const hints=roleHints(bossId),roles=['tank','heal','damage'].map(r=>`<span class="dj-role" data-dj-role="${r}" tabindex="0" data-tooltip-label="${esc(U.entry.roles[r])}" data-tooltip-note="${esc((hints[r].length?hints[r]:[J.roleNone]).join(' · '))}">${dicon('role-'+r,28)}<b>${hints[r].length}</b></span>`).join('');
 const loot=bossLoot(bossId),lootHtml=loot.length?loot.slice(0,6).map(id=>`<span class="dj-loot" tabindex="0" data-tooltip-item="${esc(id)}"><canvas width="48" height="48" data-item-art="${esc(ITEMS[id]?.icon||id)}" aria-hidden="true"></canvas></span>`).join(''):Array.from({length:3},()=>`<span class="dj-loot empty" tabindex="0" data-tooltip-label="${esc(J.loot)}" data-tooltip-note="${esc(J.lootNone)}">${dicon('loot',22,'dim')}</span>`).join('');
 const facts=[`<span class="dj-chip" data-tooltip-label="${esc(J.level(def.level))}" data-tooltip-note="">${dicon('boss',16)}<b>${def.level}</b></span>`,`<span class="dj-chip" data-tooltip-label="${esc(J.hp(def.hp))}" data-tooltip-note="">${dicon('trait-hit',16)}<b>${Math.round(def.hp/1000)}k</b></span>`,slot?.seal?`<span class="dj-chip" data-tooltip-label="${esc(J.seal)}" data-tooltip-note="">${dicon('seal',16)}</span>`:'',killed?`<span class="dj-chip" data-tooltip-label="${esc(U.map.bossDead)}" data-tooltip-note="">${dicon('skull-dead',16)}</span>`:''].join('');
 return `<span hidden data-ui-window-title="${esc(J.title+' · '+dg.name)}"></span><div class="dj" data-dj="${esc(dungeonId)}" data-dj-page="${esc(bossId)}">
<nav class="dj-tabs" aria-label="${esc(J.title)}">${tabs}</nav>
<header class="dj-head"><canvas class="dj-portrait" width="112" height="112" data-dj-portrait="${bossId}" aria-hidden="true"></canvas><div><h3>${esc(def.name)}</h3><small class="dj-title" data-tooltip-label="${esc(def.name)}" data-tooltip-note="${esc(def.look||'')}">${esc(def.title||'')}</small><div class="dj-facts">${facts}</div></div></header>
...
<div class="dj-abilities" aria-label="${esc(J.abilities)}">${abilities}</div>
...
export function paintBossPortraits(root,time=0){
 for(const cv of root.querySelectorAll('canvas[data-dj-portrait]')){const id=cv.dataset.djPortrait,def=DUNGEON_BOSSES[id];const c=cv.getContext('2d');c.setTransform(1,0,0,1,0,0);c.clearRect(0,0,cv.width,cv.height);
  ...
  let ok=false;try{const e=makeEnemy({x:0,y:0},-1,{type:def.type,skin:def.skin,name:def.name,family:def.family,hp:def.hp,level:def.level});Object.assign(e,{variant:def.art,bossId:id,type:'boss',hurt:0,attack:0,cast:null,moving:false,gaitWeight:0,facing:-1});
