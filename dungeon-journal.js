...
import {enrageTip,enrageClock} from './dungeon-einsatz-ui.js';/* Held aktiv: Wut mit den Zahlen des Bosses */
import {enrageInfo} from './dungeon.js';/* Dungeon-Fix 6: Wut nach Laufstand */
...
export function journalPanel(g,bossId=null,dungeonId='schloss-bigb'){
 ...
 const facts=[`<span class="dj-chip" data-tooltip-label="${esc(J.level(def.level))}" data-tooltip-note="">${dicon('boss',16)}<b>${def.level}</b></span>`,`<span class="dj-chip" data-tooltip-label="${esc(J.hp(def.hp))}" data-tooltip-note="">${dicon('trait-hit',16)}<b>${Math.round(def.hp/1000)}k</b></span>`,slot?.seal?`<span class="dj-chip" data-tooltip-label="${esc(J.seal)}" data-tooltip-note="">${dicon('seal',16)}</span>`:'',killed?`<span class="dj-chip" data-tooltip-label="${esc(U.map.bossDead)}" data-tooltip-note="">${dicon('skull-dead',16)}</span>`:'',/* Etappe 3: Wut (Zeitgrenze) */def.enrage?(()=>{/* Dungeon-Fix 6: im Durchgang die Wut nach Laufstand (Rita, Beweise) */const info=run?enrageInfo(run,bossId):null;return `<span class="dj-chip" data-tooltip-label="${esc(U.traits.enrage.name+' · '+U.alerts.enrageIn(info?.after??def.enrage.after).replace(/^Wut /,''))}" data-tooltip-note="${esc(enrageTip(def,info))}">${dicon('trait-enrage',16)}<b>${enrageClock(def,info)}</b></span>`;})():''].join('');
