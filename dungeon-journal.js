...
export function journalPanel(g,bossId=null,dungeonId='schloss-bigb'){
 ...
 const abilities=casts.map(({type,d})=>`<div class="dj-ability" role="listitem" tabindex="0" data-dj-cast="${type}" data-tooltip-label="${esc(d.name)}" data-tooltip-note="${abilityNote(d)}">${dicon(d.icon,28)}<span><b>${esc(d.name)}</b><small>${esc(d.hint)}</small></span><i>${d.traits.slice(1).map(t=>dicon(t.icon,14)).join('')}</i></div>`).join('')
...
<div class="dj-abilities" role="list" aria-label="${esc(J.abilities)}">${abilities}</div>
