...
export function dressSites(w){let accepted=0,omitted=0;const units=[w.npc,...w.quests.map(q=>q.giver)],occupied=[];
 for(const site of [...w.hubs,...w.camps]){const hub=w.hubs.includes(site),church=site.id==='kirchplatz',templates=church?[['board',-72,4,40,12]]:hub?[['tent',0,-20,69,34],['supplies',-64,3,29,19],['board',80,4,40,12],['lantern',-87,-6,32,8]]:[['tent',-70,-68,62,31],['supplies',73,-30,32,20],['lantern',-112,12,32,8],['board',101,18,40,12]];site.dressing=[];
  for(const [type,dx,dy,templateHeight,radius]of templates){let chosen=null;for(const [sx,sy]of [[0,0],[-20,0],[20,0],[0,-24],[0,24],[-32,-18],[32,-18],[-32,18],[32,18]]){const p={type,x:site.x+dx+sx,y:site.y+dy+sy,height:type==='lantern'?templateHeight:WORLD_SCALE[type]??templateHeight,radius};if(placementReason(w,p,'flowers',{hub:church,reserved:true,radius}))continue;if(units.some(u=>distance(p,u)<radius+14)||occupied.some(o=>distance(p,o)<radius+o.radius+6)||(site.spawns||[]).some(u=>distance(p,u)<radius+13))continue;chosen=p;break;}if(chosen){site.dressing.push(chosen);occupied.push(chosen);accepted++;}else omitted++;}
