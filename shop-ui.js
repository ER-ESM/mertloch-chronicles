...
export function shopPanel(g,state){
 ...
 const slots=Array.from({length:BAG_SIZE},(_,index)=>{
  ...
  return `<button type="button" class="shop-bag-slot rarity-${d.rarity}${sellable?'':' is-protected'}" data-shop-slot="${index}" data-shop-item="${esc(e.id)}" aria-label="${esc(label)}" title="${esc(label)}" aria-pressed="${selected===index}">${itemSymbol(e.id,iconStep('shop'))}<b>${e.count}</b><small>${sellable?total:'—'}</small></button>`;
 }).join('');
