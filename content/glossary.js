...
export const GLOSSARY={
 ...
 parade:{name:'Parade',short:'Ein kurzes Fenster, in dem du den nächsten Treffer schluckst und zurückgibst.',
  long:`Fenster 0,8 s (Dieter), 0,85 s (Schorsch), 0,9 s (Anni, Käthe) oder 1,1 s (Kevin), Abklingzeit 7 s, kostenlos. Ein Treffer im Fenster wird abgefangen, reflektiert 55 bis 75 Schaden und gibt 20 Ressourcenpunkte (${grantText(20)}); Dieter heilt zusätzlich 35 Leben. Eine geglückte Parade ist der Proc-Auslöser parry. Sie braucht einen Schild in der Nebenhand.`},
 ausweichen:{name:'Ausweichen',short:'Ein kurzer Satz zur Seite mit 0,4 s Schutz vor Treffern.',
  long:'Kostenlos, ohne globale Abklingzeit spürbar, Abklingzeit 3 s (Kevin), 4 s (Schorsch), 4,5 s (Käthe), 5 s (Dieter) oder 6 s (Anni). Ohne Eingabe geht es vom Ziel weg, mit Laufrichtung dorthin. Während der 0,4 s gehen Treffer ins Leere. Der Proc-Auslöser dodge hängt daran.'},
 ...
 staerkung:{name:'Stärkung',short:'Der Klassenbuff auf Taste 5: ein kurzes Fenster, in dem du mehr aushältst.',
  long:`${BUFF_SKILLS.common.duration} s Wirkung, ${BUFF_SKILLS.common.cd} s Abklingzeit; Kosten ${CLAN_MEMBERS.map(m=>{const c=skillCost({...BUFF_SKILLS.common,...BUFF_SKILLS[m.id]},m.id,'buff');return firstName(m)+' '+(c?.value?nice(c.value)+' '+c.unit:'keine');}).join(', ')}. Dieter senkt den Schaden, Anni heilt je Sekunde, Kevin legt Deckung auf; Schorsch facht mit dem Blasebalg die Glut an, Käthe gibt sich eine neue Hand mit sicherem Buben. Vor der Gruppe zünden, nicht wenn du schon liegst – die Abklingzeit ist länger als jeder Kampf.`},
 ...
 reichweite:{name:'Reichweite',short:'Wie weit ein Kniff trägt – gemessen in Metern.',
  long:'8 Welteinheiten sind 1 Meter. Nahkampf reicht 5,6 bis 6,9 m, Annis Sprühwerfer 19,4 m, Käthes Karten 25 m, Kevins Pfandgeschoss 26,3 m, der Wurf 29,4 m. Steht das Ziel weiter weg oder liegt etwas dazwischen, zündet der Kniff nicht.'},
