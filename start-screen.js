...
export function mountStartScreen(host){
 ...
 function rosterHtml(){
  ...
  const hall=list.length?`<div class="mmo-choices hero-hall" role="group" aria-label="${esc(HERO_UI.title)}">${cards}${add}</div>`:'';
  return `<header><p class="eyebrow">${esc(HERO_UI.eyebrow)}</p><h3>${esc(HERO_UI.title)}</h3><p>${esc(HERO_UI.text)}</p><div class="start-who ui-row">${whoRow()}</div></header>${hall}${detail}`;
 }
