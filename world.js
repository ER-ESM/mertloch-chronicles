...
import {placeCampProps,placeBase,placeKiosk} from './world-props.js';
...
export class World {
  constructor(data,options={}) {
    ...
    this.roads=[];this.buildings=[];this.areas=[];this.water=[];this.barriers=[];this.landmarks=[];this.trees=[];this.props=[];this.gardens=[];this.quests=[];this.camps=[];this.places={};this.grid=new Map();this.roadGrid=new Map();this.nodes=[];
    const raw=[];
  ...
  /** Lager der Akt-1-Kapitel 2–4. Sie stehen immer in der Welt; bevölkert werden sie erst, wenn das Kapitel läuft (engine.js). */
  generateChapterCamps(){const used=[{...this.spawn,reserve:210},...this.camps,...(this.hubs||[]),...this.quests.flatMap(q=>[{...q.giver,reserve:90},{...q.target,reserve:90}])];this.camps.push(...chapterCamps(this,used));}
  reserved(x,y,pad=0){const p={x,y};return Object.values(this.places||{}).some(o=>p.x>o.minX-pad&&p.x<o.maxX+pad&&p.y>o.minY-pad&&p.y<o.maxY+pad)||(this.hubs||[]).some(h=>distance(p,h)<110+pad)||distance(p,this.plaza)<this.plaza.radius+pad||this.camps.some(c=>distance(c,p)<(c.type==='boss'?165:135)+pad)||this.quests.some(q=>distance(q.giver,p)<50+pad||distance(q.target,p)<75+pad||q.items.some(item=>distance(item,p)<25+pad));}
  ...
  validate(){const targets=[...(this.hubs||[]).map(h=>({...h,label:h.id+' hub'})),...(this.base?[{...this.base.approach,label:'Bude'}]:[]),...(this.base?.house?[{...this.base.house.spots.wake,label:'Bude Schankraum'}]:[]),...Object.values(this.places||{}).map(o=>({...o.approach,label:o.name})),...this.camps.filter(c=>c.approach).map(c=>({...c.approach,label:c.id+' approach'})),{label:'Ida',...this.npc},{label:'Heilquelle',...this.shrine},...this.camps.flatMap(c=>c.spawns.map(p=>({...p,label:c.id}))),...this.quests.flatMap(q=>[{...q.giver,label:q.id+' giver'},...q.items.map(p=>({...p,label:p.id})),{...q.target,label:q.id+' target'}])];this.report.routes=[];
  ...
  export(){return {format:'mertloch-world-v2',seed:this.seed,rules:this.rules,bbox:this.bbox,source:{url:this.data.url,license:this.data.license,attribution:this.data.attribution},width:this.width,height:this.height,spawn:this.spawn,start:this.start,plaza:this.plaza,hubs:this.hubs,mentors:this.mentors,details:this.details,npc:this.npc,shrine:this.shrine,buildings:this.buildings,roads:this.roads,areas:this.areas,water:this.water,trees:this.trees,props:this.props,gardens:this.gardens,camps:this.camps,base:this.base,places:this.places,settlement:{cell:settlementMask(this).cell,radius:settlementMask(this).radius,minBuildings:settlementMask(this).minBuildings,cells:[...settlementMask(this).cells]},quests:this.quests,report:this.report};}
}
