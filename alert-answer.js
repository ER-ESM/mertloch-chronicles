...
/** Sichere Bereiche quer zu den echten Bahnen (x-Bahnen, Big B) und wohin der Held muss. → {hint,arrow,key,hold,side} oder null. */
export function laneAction(g,k){const lanes=k?.lanes||[],bad=(k?.truthLanes||[]).map(i=>lanes[i]).filter(Boolean);if(!lanes.length||!bad.length||lanes[0].axis==='y')return null;
 ...
 if(inside){const w=where(inside);return {hint:w==='left'?A.stayLeft:w==='right'?A.stayRight:A.stayMiddle,arrow:null,key:'',hold:true,side:w};}
 ...
 return {hint:w==='middle'?A.middle:dir==='right'?A.right:A.left,arrow:w==='middle'?'in':dir,key:bindKey(dir==='right'?'moveRight':'moveLeft'),hold:false,side:w};}
/**
 ...
 * null, d = describeCast (Antwort-Text und Hauptmerkmal). → {kind, hint, key, alt:{hint,key}|null, arrow:null|'left'|'right'|'in', hold}
 * hold = nichts drücken (stehen bleiben bzw. warten). key = Taste des Helden; leer nur bei hold.
 */
export function heroAnswer(g,{cast,live=null,active=false,d={}}={}){
 const kind=answerKind(cast),hint=d?.hint||'',dash=dashKey(g),out={kind,hint,key:'',alt:null,arrow:null,hold:false};
 ...
 if(kind==='spots'){const k=active?live:null;if(!k||k.told===false)return {...out,hint:cast.decoy?A.stamp:A.wait,key:dash};
  const inside=!g.dead&&(k.spots||[]).some(s=>!s.decoy&&Math.hypot((g.player.x-s.x)/k.radius,(g.player.y-s.y)/(k.radius*.75))<1.05);
  return inside?{...out,hint:A.out,key:dash}:{...out,hint:A.stay,hold:true};}
