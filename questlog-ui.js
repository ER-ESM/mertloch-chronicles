...
function mainEntry(game){
 ...
 const done=!!q.actDone,tracked=!game.trackedQuest;
 return `<article class="quest-entry main-quest"><span class="eyebrow">${esc(STORY.title+' · '+STORY.giver)}</span><h3>${esc(chapter.title)}</h3><p>${esc(chapter.summary||'')}</p>${
  q.accepted&&!done?`<ul>${progress.map(p=>`<li class="${p.complete?'done':''}">${esc(p.objective.label)}: ${p.done} / ${p.need}</li>`).join('')}</ul>`
   :`<div class="quest-objective">${done?esc(STORY.giver+' wartet an der Bude.'):esc('Sprich mit '+STORY.giver+', um anzufangen.')}</div>`
 }<div class="quest-reward">${esc(rewardLine(chapter.reward))} · ${esc(rewardNote(chapter.reward))}</div><div class="quest-entry-actions">${q.accepted&&!done?`<button class="${tracked?'outline-button':'gold-button'}" data-track-main>Hauptgeschichte verfolgen</button>`:''}<button class="outline-button" data-main-giver>${esc(STORY.giver)} auf der Karte</button></div>${done?'<span class="quest-complete">Abgeschlossen</span>':''}</article>`;
}
