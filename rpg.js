...
import {ACTION_BAR_TEXT,BALANCE,ITEM_CATALOG,rating,ratingK,powerRate,SYSTEM_LINES,STAT_NAMES,GEAR_COMPARE,WEAPON_TYPES,BAG_UI,RARITIES,DROP_TABLES} from './content/index.js';
...
export function createDrop(game,enemy){const r=game.rpg,n=++r.sequence,drop=dungeonBossBonus(game,enemy,worldBossBonus(game,enemy,rollDrop(game,enemy,ITEMS))),coins=drop.coins,items=game.netParty?.loot?game.netParty.loot(drop.items,enemy):drop.items;if(!items.length&&!coins&&!enemy.lootMoment)return null;
