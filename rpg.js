...
export function createDrop(game,enemy){if(enemy?.noLoot)return null;/* Helfer ohne Beute (Big Bs Follower, Etappe 3) */const r=game.rpg,n=++r.sequence,drop=dungeonBossBonus(game,enemy,worldBossBonus(game,enemy,rollDrop(game,enemy,ITEMS))),coins=drop.coins,items=game.netParty?.loot?game.netParty.loot(drop.items,enemy):drop.items;if(!items.length&&!coins&&!enemy.lootMoment)return null;
 ...
 const moment=enemy.lootMoment?{moment:true,reach:enemy.lootReach||170,noEquip:true,reward:enemy.dungeonReward||null}:{};
...
export function takeLoot(game,id,selection=null){const r=game.rpg,bag=r.loot.find(b=>b.id===id);if(!bag||game.dead)return false;if(Math.hypot(bag.x-game.player.x,bag.y-game.player.y)>lootReach(bag)){game.toast('Der Beutel ist zu weit weg. Geh näher heran.');return false;}if(selection&&selection!=='coins'&&!bag.items.some(e=>e.id===selection))return false;/* Beutel mit Wahl (Endtruhe, Dungeon Etappe 3): nur ein Teil nach Wahl, der Rest verfällt */if(bag.choice&&(!selection||selection==='coins')){game.toast(DUNGEON_TEXT.chest.pick);return false;}let coins=0;if(!selection||selection==='coins'){coins=bag.coins;r.coins+=coins;bag.coins=0;}const taken=[];for(const item of bag.items)if(!selection||selection===item.id){const rest=addItem(r,item.id,item.count);if(item.count-rest>0)taken.push({id:item.id,count:item.count-rest});item.count=rest;}bag.items=bag.items.filter(e=>e.count>0);if(bag.choice&&taken.length)bag.items=[];if(!bag.items.length&&!bag.coins)r.loot=r.loot.filter(b=>b.id!==id);placeUsables(game,taken.map(e=>e.id));lootEvent(game,taken,coins,{...bag.source,kind:bag.source?.kind||'chest'});if(!bag.noEquip)autoEquipFound(game,taken.map(e=>e.id));game.toast(bag.items.some(e=>!selection||e.id===selection)?'Rucksack voll. Der Rest bleibt liegen.':'Beute eingepackt.');changed(game);return true;}
