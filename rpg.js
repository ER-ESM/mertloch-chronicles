...
export function useItem(game,id){const def=ITEMS[id],p=game.player;if(game.paused||game.dead||!usableItem(id))return false;
 ...
 if(game.time<game.rpg.consumableReady){game.toast(def.name+' ist noch nicht bereit · '+Math.max(0,game.rpg.consumableReady-game.time).toFixed(1)+' s.');return false;}if(def.heal&&p.hp>=p.maxHp||def.energy&&p.energy>=100){game.toast('Das brauchst du gerade nicht.');return false;}removeItem(game.rpg,id);const b=game.baseEffects?.()||{};if(def.heal)p.hp=Math.min(p.maxHp,p.hp+Math.round(def.heal*(1+(b.foodHeal||0))));if(def.energy)p.energy=Math.min(100,p.energy+Math.round(def.energy*(1+(b.foodHeal||0))));game.rpg.consumableReady=game.time+Math.max(0,BALANCE.player.consumableCooldown-(b.consumableCd||0));game.effect('heal',p.x,p.y);game.toast(def.name+' benutzt.');game.memoryEvent?.({kind:'consumable',item:id});changed(game);if(actionBar(game).includes(barItemEntry(id))&&!countItem(game.rpg,id))game.emit('barChanged');return true;}
