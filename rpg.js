import {MOUNT_UI,ALL_MEMBERS,RESOURCES} from './content/index.js';
import {grantResource,resourceFull} from './class-resources.js';
...
import {ACTION_BAR_TEXT,BALANCE,ITEM_CATALOG,rating,ratingK,powerRate,SYSTEM_LINES,STAT_NAMES,GEAR_COMPARE,WEAPON_TYPES,BAG_UI,RARITIES,DROP_TABLES,DUNGEON_TEXT} from './content/index.js';
...
export const CLASS_DEFAULT_BAR={
 kevin:['auto','strike','reload','buff','throw','parry','mark','burst','ground','heal'],/* Pfandautomat gleich neben dem Pfandgeschoss */
...
export function combatStats(game){
 ...
 return {...talent,...raw,health:(raw.stamina-P.baseStamina)*P.hpPerStamina,power:raw.might*powerRate('might',level)+classBuffValue(game,'damage'),healPower:raw.wit*powerRate('healWit',level)+classBuffValue(game,'healPower'),shieldPower:raw.wit*powerRate('shieldWit',level)+classBuffValue(game,'shieldPower'),critDamage:(talent.critDamage||0)+classBuffValue(game,'critDamage'),armor:Math.min(R.armor.cap,rating(raw.armorRating,ratingK(R.armor,level)))+classBuffValue(game,'armor'),crit:Math.min(R.crit.cap,R.crit.base+rating(raw.finesse*R.crit.finesseWeight,ratingK(R.crit,level)))+classBuffValue(game,'crit'),haste,flatScale:1+(level-1)*(P.flatPerLevel||0),energyRegen:(talent.energyRegen||0)+raw.wit*powerRate('energyRegenWit',level)+classBuffValue(game,'energyRegen'),gcd:Math.max(P.gcdMin,P.gcdBase*(1-haste)),procs,spec,
...
export function useItem(game,id){const def=ITEMS[id],p=game.player;if(game.paused||game.dead||!usableItem(id))return false;
 const fail=(t,m)=>(game.fail||game.toast).call(game,t,m);/* Ablehnung als rote Fehlerzeile (Runde 3a) */if(!game.rpg.inventory.some(e=>e.id===id)){fail(def.name+': Davon hast du nichts mehr dabei.');return false;}
 if(game.time<game.rpg.consumableReady){fail(def.name+' ist noch nicht bereit · '+Math.max(0,game.rpg.consumableReady-game.time).toFixed(1).replace('.',',')+' s.',{early:'item:'+id});return false;}if(def.heal&&p.hp>=p.maxHp||def.energy&&!def.heal&&resourceFull(game)){fail('Das brauchst du gerade nicht.');return false;}const hpBefore=p.hp;let resourceGain=0;removeItem(game.rpg,id);const b=game.baseEffects?.()||{};if(def.heal)restoreMeterHealth(game,Math.round(def.heal*(1+(b.foodHeal||0))),{id:'item:'+id,name:def.name});if(def.energy)resourceGain=grantResource(game,Math.round(def.energy*(1+(b.foodHeal||0))),'item');game.rpg.consumableReady=game.time+Math.max(0,BALANCE.player.consumableCooldown-(b.consumableCd||0));if(p.hp>hpBefore)emitCombatFx(game,'heal',p,{amount:p.hp-hpBefore,direct:true});if(resourceGain>0)emitCombatFx(game,'proc',p,{signal:'resource',label:'+'+Math.round(resourceGain)+' '+(RESOURCES[game.member.id]?.unit||'Randale').toUpperCase()});game.toast(def.name+' benutzt.');game.memoryEvent?.({kind:'consumable',item:id});changed(game);if(actionBar(game).includes(barItemEntry(id))&&!countItem(game.rpg,id))game.emit('barChanged');return true;}
...
export function actionBar(game){const key=game.member.id,bar=game.rpg.actionBars[key],length=barLength(game);if(!Array.isArray(bar)){game.rpg.actionBars[key]=Array.from({length},(_,i)=>{const id=defaultBar(key)[i];return id&&SPECIAL_KEYS[id]===undefined&&available(game,id)?id:null;});return game.rpg.actionBars[key];}const seen=new Set();game.rpg.actionBars[key]=Array.from({length},(_,i)=>{const id=bar[i],item=barItemId(id);if(id==='mount')return game.mounts?.owned.length&&!seen.has(id)?(seen.add(id),id):null;if(item)return !usableItem(item)||seen.has(id)?null:(seen.add(id),id);if(SPECIAL_KEYS[id]!==undefined||!game.skills.some(s=>s.id===id)||!available(game,id)||seen.has(id))return null;seen.add(id);return id;});/* E-72: Kevins Pfandautomat (Kern der Klasse) liegt auf der Leiste – ältere Helden bekommen ihn einmal nachgelegt; wer ihn danach abräumt, bekommt ihn nicht zurück (rpg.resourceBars) */if(key==='kevin'&&available(game,'reload')){const done=game.rpg.resourceBars||(game.rpg.resourceBars=[]);if(!done.includes('reload')){done.push('reload');if(!game.rpg.actionBars[key].includes('reload'))unlockOnBar(game,['reload']);}}return game.rpg.actionBars[key];}
...
/** Startausstattung eines neuen Helden: seine Verpflegung nimmt einmalig einen freien Leistenplatz (Leiste 1 von hinten, sonst die nächste Leiste).
 *  Dungeon-Fix 2: nur noch beim allerersten Start (engine.js) – Beute, Einkauf und Handwerk legen nichts mehr ungefragt auf die Leiste. */
...
/** Reichweite eines Beutels: Boss-Beute im Dungeon (E-71) lässt sich in der ganzen Arena öffnen, sonst COMBAT_RULES.lootRange. */
export const lootReach=bag=>bag?.reach||43;
...
export function autoLootBag(game,bag){const r=game.rpg,taken=[],coins=bag.coins;let lost=0;/* Beutel mit Wahl (Endtruhe, Dungeon Etappe 3): beim Einsammeln nur so viele wie wählbar */if(bag.choice)bag.items=bag.items.slice(0,bag.choice);
 ...
 /* Dungeon-Fix 2 (Endabnahme #715: die Notfallbrezel aus Trash-Beute lag ungefragt auf Leistenplatz 0): Beute legt nichts mehr auf die Leiste –
    das Auto-Belegen steht nicht in docs/ENTSCHEIDUNGEN.md (nur Umsetzung Welle D, 31ee21ce). Verpflegung zieht der Spieler selbst auf die Leiste. */
 lootEvent(game,taken,coins,bag.source);/* Boss-Beute im Dungeon (E-71): nie ungefragt anlegen */if(!bag.noEquip)autoEquipFound(game,taken.map(e=>e.id));
...
/** Dungeon-Boss (E-71): fällt kein Ausrüstungsteil, legt der Boss ein seltenes nach – ein Boss geht nie leer aus. */
function dungeonBossBonus(game,enemy,drop){if(!enemy?.lootMoment||drop.items.some(e=>e.id.startsWith('roll-')))return drop;const t=DROP_TABLES[enemy.family]||{},slots=t.slots||['weapon','body'],specs=['tresen','bass','pfand'],rnd=()=>game.lootRandom();drop.items.push({id:registerRoll(game.rpg,ITEMS,{slot:slots[Math.floor(rnd()*slots.length)],spec:specs[Math.floor(rnd()*specs.length)],level:Math.max(1,Math.min(game.player.level+1,(enemy.level||1)+1)),quality:'rare',family:enemy.family||'quest',roll:Math.floor(rnd()*1000)}),count:1});return drop;}
export function createDrop(game,enemy){if(enemy?.noLoot)return null;/* Helfer ohne Beute (Big Bs Follower, Etappe 3) */const r=game.rpg,n=++r.sequence,drop=dungeonBossBonus(game,enemy,worldBossBonus(game,enemy,rollDrop(game,enemy,ITEMS))),coins=drop.coins,items=game.netParty?.loot?game.netParty.loot(drop.items,enemy):drop.items;if(!items.length&&!coins&&!enemy.lootMoment)return null;
 /* Beute-Moment (E-71): Dungeon-Bosse lassen den Beutel liegen, der sich in der Arena öffnen lässt; nichts wird ungefragt angelegt. */
 const moment=enemy.lootMoment?{moment:true,reach:enemy.lootReach||170,noEquip:true,reward:enemy.dungeonReward||null,room:enemy.dungeonBoss?.room||null/* Dungeon-Fix 2: in der ganzen Arena zu öffnen */}:{};
 const bag={id:'drop-'+n,x:enemy.x,y:enemy.y,coins,items,source:{name:enemy.name,kind:enemy.type==='boss'?'boss':'enemy'},...moment};r.loot.push(bag);game.emit('rpgChanged');if(moment.moment)game.emit('lootMoment',{bagId:bag.id,boss:enemy.bossId||null});else if(game.settings?.autoLoot)autoLootBag(game,bag);return bag;}
export function nearestLoot(game){return game.rpg.loot.filter(b=>Math.hypot(b.x-game.player.x,b.y-game.player.y)<43/* F-Taste nur am Beutel; die Arena-Reichweite der Boss-Beute (lootReach) gilt fürs Fenster */).sort((a,b)=>Math.hypot(a.x-game.player.x,a.y-game.player.y)-Math.hypot(b.x-game.player.x,b.y-game.player.y))[0]||null;}
...
export function takeLoot(game,id,selection=null){const r=game.rpg,bag=r.loot.find(b=>b.id===id);if(!bag||game.dead)return false;if(!(game.lootReachable?game.lootReachable(bag):Math.hypot(bag.x-game.player.x,bag.y-game.player.y)<=lootReach(bag))/* Dungeon-Fix 2: Boss-Beute in der ganzen Arena */){game.toast('Der Beutel ist zu weit weg. Geh näher heran.');return false;}if(selection&&selection!=='coins'&&!bag.items.some(e=>e.id===selection))return false;/* Beutel mit Wahl (Endtruhe, Dungeon Etappe 3): nur ein Teil nach Wahl, der Rest verfällt */if(bag.choice&&(!selection||selection==='coins')){game.toast(DUNGEON_TEXT.chest.pick);return false;}let coins=0;if(!selection||selection==='coins'){coins=bag.coins;r.coins+=coins;bag.coins=0;}const taken=[];for(const item of bag.items)if(!selection||selection===item.id){const rest=addItem(r,item.id,item.count);if(item.count-rest>0)taken.push({id:item.id,count:item.count-rest});item.count=rest;}bag.items=bag.items.filter(e=>e.count>0);if(bag.choice&&taken.length)bag.items=[];if(!bag.items.length&&!bag.coins)r.loot=r.loot.filter(b=>b.id!==id);placeUsables(game,taken.map(e=>e.id));lootEvent(game,taken,coins,{...bag.source,kind:bag.source?.kind||'chest'});if(!bag.noEquip)autoEquipFound(game,taken.map(e=>e.id));game.toast(bag.items.some(e=>!selection||e.id===selection)?'Rucksack voll. Der Rest bleibt liegen.':'Beute eingepackt.');changed(game);return true;}
