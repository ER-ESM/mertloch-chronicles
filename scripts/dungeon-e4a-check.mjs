...
try{
 ...
 if(want(1)){
  ...
  await read(`const b=${B('expose')};b.provision=2;return 1`);await fast('expose',.4);const a2=await alerts();assert.ok(a2.status.some(t=>/Provision/.test(t)),'Bossrahmen: Provision '+JSON.stringify(a2.status));await pause();await shot('04-expose-provision');await pause(false);ok('Exposé: Bossrahmen zeigt „'+a2.status.find(t=>/Provision/.test(t))+'“');
 ...
 if(want(2)){
  ...
  const wet=await read(`return g.dungeonRun.hazards.filter(h=>h.rect).length`);const a3=await alerts();assert.ok(wet>=2,'nasse Streifen');assert.ok(a3.status.some(t=>/Nass/.test(t)),'Bossrahmen: Nass '+JSON.stringify(a3.status));await shot('08-kurt-sprinkler');ok('Korken-Kurt: Sprinkleranlage – '+wet+' nasse Streifen vom Rand, Bossrahmen „'+a3.status.find(t=>/Nass/.test(t))+'“');await pause(false);
 ...
 if(want(3)){
  ...
  await read(`const b=${B('rita')},z=D.bossZones(g.dungeonRun,b);b.cast=null;Object.assign(b,{x:z.hidden.x+z.hidden.w/2,y:z.hidden.y+z.hidden.h/2});b.retreat={to:{x:b.x,y:b.y},until:g.time+4,kind:'hidden'};return 1`);await fast('rita',.3);await pause();
  const a=await alerts();assert.ok(a.status.some(t=>/Unsichtbar/.test(t)),'Bossrahmen: Unsichtbar '+JSON.stringify(a.status));await shot('10-rita-greenscreen');await pause(false);
