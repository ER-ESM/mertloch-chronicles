...
import {tickCasting,startAuto} from '../auto-combat.js';
import {changeSpec,pathBuild,learnTalent,talentPoints,TALENTS,SPECS as SPEC_DEFS} from '../talents.js';
...
function learnBuild(g,spec,path,drop){const budget=Math.max(0,talentPoints(g)),ids=[];
 if(path!=null)for(const id of pathBuild(spec,path,budget))ids.push(id);
 for(const id of ids)if(id!==drop)learnTalent(g,id);
 // Restpunkte wie spec-sim: erst der eigene Baum Reihe für Reihe, dann die Nachbarbäume.
 for(const tree of [spec,...Object.values(CLASS_SPECS).find(l=>l.includes(spec)).filter(x=>x!==spec)])for(const t of TALENTS[tree].slice().sort((a,b)=>a.row-b.row))if(t.id!==drop&&g.rpg.talents.learned.length<budget)learnTalent(g,t.id);
 return [...g.rpg.talents.learned];}

/** Ein Übungskampf: 40 s gegen drei Puppen mit fester Prioritäten-Rotation. */
export function simulate({classId,spec,path=0,level=10,gear='none',extra=null,drop=null,seconds=SHEET.seconds}){
 ...
 g.refreshStats?.();
 const talents=specOk?learnBuild(g,spec,path,drop):[];g.refreshStats?.();
 g.player.x=1000;g.player.y=1000;g.player.hp=g.player.maxHp;
 const foes=[0,1,2].map(i=>{const e=makeEnemy({x:1040+i*40,y:1000+(i%2)*30},i+1,{hp:1e7,roamWait:100,attackTimer:100,stun:1e9,damage:1});e.aggro=true;e.ai='combat';g.enemies.push(e);return e;});
 g.target=foes[0];g.player.inCombat=7;startAuto(g);const has=id=>g.skills.some(s=>s.id===id),healer=/Heilung/.test(SPEC_DEFS[spec]?.role||'');
 ...
 for(let t=0;t<seconds;t+=SHEET.dt){
  ...
  const p=g.player;if(!g.casting&&g.gcd<=0){
   if(has('heal')&&g.cooldowns.heal<=0&&(healer||p.hp/p.maxHp<.6))g.action('heal');
   else if(has('mark')&&g.target.mark<=0&&g.cooldowns.mark<=0&&p.energy>=20)g.action('mark');
  ...
  if(g.casting)tickCasting(g,SHEET.dt);g.tick(SHEET.dt);
 ...
 return {dps:dealt/seconds,hps:Math.max(healed,(heal.total||0)+(heal.excess||0))/seconds,mitigated:(raw-taken)/seconds,shield:shield/seconds,energy:energySum/ticks,talents,
...
export function buildSheet({quick=false}={}){
 ...
 for(const {classId,spec} of specsOf())for(const level of levels)for(const path of level>=BALANCE.player.specLevel?paths:[null])for(const g of gear){
  const r=simulate({classId,spec,path,level,gear:g});const role=SPEC_DEFS[spec]?.role||'';rows.push({classId,spec,role,group:roleGroup(role),path,level,gear:g,dps:round(r.dps),hps:round(r.hps),mitigated:round(r.mitigated),shield:round(r.shield),protection:round(r.mitigated+r.shield),energy:round(r.energy),skills:r.skills.slice(0,6).map(s=>({name:s.name,share:round(s.share)}))});}
 ...
 const attribution=[];
 for(const {classId,spec} of specsOf())for(const level of quick?[10]:SHEET.attributionLevels){
  const base=simulate({classId,spec,path:0,level,gear:SHEET.attributionGear}),bare=simulate({classId,spec,path:0,level,gear:'none'});
  const talents=base.talents.map(id=>{const r=simulate({classId,spec,path:0,level,gear:SHEET.attributionGear,drop:id});return {id,dps:pct(base.dps/Math.max(1,r.dps)-1),hps:pct(base.hps/Math.max(1,r.hps)-1)};}).sort((a,b)=>b.dps-a.dps);
  const stats=Object.keys(STAT_NAMES).map(k=>{const r=simulate({classId,spec,path:0,level,gear:SHEET.attributionGear,extra:{[k]:SHEET.statProbe}});return {stat:k,name:STAT_NAMES[k],dpsPerPoint:round((r.dps-base.dps)/SHEET.statProbe),hpsPerPoint:round((r.hps-base.hps)/SHEET.statProbe),mitigatedPerPoint:round((r.mitigated-base.mitigated)/SHEET.statProbe)};});
  attribution.push({classId,spec,level,gear:SHEET.attributionGear,dps:round(base.dps),gearShare:pct(base.dps/Math.max(1,bare.dps)-1),talents:quick?talents.slice(0,3):talents,stats,skills:base.skills.map(s=>({name:s.name,share:round(s.share)}))});}
...
function markdown(sheet){const L=[],gearName={none:'Startausrüstung',uncommon:'ungewöhnlich',rare:'selten',epic:'episch'},groupName={dps:'Schaden',heal:'Heilung',tank:'Tank'};
 ...
 for(const g of gears){L.push('## Ausrüstung: '+gearName[g],'');
  for(const group of Object.keys(METRIC)){const specs=[...new Set(sheet.rows.filter(r=>r.group===group).map(r=>r.spec))];if(!specs.length)continue;
   ...
   L.push('');}}
 L.push('## Zerlegung (Pfad 0, Ausrüstung '+gearName[SHEET.attributionGear]+')','');
 for(const a of sheet.attribution){L.push(`### ${a.spec} · Stufe ${a.level} · ${Math.round(a.dps)} Schaden/s · Ausrüstung +${a.gearShare} % gegenüber Startausrüstung`,'',
  ...
  '**Talente (Schaden mit gegenüber ohne dieses Talent):** '+a.talents.map(t=>`${t.id} ${t.dps>0?'+':''}${t.dps} %`).join(' · '),'');}
 return L.join('\n');}
function csv(sheet){const head=['klasse','spec','rolle','gruppe','pfad','stufe','ausruestung','schaden_s','heilung_s','verhindert_s','deckung_s','schutz_s','kennzahl','abweichung_median_pct','randale_avg'];
 return [head.join(';'),...sheet.rows.map(r=>[r.classId,r.spec,r.role,r.group,r.path??'kern',r.level,r.gear,r.dps,r.hps,r.mitigated,r.shield,r.protection,r.metric,r.vsMedian,r.energy].join(';'))].join('\n')+'\n';}
