...
const PARTS={
 ...
 async intro(){
  ...
  const settle=async rec=>{const done=await until(`!!document.querySelector('[data-tutorial-next]')&&!document.querySelector('.intro-film:not([hidden])')`,20000,150);await b.send('Page.removeScriptToEvaluateOnNewDocument',rec);return done;};
  ...
  {const rec=await create('dieter','button');await wait(40);await press('Escape');const done=await settle(rec);const film=await filmLog();
   runs.push({fall:'a Esc 40 ms nach Klick (alte Seite)',film,done});console.log('  a:',JSON.stringify({old:await read("sessionStorage.getItem('dbg-old')"),hide:await read("sessionStorage.getItem('dbg-hide')"),carry:await read('window.__carry'),keys:await read('JSON.stringify(window.__keys)'),film,done,seen:await seenFlag(),url:await read('location.href').catch(()=>'?'),start:await read("!!document.querySelector('#startScreen:not([hidden])')").catch(()=>'?'),tut:await read("!!document.querySelector('[data-tutorial-next]')").catch(()=>'?')}));ok(done&&!film.some(f=>f[0]==='on')&&await seenFlag(),'a) Esc direkt nach „Erstellen“ (Knopf): Film erscheint gar nicht, Ida wartet');await shot('20-intro-esc-sofort-ida');}
  // b) Enter im Namensfeld, Esc 1,5 s später (Ladeschirm)
  {const rec=await create('baerbel','enter');await wait(1500);const lo=await read(`document.getElementById('loading')?.className||''`).catch(()=>'?');await press('Escape');const done=await settle(rec);const film=await filmLog();
   runs.push({fall:'b Enter im Namensfeld, Esc nach 1,5 s ('+lo+')',film,done});ok(done&&!film.some(f=>f[0]==='on'),'b) Enter im Namensfeld, Esc auf dem Ladeschirm ('+(lo||'-')+'): Film erscheint gar nicht');}
  // c) Esc, wenn das Spiel steht, der Film aber noch auf das Abblenden des Ladeschirms wartet
  {const rec=await create('anni','button');await until(`!!window.mertloch`,60000,40);const lo=await read(`document.getElementById('loading')?.className||''`);await press('Escape');const done=await settle(rec);const film=await filmLog();
   runs.push({fall:'c Esc beim Spielstart ('+lo+')',film,done});ok(done&&!film.some(f=>f[0]==='on'),'c) Esc beim Spielstart (Film wartet, Ladeschirm „'+lo+'“): Film erscheint gar nicht');}
  // d/e) kein Esc vorher: im Film wirkt der ERSTE Esc (100 ms bzw. 1,5 s nach dem ersten Bild)
  for(const [fall,cls,delay] of [['d','kevin',100],['e','kaethe',1500]]){const rec=await create(cls,'button');const shown=await until(`!!document.querySelector('.intro-film:not([hidden])')`,60000,40);await wait(delay);if(fall==='e')await shot('21-intro-film-laeuft');await press('Escape');await wait(400);
