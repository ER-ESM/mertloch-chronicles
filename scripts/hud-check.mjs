...
try{
 ...
 await read('game.paused=false');await b.press('F10');assert.ok(await editing());assert.equal(await read('game.paused'),true);
 ...
 pass('F10 pauses combat and isolates keys; physical drag, scaling and saved coordinates survive reload');
 await b.press('F10');await drag('[data-hud-handle="player"]',24,56);await b.press('Escape');assert.deepEqual(await rect('.player-panel'),moved);
 await b.press('F10');await click('[data-hud-copy]');await field('[data-hud-name]','Raid-Test','input');await click('[data-hud-reset-all]');await click('[data-hud-save]');
 ...
 await b.press('F10');await field('[data-hud-profile]','0');await click('[data-hud-save]');assert.deepEqual(await rect('.player-panel'),moved);
 await b.press('F10');await field('[data-hud-profile]','1');await click('[data-hud-delete]');await click('[data-hud-cancel]');assert.equal((await stored()).profiles.length,2);
 pass('cancel restores previous geometry; named profile copies, switching and reset are transactional');
 await b.press('F10');await read(`window.originalSetItem=Storage.prototype.setItem;Storage.prototype.setItem=function(k,v){if(k==='${key}')throw Error('test quota');return window.originalSetItem.call(this,k,v);}`);
 ...
 await b.press('F10');await field('[data-hud-element]','meter');await field('[data-hud-scale]','110','input');await click('[data-hud-save]');
 ...
 await click('#touchMenu',true);await click('[data-book-tab="guide"]',true);
 await click('.panel-tabs button:last-child',true);await click('[data-hud-open]',true);assert.ok(await editing());
 ...
 await b.press('F10');await click('[data-hud-options]',true);await field('[data-hud-element]','player');assert.equal(await read(`document.querySelector('[data-hud-scale]').min`),'100');await click('[data-hud-reset-all]',true);await click('[data-hud-save]',true);
 pass('touch Help → Settings entry, physical finger drag, separate portrait/landscape layouts and touch scale floor');
 ...
 await b.press('F10');await click('[data-hud-options]',true);await read(`document.querySelector('.hud-form').scrollTop=0`);
 ...
 await b.resize(390,844);await wait(300);await b.press('F10');await b.screenshot(dir+'/touch-editor.png');
