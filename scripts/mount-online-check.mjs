...
try{
 ...
 for(const [client,mount]of [[a,'rasenkoenig'],[b,'packesel']])assert.ok(await run(client,`return g.others.some(p=>p.mount==='${mount}');`),'remote mount received in actual browser');
 ...
 await a.goto(url);await wait(800);assert.equal(await run(a,'return g.player.mount;'),null);assert.equal(await run(a,'return g.mounts.owned.length;'),6);
