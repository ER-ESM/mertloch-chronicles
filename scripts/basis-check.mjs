...
try{
 ...
 for(const [width,height,touch] of [[2024,900,false],[1440,900,false],[390,844,true],[320,568,true],[844,390,true],[667,375,true]]){
  ...
  const hud=await b.evaluate(`(()=>{const box=s=>{const r=document.querySelector(s).getBoundingClientRect();return{x:r.x,y:r.y,w:r.width,h:r.height,bottom:r.bottom,right:r.right}},panel=box('.player-panel');return{panel,energy:box('.player-panel .energy'),hasCombo:!!document.querySelector('.rune-row,#runes'),top:box('.touch-topline'),controls:[...document.querySelectorAll('#touchActions button,#touchUtility button')].filter(e=>e.getBoundingClientRect().width).map(e=>({id:e.id||e.dataset.skill,w:e.getBoundingClientRect().width,h:e.getBoundingClientRect().height}))}})()`);
  assert.equal(hud.hasCombo,false,label+' has no shared combo row');assert.ok(hud.energy.bottom<=hud.panel.bottom,label+' energy stays inside HUD');if(touch)for(const c of hud.controls)assert.ok(c.w>=44&&c.h>=44,label+' touch size '+c.id);
...
}catch(error){await b.shot('failure.png');throw error;}finally{b.close();for(const child of children)disposeChrome(child,child.profile);}
...
async function boot(){
 ...
 const profile=makeProfile('mertloch-basis-');
