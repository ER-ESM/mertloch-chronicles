...
{const rate=rs=>rs.length?rs.filter(r=>r.won).length/rs.length:0,pc=x=>Math.round(x*100)+' %',frac=rs=>rs.filter(r=>r.won).length+'/'+rs.length,ROLE_NAME={tank:'Tank',heal:'Heiler',damage:'Schaden'};
 const ps=out.passiv;if(ps.length){const combos=new Map();for(const r of ps){const k=[r.boss,r.state,r.role,r.mode].join('|');if(!combos.has(k))combos.set(k,[]);combos.get(k).push(r);}
  ...
  for(const b of [...new Set(ps.map(r=>r.boss))])for(const s of [...new Set(ps.filter(r=>r.boss===b).map(r=>r.state))])checks.push(['Info · passiv '+b+(s!=='–'?' '+s+' ('+BIGB_STATES[s].label+')':''),true,Object.keys(ROLE_NAME).map(ro=>ROLE_NAME[ro]+' '+Object.keys(PASSIVE_MODES).map(m=>frac(ps.filter(r=>r.boss===b&&r.state===s&&r.role===ro&&r.mode===m))).join(' ')).join(' · ')+' (passiv/steht/liegt/anfang)']);
  /* Dungeon-Fix 7: die Wut ist ein harter Wipe – wer passiv bis zur Wut kommt, liegt spätestens 20 s danach */{const enr=r=>r.enrage??DUNGEON_BOSSES[r.boss]?.enrage?.after??Infinity,late=ps.filter(r=>!r.won&&r.time>=enr(r)),slow=late.filter(r=>r.time-enr(r)>20),d=late.map(r=>r.time-enr(r));
 ...
 if(l7.length){const old=l7.filter(r=>r.rules==='#770'),cur=l7.filter(r=>r.rules==='jetzt'),after=rs=>rs.map(r=>r.time-r.enrage),span=a=>a.length?Math.min(...a)+'–'+Math.max(...a)+' s':'–';
  checks.push(['Live-Fall #770 in der Simulation getroffen: mit den Regeln von #770 hält die passive Gruppe die Wut über 20 s und gewinnt oder verliert knapp (≤ 8 % Bossleben)',old.every(r=>r.time-r.enrage>20&&(r.won||r.bossMin<=8))&&old.some(r=>r.won||r.bossMin<=3),'Regeln #770: '+frac(old)+' · Ende '+span(after(old))+' nach der Wut · verloren bei '+(old.filter(r=>!r.won).map(r=>r.bossMin+' %').join('/')||'–')],
