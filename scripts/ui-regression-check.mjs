...
export async function runUI(suites=['navigation','inventory','classes','combat','layout']){
 ...
 try{
  ...
  if(suites.includes('navigation')){
   ...
   for(const [n,id] of ['bag','person','book','talents','quest','map','guide'].entries()){await panel(id);assert.deepEqual((await state()).popups.map(p=>p.id),[id]);const before=await state();await b.hold(n%2?'w':'s',300);/* abwechselnd, sonst läuft die Figur über sieben Seiten in ein Hindernis */const after=await state();assert.equal(after.paused,false);assert.ok(after.time>before.time);assert.ok(Math.hypot(after.player.x-before.player.x,after.player.y-before.player.y)>1);}
