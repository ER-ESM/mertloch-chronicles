...
export async function runUI(suites=['navigation','inventory','classes','combat','layout']){
 const b=await browserSession({url:process.argv.find(a=>a.startsWith('http')),port:Number(process.env.CDP_PORT||9372),...(process.env.SERVER_PORT?{serverPort:Number(process.env.SERVER_PORT)}:{})}),dir='visual-review/ui-regression',checks=[];
 ...
 async function panel(id){const touch=await read(`document.body.classList.contains('touch-mode')`);if(touch){await click('#touchMenu',{touch:true});await click(`.game-menu-windows [data-shell="${id}"]`,{touch:true});return;}await click(`.game-menu-rail [data-panel="${id}"]`,{touch});}
 /** Einzelfenster: alles schließen, dann genau dieses Fenster öffnen (die Menüleiste schaltet um). */
 async function fresh(id){await read(`document.querySelectorAll('[data-window-close]').forEach(x=>x.click())`);await wait(80);await panel(id);}
 ...
 try{
  ...
  if(suites.includes('inventory')){
   ...
   await panel('person');await click('[data-equipped="body"]');await click('[data-unequip="body"]');assert.equal((await state()).rpg.equipment.body,null);await fresh('bag');
   await click('[data-item="regenjacke"]');await click('[data-equip="regenjacke"][data-equip-slot="body"]');assert.equal((await state()).rpg.equipment.body,'regenjacke');await fresh('bag');
   await read('game.player.hp=100');await click('[data-item="brezel"]');await click('[data-use="brezel"]');assert.ok((await state()).player.hp>100);assert.ok((await state()).rpg.inventory.find(i=>i.id==='brezel').count<3);await fresh('bag');
   await click('.popup-bag .bag-lupe');/* Runde 3b: die Suche klappt hinter der Lupe auf und hat dann den Fokus */await b.send('Input.insertText',{text:'wasser'});const p=(await state()).player;await b.hold('d',200);assert.ok(Math.hypot((await state()).player.x-p.x,(await state()).player.y-p.y)<2);assert.deepEqual(await read(`[...document.querySelectorAll('[data-item]:not([hidden])')].map(e=>e.dataset.item)`),['wasser']);
  ...
  if(suites.includes('classes')){
   ...
   for(const classId of ['dieter','baerbel','kevin']){
    ...
    await b.press('p');await b.press('n');assert.deepEqual((await state()).popups.map(p=>p.id),['talents']);/* Einzelfenster: P schließt Kniffe wieder *//* E-42: Talente auf N */
  ...
  if(suites.includes('layout')){
   ...
   for(const [name,width,height,touch,hand] of [['desktop',1440,1000,false,'right'],['phone',390,844,true,'right'],['small',320,740,true,'right'],['landscape',844,390,true,'right'],['landscape-left',844,390,true,'left']]){
    ...
    /* Runde 4b: am Desktop steht die Hofprobe als Auftrag in der Verfolgung, der Kasten gilt nur noch am Handy */if(touch){assert.ok(tutorial.styled);for(const r of [tutorial.a,tutorial.b])assert.ok(r.w>=44&&r.h>=44);assert.ok(tutorial.guide.x>=0&&tutorial.guide.right<=width+1);}else assert.ok(await read(`!!document.querySelector('.quest-panel .qt-quest.is-focus kbd')&&document.querySelector('#tutorialGuide').hidden`),'Hofprobe in der Verfolgung');
    ...
    // Kein Balkentext darf abgeschnitten sein (Befund 2026-09-20: 13-px-Schrift in der 9 px hohen Randale-Leiste).
    {const clipped=await read(`(()=>{document.querySelector('.player-panel .energy')?.classList.remove('bar-hidden');return [...document.querySelectorAll('.bar,.cast,.rpg-xp,.party-hp')].filter(b=>b.offsetParent).flatMap(b=>[...b.querySelectorAll('span')].filter(t=>t.textContent.trim()&&parseFloat(getComputedStyle(t).fontSize)*.72>b.clientHeight+.5).map(t=>(b.className+': Schrift '+getComputedStyle(t).fontSize+' in '+b.clientHeight+' px')));})()`);assert.deepEqual(clipped,[],name+': abgeschnittener Balkentext');}
    await b.screenshot(dir+'/'+name+'-tutorial.png');/* Runde 4b: am Desktop gibt es den Hofprobe-Kasten nicht mehr – Idas Anleitung öffnet sich im Gespräch mit Ida (F) */if(touch)await click('[data-tutorial-help]',{touch});else{await read(`(()=>{const g=window.game,n=g.world.npc;Object.assign(g.player,{x:n.x,y:n.y+8});g.moveTo=null;g.path=[];})()`);await wait(300);await b.press('f');await wait(500);}assert.ok((await state()).popups.some(p=>p.id==='dialog'));await b.press('Escape');
    for(const id of ['person','bag','quest','map','guide']){await fresh(id);const box=await read(`(()=>{const el=document.querySelector('.popup-${id}'),r=el.getBoundingClientRect(),body=el.querySelector('.popup-body');return{x:r.x,right:r.right,y:r.y,bottom:r.bottom,overflow:body.scrollWidth-body.clientWidth,vscroll:[body,...body.querySelectorAll('*')].filter(e=>e.offsetParent&&(e===body||/auto|scroll/.test(getComputedStyle(e).overflowY))&&e.scrollHeight>e.clientHeight+2).map(e=>String(e.className).split(' ')[0]+' '+e.scrollHeight+'/'+e.clientHeight)}})()`);assert.ok(box.x>=-1&&box.right<=width+1&&box.y>=-1&&box.bottom<=height+1,JSON.stringify({name,id,box}));if(box.overflow>=2){console.log(await read(`(()=>{const b=document.querySelector('.popup-${id} .popup-body'),r=b.getBoundingClientRect();return [...b.querySelectorAll('*')].filter(e=>e.getBoundingClientRect().right>r.right-5).map(e=>({tag:e.tagName,cls:e.className,right:e.getBoundingClientRect().right,body:r.right})).slice(0,15)})()`));await b.screenshot(dir+'/'+name+'-overflow.png');}assert.ok(box.overflow<2,JSON.stringify({name,id,box}));/* Runde 3b (2026-09-24): Scroll-Regel jetzt auch am Handy scharf */if(process.env.SCROLL_STRICT!=='0')assert.deepEqual(box.vscroll,[],'Handy '+name+': '+id+' scrollt');}
    await b.screenshot(dir+'/'+name+'-book.png');pass(name+': tutorial controls, target clearance and five single windows');
   }
  }
