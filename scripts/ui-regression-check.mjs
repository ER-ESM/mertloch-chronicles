 ...
 try{
  ...
  if(suites.includes('classes')){
   ...
   for(const classId of ['dieter','baerbel','kevin']){
    ...
    for(const spec of specs){await click('[data-spec="'+spec+'"]');const nodes=await read(`[...document.querySelectorAll('[data-talent]')].map(e=>e.dataset.talent)`);assert.equal(nodes.length,10);await click('[data-talent="'+nodes[9]+'"]');assert.equal((await state()).rpg.talents.learned.length,0);for(const node of nodes)await click('[data-talent="'+node+'"]');const s=await state();assert.equal(s.rpg.talents.learned.length,10);for(const skill of s.skills.filter(x=>x.talent&&nodes.includes(x.talent))){assert.ok(s.unlocked.includes(skill.id));assert.ok(s.actionBar.includes(skill.id));}}
    await persist();assert.equal((await state()).rpg.talents.learned.length,10);pass(classId+': three trees, 30 talents, granted skills and persistence');
   }
