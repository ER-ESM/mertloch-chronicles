...
try{
 ...
 /* Runde 3b (WoW teilt nur die Oberkante): Rucksack und Kniffe sind so hoch wie ihr Inhalt; seit Runde 5b auch die Aufträge – die gemeinsame Grenze ist die Unterkante über den sichtbaren Leisten */const floorY=await read(`return Math.min(innerHeight,...[...document.querySelectorAll('.action-area>*,.game-menu-rail')].filter(e=>!e.matches('#interact,.interact,.rotation-tip,#rotationTip,#classMechanicArt,[role=tooltip]')&&getComputedStyle(e).visibility!=='hidden'&&getComputedStyle(e).display!=='none'&&e.getBoundingClientRect().height>2&&e.getBoundingClientRect().top>innerHeight/2).map(e=>e.getBoundingClientRect().top))-8`);assert.ok(['bag','book','quest','person'].every(id=>four[id].b<=floorY+1),'Rucksack, Kniffe, Aufträge und Figur enden über der gemeinsamen Grenze '+floorY+' '+JSON.stringify(four));
 ...
 /* Runde 4a: Die Legende ist der Filterliste (Trichter in der Titelzeile) gewichen – dieselbe Aussage dort: Häkchen klein, kein weißes Quadrat */
 await b.press('m');await wait(900);await read(`document.querySelector('.popup-map [data-wk-filter]')?.click()`);await wait(300);const box=await rect('.popup-map [data-wk-show] i,.wk-filter [data-wk-show] i');assert.ok(box&&box.w<=18&&box.h<=18,'Filterliste: Häkchen klein '+JSON.stringify(box));assert.notEqual(await read(`return getComputedStyle(document.querySelector('[data-wk-show] i')).backgroundColor`),'rgb(255, 255, 255)','kein weißes Quadrat');
 const em=await read(`const e=document.querySelector('.wk-row em,.atlas-place em,.atlas-list em');return e?getComputedStyle(e).textTransform:'none'`);assert.equal(em,'none','„m“ nicht in Versalien');await b.screenshot(`${dir}/r1-43-karte-filter.jpg`);await read(`document.querySelector('.popup-map [data-wk-filter]')?.click()`);await b.press('m');await wait(200);
