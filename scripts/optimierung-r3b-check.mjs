...
try{
 ...
 assert.equal(book.t,person.t,'gemeinsame Oberkante');/* Runde 5b: auch die Aufträge sind jetzt so hoch wie ihr Inhalt – Maßstab ist die volle Fensterhöhe bis über die sichtbaren Leisten */const floorY=await read(`return Math.min(innerHeight,...[...document.querySelectorAll('.action-area>*,.game-menu-rail')].filter(e=>!e.matches('#interact,.interact,.rotation-tip,#rotationTip,#classMechanicArt,[role=tooltip]')&&getComputedStyle(e).visibility!=='hidden'&&getComputedStyle(e).display!=='none'&&e.getBoundingClientRect().height>2&&e.getBoundingClientRect().top>innerHeight/2).map(e=>e.getBoundingClientRect().top))-8`);assert.ok(book.h<floorY-book.t-60,'Kniffe so hoch wie der Inhalt, nicht wie die volle Fensterhöhe '+JSON.stringify({book,person,floorY}));assert.ok(book.empty<=40,'kein leeres Drittel unten '+JSON.stringify(book));
