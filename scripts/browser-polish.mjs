...
export async function browser({port=9222}={}){
 ...
 return {send,evaluate,errors,on:(method,fn)=>{if(!listeners.has(method))listeners.set(method,[]);listeners.get(method).push(fn);},close:()=>ws.close(),key,press:async k=>{await key(k);await key(k,'keyUp');},hold:async(k,ms)=>{await key(k);await wait(ms);await key(k,'keyUp');},click:s=>evaluate(`document.querySelector(${JSON.stringify(s)}).click()`),
  ...
  async goto(url='http://localhost:4173',{passStart=true}={}){await send('Page.navigate',{url});for(let i=0,n=Number(process.env.BOOT_TRIES||160);i<n;i++){await wait(100);if(await evaluate('!!window.mertloch')){if(passStart)for(let j=0;j<50;j++){const open=await evaluate(`(()=>{const s=document.querySelector('#startScreen');if(!s||s.hidden)return false;s.querySelector('[data-start=guest]')?.click();const enter=s.querySelector('[data-start=enter]');if(enter){enter.click();return true;}/* Helden-Slots: ohne Held legt das Prüfskript einen Testhelden an (Klasse → Aussehen → Name) */const name=s.querySelector('[name=heroName]');if(name){if(!name.value){name.value='Pruefheld';}s.querySelector('[data-start=draft-next]')?.click();}else if(s.querySelector('[data-start=draft-next]'))s.querySelector('[data-start=draft-next]').click();else s.querySelector('[data-start=create]')?.click();return true;})()`);if(!open)break;await wait(100);}return;}}throw Error('Game did not initialize');},
