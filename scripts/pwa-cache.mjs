...
export async function writePrecache(){const files=(await readdir(root)).filter(n=>/\.(js|css|html|webmanifest)$/.test(n)&&!['sw.js','precache-manifest.js'].includes(n));files.push('data/mertloch.json');for(const file of await readdir(new URL('content/',root)))if(file.endsWith('.js'))files.push('content/'+file);// Nur die tatsächlich verwendeten Exporte der Grafiklieferung: sources/, review/, generation-*.json
...
for(const folder of ['maifeld-09','maifeld-rpg','maifeld-ui-011','clan-skills-013','app','content-art/npcs','content-art/items','content-art/talents','content-art/talents/procs','content-art/aperol-anni','content-art/ui','content-art/heroes','content-art/enemies','content-art/bosses','content-art/props'])for(const file of await readdir(new URL('assets/'+folder+'/',root)))if(/\.(png|svg)$/.test(file)&&!(file==='baerbel.png'&&['clan-skills-013','content-art/talents'].includes(folder))&&!(folder==='content-art/talents/procs'&&file.startsWith('proc-')))files.push('assets/'+folder+'/'+file);
