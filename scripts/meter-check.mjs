...
try{
 await b.resize(1440,1000);await fixture();await b.press('v');await wait(300);
 ...
 await click('[data-meter-reset]');await click('[data-meter-cancel]');assert.ok((await b.state()).meter.damage>0);
 ...
 await b.press('Escape');assert.equal(await read(`document.querySelector('#combatMeter').hidden`),true);
 ...
 await b.goto(b.url);await wait(300);assert.equal((await b.state()).meter.damage,0);assert.ok((await b.state()).player.level>=11);pass('closing returns to game; reload clears session statistics and preserves progress');
