...
async function hoverBind(i,press){/* E-72 R5 (klicks 1): leere Plätze der Zusatzleisten sind unsichtbar und fangen keine Maus – erst im Bearbeiten-Modus (Kniffe-Buch offen) */if(await read(`!!document.querySelector('${slot(i)}.empty-slot')&&!!document.querySelector('${slot(i)}').closest('.extra-bar')&&!document.body.classList.contains('bar-edit')`)){await b.press('k');await wait(600);}const p=await center(slot(i));await mouse('mouseMoved',p.x,p.y);await wait(150);await b.send('Input.dispatchKeyEvent',{type:'keyDown',key:'b',code:'KeyB',windowsVirtualKeyCode:66});await b.send('Input.dispatchKeyEvent',{type:'keyUp',key:'b',code:'KeyB',windowsVirtualKeyCode:66});await wait(150);
