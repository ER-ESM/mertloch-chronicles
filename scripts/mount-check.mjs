...
try{
 ...
 await wait(200);await b.press('x');await wait(1800);assert.equal(await run('return g.player.mount;'),'hofpferd');assert.match(await run("const {collectAuras}=await import('./auras.js');return collectAuras(g).buffs.find(a=>a.id==='mount').text;"),/100 % schneller/);await b.screenshot(dir+'/world-horse.png');await b.press('0');assert.equal(await run('return g.player.mount;'),null);pass('Mounting completes and action slot toggles dismount');
