...
try{
 ...
 const all=['klappermofa','blechroller','packesel','drahtesel','rasenkoenig','hofpferd'];for(const id of all)await click('[data-mount-acquire="'+id+'"]');
 assert.deepEqual(await run('return g.mounts.owned;'),all);assert.equal(await run('return g.rpg.coins;'),20);pass('Six mounts acquired through real collection buttons; costs charged once');
 assert.equal(await run('return document.querySelector("[data-mount-speed]").textContent;'),'Reisegeschwindigkeit: +60 %');
 await run('g.rpg.coins=299;');await wait(500);assert.equal(await run('return document.querySelector("[data-riding-skill]").disabled;'),true);
 await run('g.rpg.coins=340;');await wait(500);await click('[data-riding-skill]');assert.equal(await run('return g.mounts.ridingSkill;'),true);assert.equal(await run('return g.rpg.coins;'),40);
 assert.equal(await run('return document.querySelector("[data-riding-skill]").disabled;'),true);assert.equal(await run('return document.querySelector("[data-mount-speed]").textContent;'),'Reisegeschwindigkeit: +100 %');
 await b.screenshot(dir+'/riding-skill.png');pass('Reitskill purchase button checks funds, charges 300 once and displays +100%');
 ...
 await wait(200);await b.press('x');await wait(1800);assert.equal(await run('return g.player.mount;'),'hofpferd');assert.match(await run("const {collectAuras}=await import('./auras.js');return collectAuras(g).buffs.find(a=>a.id==='mount').text;"),/100 % schneller/);assert.ok(await run("const m=await import('./paperdoll-mount.js');return await m.whenPaperdollMount({mount:'hofpferd',look:g.player.look,classId:g.player.classId});"),'paperdoll rider art loads on demand');await wait(300);await b.screenshot(dir+'/world-horse.png');await b.press('0');assert.equal(await run('return g.player.mount;'),null);pass('Mounting completes and action slot toggles dismount');
 await run(`g.emit('save');`);await wait(400);await b.goto(b.url);await wait(700);assert.deepEqual(await run('return g.mounts.owned;'),all);assert.equal(await run('return g.bar()[9].kind;'),'mount');assert.equal(await run('return g.player.mount;'),null);assert.equal(await run('return g.mounts.ridingSkill;'),true);assert.equal(await run("const {mountSpeed}=await import('./mounts.js');return mountSpeed(g);"),2);pass('Collection, riding skill and mount action slot survive reload without a stale active mount');
