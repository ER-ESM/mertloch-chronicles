...
try{
 ...
 await click('#gameMenuButton');await click('[data-shell="clan"]');assert.equal(await b.evaluate('document.querySelectorAll("[data-member-wear],.clan-card.worn").length'),3);await b.screenshot(dir+'/desktop-classes.jpg');await click('[data-member-wear="baerbel"]');assert.equal((await b.state()).classId,'baerbel');assert.equal((await storage()).classId,'baerbel');
 await b.press('Escape');await b.press('c');await click('[data-rpg-clan]');await click('[data-member-wear="kevin"]');assert.equal((await b.state()).classId,'kevin');await b.press('Escape');await b.press('n');const specs=await b.evaluate('[...document.querySelectorAll("[data-spec]")].map(e=>e.dataset.spec)');assert.equal(specs.length,3);assert.ok(specs.every(s=>s.startsWith('kevin-')));await b.press('Escape');checks.push('Real clicks: direct menu and Character > Figure switch; all three figures available; class and three specs update');
