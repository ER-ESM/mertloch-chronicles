...
//  - Kampf über die Trainingsarena (Spielmenü → Einstellungen → Admin, Stufe 6): Fenster nie über Joystick/Kniffen, Ortszeile und leere
...
const AUDIT=`(()=>{
 ...
 return {canvas,dead:!!document.querySelector('#deathScreen:not([hidden])'),textSmall,textTiny,contrast,textCount:textEls.length,toast,toastOverHud:!!toast&&hud.some(h=>overlap(toast,h)),lamps,combat,toplineShown,emptyShown,configureShown,touch:document.body.classList.contains('touch-mode'),vw:innerWidth,vh:innerHeight,scrollW:document.documentElement.scrollWidth,popups,corners,unsafe,safe:S,hand:document.body.dataset.touchHand,stick:stick&&vis(stick)?box(stick):null,skills:skills&&vis(skills)?box(skills):null,
...
try{
 ...
 for(const [name,w,h] of DEVICES){
  ...
  const steps=[['hud',async()=>{}],
   ...
   ['kniffe',async()=>{await b.evaluate(`document.querySelectorAll('.popup-inspection [data-window-close]').forEach(x=>x.click())`);await b.evaluate(`document.querySelector('.game-menu-rail [data-panel="book"]').click()`);await wait(300);}],
   ['kniff-tipp',()=>b.tap('.book-skill:not(.locked)')],
   ['talente',async()=>{await b.evaluate(`document.querySelectorAll('.popup-touchhelp [data-window-close]').forEach(x=>x.click())`);await b.evaluate(`document.querySelector('.game-menu-rail [data-panel="talents"]').click()`);await wait(300);}],
   ...
   ['arena-raeumen',async()=>{await b.evaluate(`document.querySelectorAll('[data-window-close]').forEach(x=>x.click());/* Runde 5a: der Todesbildschirm ist kein Fenster mehr – wer im Übungskampf umkippt, wacht hier auf */document.querySelector('#deathScreen:not([hidden]) [data-ds-wake]')?.click()`);await b.arenaClear();}],
   ...
   ['tod',async()=>{const out=[];await b.evaluate(`document.querySelectorAll('[data-window-close]').forEach(x=>x.click())`);await b.arena('/Boss/',8);let dead=false;for(let i=0;i<60&&!dead;i++){await wait(500);dead=await b.evaluate(`!!document.querySelector('#deathScreen:not([hidden])')`);}if(!dead)out.push('Kein Todesfenster nach 30 s Arena-Boss × 8');await b.rotate(h,w);const r=await b.evaluate(AUDIT);await b.shot(name+'-tod-gedreht');if(dead&&!r.dead)out.push('Todesfenster nach Drehen weg (M-17)');await b.rotate(w,h);return out;}],
   ['tod-zurueck',async()=>{const out=[];if(!await b.evaluate(`!!document.querySelector('#deathScreen [data-ds-wake]')`))return ['Kein Aufsteh-Knopf'];await b.tap('#deathScreen [data-ds-wake]');let r;for(let i=0;i<10;i++){await wait(250);r=await b.evaluate(`({popups:[...document.querySelectorAll('.game-popup')].map(p=>p.dataset.window+(p.hidden||getComputedStyle(p).display==='none'?'(unsichtbar)':'')),dead:!!document.querySelector('#deathScreen:not([hidden])')})`);if(!r.popups.length&&!r.dead)break;}if(r.dead||r.popups.length)out.push('Nach einem Tipp nicht zurück im Spiel: '+r.popups.join(',')+' (M-15)');if(!await b.evaluate(`window.mertloch.state().memories.includes('wurst-ins-gesicht')`))out.push('Todes-Erinnerung fehlt im Tagebuch');await b.arenaClear();return out;}]];
 ...
 const fixture=await b.evaluate(`(async()=>{/* Uhrfehler-Runde: Spielstand des AKTIVEN Helden (E-38: je Held ein Schlüssel, characters.js characterKey). Vorher nahm die Fixture den ersten passenden Schlüssel – das ist der Stand, den das Spiel vor der Heldenwahl anlegt (ohne Held), nicht der geladene; Hofprobe und Beutel kamen nie an (Schritt Beute rot, auch auf main). */const C=await import('/characters.js'),key=C.characterKey(game.world.id,C.activeCharacter(C.loadRoster(localStorage))),raw=localStorage.getItem(key),s=raw?JSON.parse(raw):game.save?.();if(!s)return 'kein Spielstand ('+key+')';if(!s.position||!Number.isFinite(s.position.x))return 'Spielstand ohne Position';s.tutorial={...(s.tutorial||{}),completed:true};s.settings={...(s.settings||{}),autoLoot:false};s.rpg=s.rpg||{};s.rpg.loot=[{id:'drop-fixture',x:s.position.x+24,y:s.position.y,coins:12,items:[{id:'brezel',count:2}],source:{name:'Prüf-Beutel',kind:'enemy'}}];localStorage.setItem(key,JSON.stringify(s));const orig=Storage.prototype.setItem;Storage.prototype.setItem=function(k,v){if(k===key)return;return orig.call(this,k,v);};return 'ok';})()`);
 ...
 // Talente gibt es erst ab Stufe 6 (Freischaltung): wie die Arena-Schritte über Spielmenü → Einstellungen → Admin anheben, ohne Kampf.
 const toLevel6=async()=>{for(const js of [`document.querySelector('.game-menu-rail [data-panel="map"]').click()`,`document.querySelector('#gameMenuButton')?.click()`,`document.querySelector('.popup-menu [data-shell="settings"]')?.click()`,`document.querySelector('[data-opt-cat=system]')?.click()`,`document.querySelector('[data-shell="admin"]')?.click()`,`document.querySelector('[data-arena-level="6"]')?.click()`,`document.querySelectorAll('[data-window-close]').forEach(x=>x.click())`]){await b.evaluate(js);await wait(300);}};
 await toLevel6();
 const desktopSteps=[
  ['figur',()=>b.evaluate(`document.querySelector('.game-menu-rail [data-panel="person"]').click()`)],
  // Kniffe und Talente stehen in eigenen Reitern (seit der Werte-Umstellung E-53), nicht mehr im Figur-Reiter.
  ['kelle',async()=>{await b.evaluate(`document.querySelector('.game-menu-rail [data-panel="book"]')?.click()`);await wait(300);await hover('[data-tooltip-skill="strike"]');/* Soll-Wert aus dem Kampfmodell (content/combat.js) statt fest verdrahtet – überlebt jede Balancing-Runde */const model=await b.evaluate(`import('./content/combat.js').then(m=>m.SKILL_DAMAGE.dieter.strike)`),tip=await b.evaluate(`document.querySelector('#itemTooltip').textContent`)/* Heiler-WoW Teil 3: die Formel steht in den Details (Umschalttaste), sichtbar ist die Zahlenzeile */;assert.match(tip,new RegExp(Math.round(model.weapon*100)+'\\s*% Autoschaden \\+ '+model.flat+' fester Schaden'),'Kelle-Tooltip folgt nicht dem Kampfmodell');}],
  ['talent',async()=>{await b.evaluate(`document.querySelector('.game-menu-rail [data-panel="talents"]')?.click()`);await wait(300);await hover('[data-tooltip-talent="dieter-wall-0"]');assert.match(await b.evaluate(`document.querySelector('#itemTooltip').innerText`),/Betroffene Kniffe:.*Kronkorken-Kelle/s);}],
