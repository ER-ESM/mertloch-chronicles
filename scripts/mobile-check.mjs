...
/** Teil-Läufe (jeder Teil unter 10 Minuten): MOBILE_PART=hoch|quer|klein|sitzung|alle (Vorgabe alle). Teil-Läufe schreiben REPORT-<teil>.md. */
...
async function connect(){
 ...
 return {send,evaluate,errors,close:()=>ws.close(),
  ...
  async shot(name){try{const r=await send('Page.captureScreenshot',{format:'png'});writeFileSync(join(dir,name+'.png'),Buffer.from(r.data,'base64'));}catch(e){errors.push('Bildschirmfoto '+name+' fehlgeschlagen: '+e.message);}},
...
try{
 ...
 for(const [name,w,h] of DEVICES){
  ...
  const steps=[['hud',async()=>{}],
   ...
   ['tod-zurueck',async()=>{const out=[];if(!await b.evaluate(`!!document.querySelector('#respawn')`))return ['Kein Aufsteh-Knopf'];await b.tap('#respawn');let r;for(let i=0;i<10;i++){await wait(250);r=await b.evaluate(`({popups:[...document.querySelectorAll('.game-popup')].map(p=>p.dataset.window+(p.hidden||getComputedStyle(p).display==='none'?'(unsichtbar)':'')),dead:!!document.querySelector('.popup-death')})`);if(!r.popups.length&&!r.dead)break;}if(r.dead||r.popups.length){if(!r.dead&&r.popups.length===1&&r.popups[0]==='memory')out.push('(Hinweis) Nach dem Aufstehen folgt eine Erinnerung – zweiter Tipp nötig (M-15, Backlog ui.md)');else out.push('Nach einem Tipp nicht zurück im Spiel: '+r.popups.join(',')+' (M-15)');}await b.arenaClear();return out;}]];
