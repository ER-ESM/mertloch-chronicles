...
// Stehender Kampf mit einfacher Rotation, echte Zauber, kein Ausweichen, keine Verpflegung – misst Stärkeverhältnisse, nicht Spielspaß.
// Aufruf: node scripts/balance-report.mjs [--json]  → schreibt content/BALANCE-REPORT.md (+ generated/balance-report.json).
import {mkdirSync,writeFileSync} from 'node:fs';
import {Game} from '../engine.js';import {makeEnemy,scaledStats} from '../encounters.js';import {rng} from '../world.js';import {ITEMS,addItem,equipItem} from '../rpg.js';import {registerRoll} from '../itemization.js';import {available} from '../progression.js';import {TALENTS,learnTalent,classSpecs} from '../talents.js';
...
function fight(classId,level,def,seed){const g=player(classId,level,seed),e=makeEnemy({x:classId==='dieter'?30:120,y:0},1,{...def,aggro:true,ai:'combat',bossId:def.id});g.enemies=[e];g.target=e;const hp0=g.player.hp;let steps=0;
 while(e.hp>0&&!g.dead&&steps++<6000){const p=g.player,ready=id=>available(g,id)&&g.cooldowns[id]===0;if(e.cast?.interruptible&&ready('interrupt'))g.action('interrupt');if(g.gcd===0){let id='strike';if(ready('heal')&&p.hp/p.maxHp<.5)id='heal';else if(ready('buff')&&!g.buffs.remaining)id='buff';else if(ready('burst')&&p.runes===3&&e.mark>0)id='burst';else if(ready('mark')&&!e.mark)id='mark';else if(ready('throw')&&classId!=='dieter')id='throw';g.action(id);}g.tick(.05);}
