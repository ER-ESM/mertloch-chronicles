...
// Umland-Gegner wachsen mit dem Spieler – für sie gilt der Feld-Korridor auf jeder Stufe, nicht nur auf der eigenen.
const wildFlag=(row,c)=>{const r=row[c],[lo,hi]=CORRIDOR.field;if(r.deaths===3)return '☠';if(r.deaths)return '⚠';if(r.ttk<2.5)return '·';if(r.ttk<lo)return '⚡';if(r.ttk>hi)return '⏳';return '';};
const wildCell=(row,c)=>`${row[c].ttk} s / −${row[c].lost} % ${wildFlag(row,c)}`.trim();
const wildNotes=[];for(const c of CLASSES){const n=f=>wild.filter(r=>wildFlag(r,c)===f).length,fast=n('⚡')+n('·'),slow=n('⏳'),dead=n('⚠')+n('☠'),ttks=wild.map(r=>r[c].ttk).sort((a,b)=>a-b);if(fast||slow||dead)wildNotes.push('- '+c+': '+[fast&&fast+' von '+wild.length+' Zeilen unter '+CORRIDOR.field[0]+' s',slow&&slow+' über '+CORRIDOR.field[1]+' s',dead&&dead+' mit Toden'].filter(Boolean).join(', ')+' · Median '+ttks[ttks.length>>1]+' s.');}
