...
import {MOUNTS,EQUIPMENT_SLOTS,RARITIES} from './content/index.js';
...
export function mountPresence(raw={}){
 ...
 const eq=mt&&Array.isArray(raw.eq)?raw.eq.slice(0,16).flatMap(e=>{if(!e||!Object.hasOwn(EQUIPMENT_SLOTS,e.slot)||seen.has(e.slot)||!assets.has(e.asset))return [];seen.add(e.slot);return [{slot:e.slot,asset:e.asset,rarity:Object.hasOwn(RARITIES,e.rarity)?e.rarity:'common',hands:[0,1,2].includes(e.hands)?e.hands:null}];}):[];
 return {mt,md,eq};
}
