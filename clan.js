...
import {CLAN_MEMBERS,BASE_SKILLS,KITS,THROW_SKILL,GROUND_SKILL,TALENT_SKILLS,STORY,NPCS,pickTemplates,questReward} from './content/index.js';
...
export function skillsFor(id){const cls=member(id).id,kit=KITS[cls],{names:tn,flavor,text:tt,...throwBase}=THROW_SKILL,{names:gn,...groundBase}=GROUND_SKILL;return [...BASE_SKILLS.map((s,i)=>({...s,...kit[i],offGcd:['dash','interrupt'].includes(s.id)})),buffSkill(cls),{...throwBase,name:tn[cls],text:flavor[cls]+tt},{...groundBase,name:gn[cls]},...talentSkills(cls)];}
