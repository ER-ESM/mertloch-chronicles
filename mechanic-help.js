...
function procText(g,t,id){
 ...
 if(ef.energy)effects.push(`+${ef.energy} Randale`);if(ef.points)effects.push(`+${ef.points} ${g.member.combo} (höchstens 3)`);if(ef.shield)effects.push(`${ef.shield} Grunddeckung`);
...
export function talentHelp(g,t){
 ...
 const extra=effectNumbers(ordinary).map(row=>row.label+': '+n(row.value)+(row.unit?' '+row.unit:'')).join('; ');
...
export function skillHelp(g,id){
 ...
 if(id==='mark')return `„${s.name}“ markiert das Ziel für ${s.duration} s und verursacht einmal pro Sekunde Schaden. Dein Eskalationskniff trifft markierte Ziele stärker und entfernt danach die Markierung.${m?.dot?' '+h.lines[0]+' '+h.lines[1]:m?.chain?' '+h.lines[0]:''}`;
...
export function passiveHelp(g,id=g.member.id){
 ...
 if(id==='baerbel')return `Trifft ${skill('strike')} ${n(p.beatWindow[0])}–${n(p.beatWindow[1])} s nach dem letzten Pinsel-Piekser, erhältst du ${p.beatRunes} Glanz statt 1. Außerhalb dieses Zeitfensters bleibt es bei 1 Glanz. Automatische Angriffe lösen diesen Taktbonus nicht aus. Höchstens 3 Glanz; dein Eskalationskniff verbraucht sie.`+shared;
 ...
 return `${skill('strike')} hat ${n(p.strikeRange/8)} m Grundreichweite und gibt ${p.strikeGain} Randale. ${skill('dash')} hat ${p.dashCd} s Grundabklingzeit. Eine erfolgreiche Unterbrechung mit ${skill('interrupt')} gibt ${p.interruptRunes} Druck und verkürzt die Abklingzeit deines Eskalationskniffs um ${p.interruptBurstCd} s.`+shared;
}
