...
## Fortsetzung: Kleidung an den Gelenken

Die erste Veröffentlichung `3524186` wurde als Build #109 auf GitHub Pages und im Live-Spiel geprüft. Die folgende Verfeinerung schließt zwei Ausrüstungslücken:

- Angelegte Hosen verwenden beim normalen und beidhändigen Lauf die exportierten Hüft-, Knie- und Fußgelenke. Ober- und Unterschenkel teilen dieselbe vorhandene Textur am Knie; der Übergang überlappt um einen Pixel und endet am Stiefelschaft. Bisher lag ein gerades Hosenbein über der gebeugten Körperzeichnung.
- Handschuhe, Armschienen und Ringe werden nach Waffe und Fingerverdeckung gezeichnet. Die ursprünglichen gemalten Finger können angelegte Handschuhe nicht mehr übermalen.
- „Fundstücke anlegen“ in der Bewegungsdemo ergänzt Hose, Handschuhe und Armschienen aus echten gewürfelten Gegenstandsdefinitionen. Die Definitionen bleiben in einem lokalen Demo-Katalog; Spielstände und der gemeinsame Gegenstandskatalog werden nicht verändert.

Prüfung: **399 Tests bestanden**, Produktionsbuild erfolgreich. `tools/redesign/review-gear.browser.js` prüft 384 Darstellungen mit zusätzlicher Ausrüstung, die Bedienung des neuen Schalters und mobile Breite 390 px. Keine leeren/abgeschnittenen Bilder und keine JavaScriptfehler. Das echte Anlegen aller drei Slots wurde zusätzlich im Spiel geprüft. Vergleichsbilder: `assets/redesign/review/gear-joints-before.png`, `gear-joints-after.png`, `gear-mobile.png`, `gear-in-game.png`. Die Rasterquellen und Körperatlanten bleiben bytegleich; diese Änderung verfeinert die Laufzeitkomposition.

## Nutzerbefund: unpassende Inventarbilder in der Detaildemo

Der Screenshot „Anni / Fernkampf / Hygieneeinsatz / 8 von 8“ zeigte tatsächlich den früheren Figurenrenderer: eine statische Standfigur, eine ganze Inventarjacke über dem Rumpf und eine große Sprühflasche davor. Die vorherigen Sichtprüfungen in frischen Browserkontexten haben diesen Zustand nicht erfasst. Testzahlen und transparente Ränder waren dafür kein ausreichender Abnahmenachweis.

Der Screenshot ließ sich exakt reproduzieren, indem die aktuelle Demo mit `live-art.js` aus `30ec4bc` geladen wurde (`assets/redesign/review/anni-mixed-renderer.png`). Ein älterer Service Worker kann eine neue, ihm unbekannte Demoseite aus dem Netz und gemeinsame Module aus seinem alten Cache liefern. Die Demo hatte trotz erfolgreich geladenem Detailkatalog anschließend den allgemeinen Renderer aufgerufen.

Korrektur: `redesign-demo-app.js` lädt `drawDetailedHero()` aus `detailed-hero-art.js` direkt. Das Spiel benutzt denselben Einstieg. Die Kleidungs-Komposition liegt gemeinsam in `equipment-art.js`. Die Demo lädt weder den alten allgemeinen Figurenrenderer noch dessen Ersatzgrafiken. Fehlende Detailgrafik führt zu einer sichtbaren Fehlermeldung, nicht zu einer Ersatzfigur. Der HTML-Einstieg verwendet den neuen Modulnamen; ein Link mit Auswahlparametern holt auch unter dem alten Worker das neue Dokument. Beispiel: `redesign-demo.html?hero=anni&outfit=anni-spray&action=ranged&frame=7`.

Nachweis: `tools/redesign/review-cache.browser.js` blockiert die alten Einstiegsmodule, prüft die gemeldete Auswahl und vergleicht 216 Demo-/Spielbilder pixelgenau. Eine absichtlich unvollständige Lieferung zeigt keine Ersatzfigur. Die korrigierte konkrete Ansicht wurde separat visuell geprüft: `assets/redesign/review/anni-cache-fixed.png`. Vollständige Testsuite nach Übernahme der aktuellen main-Änderungen: **414 bestanden**, Build erfolgreich. Das behebt den nachgewiesenen Renderer-Mix; die automatische Prüfung ersetzt weiterhin keine gestalterische Abnahme jedes Ausrüstungsteils.
