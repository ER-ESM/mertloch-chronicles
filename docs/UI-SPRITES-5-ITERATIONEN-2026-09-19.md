...
Nach Zusammenführung mit Online-Stufe C und dem neuen Chatfenster: **529 Tests bestanden**, Quellen-Wächter und Build sauber. Alle 47 Runtime-Dateien (708.292 Bytes) werden im Offline-Cache erfasst; Quellen, Prompts und Reviewbilder sind im Web-Build ausgeschlossen. Chat und Gruppenrahmen verwenden denselben Adapter. HTML-Kopie wurde auf Bytegleichheit mit der Vorschau geprüft, die Heldenwahl mit Tab/Leertaste. Sichtbeleg: iteration-5-merged-game.png.

Die zusätzliche HUD-Prüfung fand einen bestehenden Fehler der globalen Regel für reduzierte Bewegung: `transition-duration:.01ms` aktiviert unbeabsichtigt Positionsübergänge auf allen Elementen. Beim Loslassen eines skalierten Fensters konnte der Renderer daher noch die alte Position zurücklesen und speichern. Die Dauer ist jetzt tatsächlich null; der HUD-Test erzwingt reduzierte Bewegung und prüft weiterhin dieselben Bewegungs- und Persistenzbedingungen. Eingaben im Test werden mit realistischen Zwischenframes zugestellt. Der Baukasten-Browsertest ist außerdem Teil der Veröffentlichungs-Pipeline.

Abschluss: Der vollständige HUD-Test besteht jetzt inklusive reduzierter Bewegung, Chat- und Meter-Persistenz, Touch und allen sicheren Randabständen. Nach der letzten Zusammenführung (neue Zielwahl) bestanden die elf gezielten UI-/Konto-/Zielwahltests und der Release-Build. Die echte Anmeldung wurde vom Chatknopf aus geöffnet; Registrierungswechsel, Fokus und Dialogbreite sind browsergeprüft (iteration-5-game-login.png, lokale API-Antwort, kein POST).
