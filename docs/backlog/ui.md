...
- [ ] **Dungeon „Schloss Big B"** (wartet auf V-D1 bis V-D11): Dungeon-Karte mit Prospekt und Wirklichkeit, Ebenen, Wegmarke über Ebenen und Minikarte; Eingangskarte, Raumnamen in zwei Zeilen, Durchsagen, Bossrahmen mit Phasen, Zauberleiste mit Ansage und Nachsatz, Siegel- und Beweisfelder, `scripts/dungeon-check.mjs`. Auftrag: [DUNGEON-SCHLOSS-BIG-B-2026-09-23.md](../DUNGEON-SCHLOSS-BIG-B-2026-09-23.md) Abschnitte 5 und 14.
...
## Von UI · 2026-09-18 (Kampftext am Helden, Kniff-Varianten)

- [x] **Scrolling Combat Text** (`combat-text.js`, Vorbild MSBT): drei Laufbereiche um den Helden – links eingehend (Schaden, Heilung, Parade/Ausweichen), rechts ausgehend (eigener Schaden je Kniff), oben Meldungen (Procs, Schwung, Eskalation, Unterbrechen, EP). Zeile = Icon + Wert (+ Kniffname klein), steigt 1,6 s auf und blendet aus; Glückstreffer größer mit „!"; gleiche Treffer innerhalb 350 ms werden summiert. Quelle: Engine-Ereignis `combat` (engine.js `sct()`), Schalter „Kampftext am Helden" in Hilfe → Einstellungen (`settings.sct`). Bei ausgeschaltetem Kampftext laufen die alten Schwebetexte weiter. Anker: `renderer.viewOrigin` (Renderer schreibt den Kamera-Ursprung je Bild).
- [x] **Skill-Leiste: nur offensive Kombos leuchten** (`combat-ui.js` `skillStatus`): Parade, Sprint, Unterbrechen, Heilung, Stärkung und die Talent-Abwehrkniffe bekommen kein `ready` mehr; Proc-Rücksetzer (`procGlow`) leuchten weiterhin auf jedem Kniff.
- [x] **Kniff-Varianten** (`skillStatus().variant`, Klasse `.variant`, `data-variant`/`data-variant-tone`): Icon wechselt Farbton + Beschriftung, solange die Bedingung gilt – Kelle → „Eskalation" (3 Schwung ohne Entladung), „Verstärkt", „Gratis"; Pfandwurf → „Gratis"; Entladung → „RESONANZ" (3 Schwung + Markierung); Proc-Zustände „Gratis"/„Verstärkt"/„Bereit". Gilt für Tastatur- und Touch-Leiste (`[data-skill]`).
- [ ] **Offen:** Debug-Handle `globalThis.__mertloch={game,renderer,combatText}` (app.js) für Browsertests; eigene Icon-Grafiken je Variante (statt Farbfilter) wären sauberer – Bedarf an Art.

## Von UI · 2026-09-18 (E-32, Talentfenster mit Pfaden)
