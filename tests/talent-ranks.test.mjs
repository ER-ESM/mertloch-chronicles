...
test('combat proc probability and damage-based healing really use upgraded ranks',()=>{
 const g=game();for(let i=0;i<5;i++)learnTalent(g,'dieter-brew-1');g.random=()=>.4;g.player.energy=40;assert.equal(fireProcs(g,'markTick',combatStats(g)),1);assert.equal('runes' in g.player,false);assert.equal(g.player.energy,50);unlearnTalent(g,'dieter-brew-1');unlearnTalent(g,'dieter-brew-1');assert.equal(fireProcs(g,'markTick',combatStats(g)),0);
