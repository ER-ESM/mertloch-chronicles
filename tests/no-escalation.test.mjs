...
test('all nine specializations can cast their special skill immediately with no shared build-up',()=>{
 for(const [cls,specs] of Object.entries(CLASS_SPECS))for(const spec of specs){
  const {g,e}=fight(cls,spec),before=e.hp;g.player.energy=70;
  ...
  assert.ok(e.hp<before,spec);assert.equal(g.player.energy,35,spec);assert.ok(g.cooldowns.burst>0,spec);assert.equal(g.action('burst'),false,spec);assert.equal('runes' in g.player,false,spec);
...
test('legacy point values cannot change damage; early strikes no longer auto-escalate',()=>{
 for(const cls of Object.keys(CLASS_SPECS)){
...
test('converted talents provide energy and burst procs no longer need a three-point condition',()=>{
 const {g}=fight('baerbel','baerbel-care');learnCoreBuild(g,'baerbel-care');g.player.energy=20;g.player.hp-=100;assert.ok(g.action('heal'));finish(g);assert.equal(g.player.energy,30);assert.equal('runes' in g.player,false);
 const {g:k}=fight('kevin');k.player.energy=20;assert.equal(fireProcs(k,'burst',{'proc:rueckstrom':1}),1);assert.equal(k.player.energy,45);
