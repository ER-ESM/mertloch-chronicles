...
test('beat and own live keg zone dispatch their talent rules once per matching strike',()=>{
 ...
 g.time=11;const likes0=g.player.energy;cast(g,'strike');assert.equal(fired(g,'perfekter-upload'),1);/* E-72: Takt gibt Likes obendrauf */assert.ok(g.player.energy>=Math.min(100,likes0+RESOURCES.baerbel.beat.likes));
...
test('class passives drive runtime skill values, damage reduction, parry, beat and interrupt bonuses',()=>{
 ...
 assert.equal(k.skills.find(s=>s.id==='strike').range,240);const bottles=k.res.bottles;cast(k,'strike');/* E-72: Kevin wirft eine Flasche */assert.equal(k.res.bottles,bottles-1);
