...
test('real damage and dodge set bounded animation timers and respawn clears them',()=>{
 ...
 const caster=new Game(world,{level:6});caster.player.energy=100;/* E-72 */assert.ok(caster.action('buff'));assert.equal(redesignPose(caster.player),'cast');for(let i=0;i<7;i++)caster.tick(.05);assert.equal(caster.player.castPose,0);
});
