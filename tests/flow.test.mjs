...
const game=(save={level:6},classId='dieter')=>{const g=new Game(arena(),{...save,classId});g.random=()=>.5;g.player.x=g.player.y=0;g.player.hp=g.player.maxHp;g.player.energy=100;/* E-71: Dieter startet sonst bei 0 Randale */return g;};
const build=(g,spec)=>{g.player.x=g.world.spawn.x;g.player.y=g.world.spawn.y;assert.ok(changeSpec(g,spec),'spec '+spec);learnCoreBuild(g,spec);g.player.x=g.player.y=0;};
...
test('E-71: Randale fließt im Kampf nicht nach und verraucht danach; Likes füllen sich nur außerhalb des Kampfes',()=>{const g=game();g.player.inCombat=7;g.player.energy=0;g.tick(.05);const start=g.player.energy;assert.ok(Math.abs(start-RESOURCES.dieter.combatStart)<.1,'Kampfeslust zu Kampfbeginn');for(let i=0;i<20;i++){g.player.inCombat=7;g.tick(.05);}assert.ok(g.player.energy-start<.5,'im Kampf fließt nur über Bastelgrips etwas nach');g.player.energy=50;g.player.inCombat=0;for(let i=0;i<140;i++)g.tick(.05);assert.ok(g.player.energy<50,'Randale verraucht außerhalb des Kampfes');const a=game({level:6},'baerbel');a.player.inCombat=0;a.player.energy=10;a.tick(.5);assert.ok(a.player.energy>10);a.player.inCombat=7;const v=a.player.energy;a.tick(.5);assert.ok(a.player.energy-v<.1,'im Kampf nur Bastelgrips');});
