...
test('Mounts: two accounts see riding, gear, dismount and reconnect; collections use separate hero saves',async t=>{
 ...
 wa.send(JSON.stringify({...pos,mt:null,eq:[{slot:'body',id:'kutte',asset:'jacket',rarity:'rare'},{slot:'head',id:'erfunden',asset:'cap',rarity:'common'}]}));await sleep(180);assert.equal(snap().mt,null);assert.equal(snap().eq[0].id,'kutte','Ausrüstung bleibt zu Fuß sichtbar, bekannte Kennung wird durchgereicht');assert.equal(snap().eq[1]?.id,undefined,'unbekannte Kennung fällt weg');
