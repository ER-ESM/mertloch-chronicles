...
test('die Zahlen im info-Block sind abgeleitet, nicht abgeschrieben', () => {
 for(const [id,d] of Object.entries(ITEM_CATALOG)){
  ...
  if(d.stats?.finesse)assert.equal(by['Glückstreffer-Chance daraus'],Math.round(ratingShare('crit',d.stats.finesse,d.level||1)*1000)/10,id+': Umrechnung Taktgefühl → Glückstreffer falsch');
 }
 ...
 assert.ok(ratingShare('armorRating',10,1)>ratingShare('armorRating',10,10),'Rüstung wirkt auf höherer Stufe schwächer');
 assert.ok(ratingShare('crit',10,1)>ratingShare('crit',10,20),'E-56: Glückstreffer-Kurs steigt mit der Stufe');assert.ok(ratingShare('haste',10,1)>ratingShare('haste',10,20),'E-56: Tempo-Kurs steigt mit der Stufe');
});
