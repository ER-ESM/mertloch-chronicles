...
window.redesignDemo={ready:true,state,draw,player};redesignArt.changed=draw;draw();let last=0;function tick(t){if(state.playing){state.phase=(state.phase+Math.min(.1,(t-last)/1000)*8*(state.action==='walk'?122/redesignArt.catalog.stride:1))%8;draw();}last=t;requestAnimationFrame(tick);}requestAnimationFrame(tick);
