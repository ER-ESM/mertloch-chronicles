...
export function partyMemberFrame(x,{leader,selected,world,targetHint='Als Ziel wählen',revive='Aufhelfen',targetName='',ready=null,pets=[],outOfRange=false}={}){
 ...
 return `<div data-party-name="${escapeUi(x.n)}" class="party-member unit-frame${far?' far':''}${dead?' dead':''}${health>0&&health<=25?' is-low-health':''}${selected===x.n?' is-selected':''}${ready?' ready-'+ready:''}${outOfRange&&!far?' out-of-range':''}"><button type="button" class="unit-select" data-party-select aria-pressed="${selected===x.n}" title="${escapeUi(targetHint)}">${unitPortrait(x.c,x.l)}<span class="unit-content"><span class="party-name">${x.n===leader?'♛ ':''}${escapeUi(x.n)}${ready?`<b class="ready-mark" aria-label="${ready==='yes'?'bereit':ready==='no'?'nicht bereit':'wartet'}">${ready==='yes'?'✓':ready==='no'?'✗':'…'}</b>`:''}</span><span class="party-hp" role="progressbar" aria-label="Leben" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${health}"><i style="width:${health}%"></i><span>${Math.round(health)} %</span></span><small class="unit-status">${dead?'Besiegt':far?'Andere Welt':targetName?`<b class="unit-target">⚔ ${escapeUi(targetName)}</b>`:x.s==='combat'?'Im Kampf':'Bereit'}</small></span></button>${dead&&!far?`<button type="button" data-party-revive>${escapeUi(revive)}</button>`:''}${pets.length?`<div class="party-pets">${pets.map(p=>`<span class="party-pet${p.down?' dead':''}" data-role="${escapeUi(p.role||'')}" title="${escapeUi(p.name)}"><b>${escapeUi(p.name)}</b><i><i style="width:${Math.max(0,Math.min(100,Number(p.hp)||0))}%"></i></i></span>`).join('')}</div>`:''}</div>`;
}
