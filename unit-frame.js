...
export function partyMemberFrame(x,{leader,selected,world,aidHint='Als Hilfsziel wählen',revive='Aufhelfen'}={}){
 const far=x.w!==world,dead=x.s==='dead',health=Math.max(0,Math.min(100,Number(x.h)||0));
 return `<div data-party-name="${escapeUi(x.n)}" class="party-member unit-frame${far?' far':''}${dead?' dead':''}${health>0&&health<=25?' is-low-health':''}${selected===x.n?' aid':''}"><button type="button" class="unit-select" data-party-select aria-pressed="${selected===x.n}" title="${escapeUi(aidHint)}">${unitPortrait(x.c,x.l)}<span class="unit-content"><span class="party-name">${x.n===leader?'★ ':''}${escapeUi(x.n)}</span><span class="party-hp" role="progressbar" aria-label="Leben" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${health}"><i style="width:${health}%"></i><span>${Math.round(health)} %</span></span><small class="unit-status">${dead?'Besiegt':far?'Andere Welt':x.s==='combat'?'Im Kampf':'Bereit'}</small></span></button>${dead&&!far?`<button type="button" data-party-revive>${escapeUi(revive)}</button>`:''}</div>`;
}
