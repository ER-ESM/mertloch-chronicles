...
export function mountIntro(host){
 ...
 function show(i){
  ...
  const still=el.querySelector('.intro-still'),src=s.image&&contentPath(s.image);still.hidden=true;
  still.onload=()=>{still.hidden=!running||index!==i;};still.onerror=()=>{still.hidden=true;};
