...
import {settingsWindow} from './options-ui.js';
...
function sections(w,definitions){const body=w.body,boxes=[];
 ...
 if(boxes.length>=3)jumpBar(w,boxes);
}
/** Sprungleiste (E-27): ein Reiter, mehrere Abschnitte, eine klebende Leiste, die hinführt und mitläuft. */
function jumpBar(w,boxes){const body=w.body,nav=document.createElement('nav');nav.className='section-jump';nav.setAttribute('aria-label',UI.bookTabs);
 for(const box of boxes){const b=button(box.dataset.section,()=>{box.scrollIntoView({block:'start',behavior:'smooth'});mark(box);});b.dataset.jump=box.dataset.section;nav.append(b);}
 const mark=box=>{for(const b of nav.children)b.setAttribute('aria-current',b.dataset.jump===box.dataset.section?'true':'false');};
 body.prepend(nav);mark(boxes[0]);
 body.addEventListener('scroll',()=>{const line=body.getBoundingClientRect().top+nav.offsetHeight+24;const current=boxes.find(box=>box.getBoundingClientRect().bottom>line)||boxes[boxes.length-1];mark(current);},{passive:true});
}
function tabs(w,definitions){const body=w.body,nav=document.createElement('nav'),panes=[];nav.className='panel-tabs';nav.setAttribute('role','tablist');for(const [name,selectors]of definitions){if(!body.querySelector(selectors))continue;/* leere Reiter (z. B. Vergleich bei Verpflegung) fallen weg */const pane=document.createElement('section');pane.className='panel-page';pane.setAttribute('role','tabpanel');for(const el of [...body.querySelectorAll(selectors)])pane.append(el);panes.push(pane);body.append(pane);const b=button(name,()=>show(panes.indexOf(pane)));b.setAttribute('role','tab');nav.append(b);}body.prepend(nav);function show(index){w.activePage=Math.min(index,panes.length-1);panes.forEach((p,i)=>{p.hidden=i!==w.activePage;nav.children[i].setAttribute('aria-selected',String(i===w.activePage));nav.children[i].tabIndex=i===w.activePage?0:-1;});}nav.onkeydown=e=>{if(!['ArrowLeft','ArrowRight'].includes(e.key))return;e.preventDefault();show((w.activePage+(e.key==='ArrowRight'?1:panes.length-1))%panes.length);nav.children[w.activePage].focus();};show(w.activePage||0);nav.addEventListener('click',()=>adaptPanel(w));}
export function decoratePanel(w){const b=w.body;if(w.id==='settings')queueMicrotask(()=>settingsWindow(w));/* Runde 5b: Handy-Einstellungen ohne Scrollen */resetPanelFlow(b);if(['dialog','detail','touchhelp','activity','inspection'].includes(w.id))w.flowPages={};b.classList.add('compact-panel');if(['person','book','talents'].includes(w.id)){b.querySelector('.gear-inspection')?.remove();b.querySelector('.rpg-foot')?.remove();const clan=b.querySelector('.person-clan');if(clan&&!clan.querySelector('[data-member-wear]:not([disabled])')){const note=clan.querySelector('.disabled-note')?.textContent||'';clan.innerHTML='<p class="clan-hint">'+note+'</p>';}const skills=b.querySelector('.person-skills');if(skills){[...skills.querySelectorAll('.book-instruction,.touch-book-help')].filter(e=>!e.textContent.includes(UI.pending)).forEach(e=>e.remove());skills.querySelector('.book-intro')?.remove();}/* Runde 3b (2026-09-24): die Figur hat am Handy keine Reiter mehr (window-compact.js verdichtet wie am Desktop) – nur sehr schmale Hochkant-Schirme (≤ 370 px) teilen Ausrüstung und Werte, sonst müsste das Fenster scrollen */if(w.id==='person'&&document.body.classList.contains('touch-mode')&&(b.querySelector('.person-skills')||b.querySelector('.person-clan')||(innerWidth<=370&&innerHeight>innerWidth)))tabs(w,[[UI.equipment,'.rpg-heading,.armory-banner,.equipment-grid,.starter-armory,.item-stats'],...(skills?[[UI.skills,'.person-skills']]:[]),...(b.querySelector('.person-talents')?[[UI.talents,'.person-talents']]:[]),[UI.stats,'.weapon-summary,.character-stats,.secondary-stats,.meter-entry-wrap'],...(b.querySelector('.person-clan')?[[UI.band,'.person-clan']]:[])]);b.querySelectorAll('[data-rpg-panel="talents"],[data-rpg-panel="bag"]').forEach(e=>e.remove());}
 ...
 if(w.id==='bag'){b.querySelector('.rpg-foot')?.remove();b.querySelector('.selected-item')?.setAttribute('hidden','');b.querySelector('.rpg-foot small')?.remove();}
