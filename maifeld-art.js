import {WORLD_ART_DENSITY,contextScale,bakedGrade,withGrade} from './art-quality.js';
...
export function fillMaifeldGround(c,name,x,y,w,h,alpha=1){const a=maifeld[name];if(!a)return false;let tile=groundTiles.get(name);if(!tile){const size=128*WORLD_ART_DENSITY;tile=document.createElement('canvas');tile.width=tile.height=size*2;const tc=tile.getContext('2d');tc.imageSmoothingEnabled=true;tc.imageSmoothingQuality='high';for(let row=0;row<2;row++)for(let col=0;col<2;col++){tc.save();tc.translate(col?size*2:0,row?size*2:0);tc.scale(col?-1:1,row?-1:1);tc.drawImage(a.image,a.x,a.y,a.w,a.h,0,0,size,size);tc.restore();}normalizeArt(tile,true);groundTiles.set(name,tile);}c.save();c.globalAlpha*=alpha;const pattern=c.createPattern(tile,'repeat');pattern.setTransform({a:1/WORLD_ART_DENSITY,d:1/WORLD_ART_DENSITY});c.fillStyle=pattern;c.fillRect(x,y,w,h);c.restore();return true;}
export function drawMaifeld(c,name,x,y,height,width=null,anchor=.5){const a=maifeld[name];if(!a)return false;const h=Math.max(1,Math.round(height)),w=Math.max(1,Math.round(width??h*a.w/a.h)),d=Math.max(1,Math.min(WORLD_ART_DENSITY,Math.ceil(contextScale(c)-.01))),key=name+':'+w+':'+h+':'+d+bakedGrade.filter;let mip=mipmaps.get(key);if(!mip){/* Dichte des Zeichenraums (E-49): 1:1 kopieren statt je Bild von Dichte 4 herunterzurechnen */mip=document.createElement('canvas');mip.width=w*d;mip.height=h*d;const mc=mip.getContext('2d');mc.imageSmoothingEnabled=true;mc.imageSmoothingQuality='high';withGrade(mc,()=>mc.drawImage(a.image,a.x,a.y,a.w,a.h,0,0,mip.width,mip.height));normalizeArt(mip,true);mipmaps.set(key,mip);if(mipmaps.size>300)mipmaps.delete(mipmaps.keys().next().value);}c.imageSmoothingEnabled=false;c.drawImage(mip,Math.round(x-w*anchor),Math.round(y-h),w,h);return true;}
