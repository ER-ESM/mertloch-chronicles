...
export function drawMaifeld(c,name,x,y,height,width=null,anchor=.5){const a=maifeld[name];if(!a)return false;const h=Math.max(1,Math.round(height)),w=Math.max(1,Math.round(width??h*a.w/a.h)),key=name+':'+w+':'+h;let mip=mipmaps.get(key);if(!mip){mip=document.createElement('canvas');mip.width=w*WORLD_ART_DENSITY;mip.height=h*WORLD_ART_DENSITY;const mc=mip.getContext('2d');mc.imageSmoothingEnabled=true;mc.imageSmoothingQuality='high';mc.drawImage(a.image,a.x,a.y,a.w,a.h,0,0,mip.width,mip.height);normalizeArt(mip,true);mipmaps.set(key,mip);if(mipmaps.size>300)mipmaps.delete(mipmaps.keys().next().value);}c.imageSmoothingEnabled=false;c.drawImage(mip,Math.round(x-w*anchor),Math.round(y-h),w,h);return true;}
