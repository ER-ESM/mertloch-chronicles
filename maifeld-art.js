...
export function fillMaifeldGround(c,name,x,y,w,h,alpha=1){const a=maifeld[name];if(!a)return false;let tile=groundTiles.get(name);if(!tile){const size=128;tile=document.createElement('canvas');tile.width=tile.height=size*2;const tc=tile.getContext('2d');tc.imageSmoothingEnabled=true;tc.imageSmoothingQuality='high';for(let row=0;row<2;row++)for(let col=0;col<2;col++){tc.save();tc.translate(col?size*2:0,row?size*2:0);tc.scale(col?-1:1,row?-1:1);tc.drawImage(a.image,a.x,a.y,a.w,a.h,0,0,size,size);tc.restore();}normalizeArt(tile,true);groundTiles.set(name,tile);}c.save();c.globalAlpha*=alpha;c.fillStyle=c.createPattern(tile,'repeat');c.fillRect(x,y,w,h);c.restore();return true;}
