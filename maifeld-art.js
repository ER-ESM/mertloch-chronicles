import {WORLD_ART_DENSITY,contextScale} from './art-quality.js';
...
export function drawMaifeld(c,name,x,y,height,width=null,anchor=.5){const a=maifeld[name];if(!a)return false;const h=Math.max(1,Math.round(height)),w=Math.max(1,Math.round(width??h*a.w/a.h)),d=Math.max(1,Math.min(WORLD_ART_DENSITY,Math.ceil(contextScale(c)-.01))),key=name+':'+w+':'+h+':'+d;let mip=mipmaps.get(key);if(!mip){/* Dichte des Zeichenraums (E-49): 1:1 kopieren statt je Bild von Dichte 4 herunterzurechnen */mip=document.createElement('canvas');mip.width=w*d;mip.height=h*d;const mc=mip.getContext('2d');mc.imageSmoothingEnabled=true;mc.imageSmoothingQuality='high';mc.drawImage(a.image,a.x,a.y,a.w,a.h,0,0,mip.width,mip.height);normalizeArt(mip,true);mipmaps.set(key,mip);if(mipmaps.size>300)mipmaps.delete(mipmaps.keys().next().value);}c.imageSmoothingEnabled=false;c.drawImage(mip,Math.round(x-w*anchor),Math.round(y-h),w,h);return true;}
