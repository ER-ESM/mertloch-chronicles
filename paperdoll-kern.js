...
export const ORDER=['legs','feet','body','charm','head','weapon','offhand'];
export function sources(set,gear){const items=[...set].sort((a,b)=>ORDER.indexOf(gear[a].slot)-ORDER.indexOf(gear[b].slot));
 return ['koerper',...(items.some(i=>gear[i].slot==='head')?[]:['dutt']),...items];}
...
export function composeCore(W,H,bands,srcs,tile){const out=new Uint8ClampedArray(W*H*4),dark=new Uint8Array(W*H);
 for(const band of bands)for(const s of srcs){const t=tile(s,band);if(!t)continue;
