...
function paint(c,id,x,y,row,frame,magnify){const equipment=$('gear').value==='none'?[]:[{slot:'legs',asset:'trouser'},{slot:'feet',asset:'boot'},{slot:'hands',asset:'glove'},{slot:'wrists',asset:'bracer'},{slot:'weapon',asset:$('gear').value==='heavy'?'maul':'club',hands:$('gear').value==='heavy'?2:1}];const p={direction:dirs[row],moving:true,walkDistance:frame/8*(contentActor(id)?.stride||80),artPose:'walk-'+frame,visualEquipment:equipment,artMagnify:magnify};
...
function draw(){const id=$('actor').value,cv=$('directions'),c=cv.getContext('2d');cv.width=cv.clientWidth;cv.height=340;c.clearRect(0,0,cv.width,cv.height);const f=Math.floor(phase)%8,magnify=innerWidth<600?2:8;for(let row=0;row<4;row++){const x=cv.width*(row+.5)/4;paint(c,id,x,270,row,f,magnify);c.fillStyle='#e8d4ac';c.font='14px sans-serif';c.fillText(dirs[row].toUpperCase(),x-12,315);}const strip=$('phases'),s=strip.getContext('2d');s.clearRect(0,0,1920,300);for(let i=0;i<8;i++){paint(s,id,120+i*240,240,0,i,8);s.fillStyle='#e8d4ac';s.font='14px sans-serif';s.fillText(['Kontakt','Absenken','Vorbeischwingen','Anheben'][i%4]+' '+(i<4?'A':'B'),50+i*240,285);}$('status').textContent=people.length+' menschliche Figuren · 4 Richtungen · 8 Phasen · aktuell '+id+' / '+f;}
for(const id of ['actor','gear','markers','tempo'])$(id).onchange=draw;$('pause').onclick=()=>{playing=!playing;$('pause').textContent=playing?'Pause':'Weiter';};$('step').onclick=()=>{playing=false;phase=(Math.floor(phase)+1)%8;draw();};
window.gaitReview={people,draw,set(id,frame=0){$('actor').value=id;phase=frame;playing=false;draw();}};
requestAnimationFrame(function loop(now){if(playing)phase=(phase+Math.min(.05,(now-(last||now))/1000)*8*Number($('tempo').value))%8;last=now;draw();requestAnimationFrame(loop);});
