...
import {tryQueue,tickQueue} from './spell-queue.js';
...
export class Game{
  constructor(world,saved={},options={}){
    ...
    this.settings={autoLoot:saved.settings?.autoLoot!==false,prerender:saved.settings?.prerender===true,sct:saved.settings?.sct!==false,light:saved.settings?.light!==false,fx:saved.settings?.fx!==false,fps:saved.settings?.fps===true,fullRes:saved.settings?.fullRes===true,autoRes:saved.settings?.autoRes!==false,lowRes:saved.settings?.lowRes===undefined?(saved.settings?.light===false&&saved.settings?.fx===false)||touchDevice():saved.settings.lowRes===true,namesFriendly:saved.settings?.namesFriendly!==false,namesEnemy:saved.settings?.namesEnemy!==false,namesPlayers:saved.settings?.namesPlayers!==false,sctIn:saved.settings?.sctIn!==false,sctNotes:saved.settings?.sctNotes!==false,sctCompanions:saved.settings?.sctCompanions!==false};
  ...
  action(id,point=null,completing=false,friend=this.friend){
    ...
    dismount(this);
    if(id==='auto')return toggleAuto(this);if(this.casting&&!completing){if(id==='dash')this.casting=null;else if(!s.offGcd){/* Zauber-Puffer (spell-queue.js, Runde 5a): in den letzten 0,4 s vormerken */if(tryQueue(this,s,{point,friend})!=='queued')this.fail(COMBAT_TEXT.busy);return false;}}
    const p=this.player,cs=combatStats(this),cost=procFree(this,id)?0:resourceCost(this,s,cs,skillCostMech(this,s,skillCost(this,s,cs))),context={interrupted:!!this.target?.cast?.interruptible};const failure=beforeSkill(this,s,cs)||resourcePrecheck(this,id,s,cs);if(failure){this.fail(failure);return false;}if(this.cooldowns[id]>.01&&!completing&&tryQueue(this,s,{point,friend})==='queued')return false;if(this.cooldowns[id]>.01){this.fail(COMBAT_TEXT.cooldown?.(s.name,deNum(this.cooldowns[id],1))||`${s.name} ist noch nicht bereit · ${deNum(this.cooldowns[id],1)} s.`);return false;}
    if(!completing&&!s.offGcd&&!resourceOffGcd(this,id)&&this.gcd>0&&!(s.ground&&!point)){/* Zauber-Puffer statt stumm verschlucken (Runde 5a, Kenner-Befund 6): in den letzten 0,4 s der GCD vormerken, vorher rote Zeile */if(tryQueue(this,s,{point,friend})!=='queued')this.fail(COMBAT_TEXT.notReady||'Noch nicht bereit.');return false;}
  ...
  hitPlayer(e,n,avoidable=true,share=0){if(this.dead)return;dismount(this);if(robbiTaunt(this,e,n))return;beginMeterCombat(this,e);if(this.adminGod)return;/* Admin-Konsole: god */const p=this.player,fresh=p.inCombat<=0;if(p.invulnerable>0&&avoidable){this.stats.dodges++;emitCombatFx(this,'dodge',p,{from:{x:e.x,y:e.y}});fireProcs(this,'dodge',combatStats(this));if(!this.sct({area:'in',kind:'avoid',text:'AUSGEWICHEN',skill:'dash'}))this.float(p.x,p.y-25,'AUSGEWICHEN','#b8e0d3');return;}
    ...
    const guardBefore=(this.classState.guard||0)+(this.buffs.remaining>0?this.buffs.shield||0:0),hpBefore=p.hp,cs=combatStats(this);n=share>0?Math.round(p.maxHp*share):Math.round(n*(e.damage||1)*resourceIncoming(this,e)*gapDamage(this,e)/* Stufenabstand (foe-rules.js) */*(1-cs.armor)*(p.hp/p.maxHp<.35?1-(cs.lastStand||0)-(cs.procs.includes('stout')?.08:0):1));if(this.buffs.remaining>0){n=Math.round(n*(1-(this.buffs.reduction||0)));const absorbed=Math.min(n,this.buffs.shield||0);this.buffs.shield=Math.max(0,(this.buffs.shield||0)-absorbed);n-=absorbed;}if(this.partyBuff?.remaining>0){n=Math.round(n*(1-(this.partyBuff.reduction||0)));const soaked=Math.min(n,this.partyBuff.shield||0);this.partyBuff.shield-=soaked;n-=soaked;}n=Math.round(resourceHit(this,Math.round(modifyHit(this,n,cs)*(this.baseEffects().damageTaken??1)*(p.cert?.until>this.time?1+p.cert.stacks*p.cert.taken:1)/* Dungeon Etappe 3: Zertifikat (Siegelring) */),e,cs,{noTab:share>0}));/* E-72: Zeche; Anteil-Treffer der Dungeons kommen voll an */onHitTakenMech(this,n,cs);p.hp=Math.max(0,p.hp-n);const absorbedFx=guardBefore-(this.classState.guard||0)-(this.buffs.remaining>0?this.buffs.shield||0:0);if(absorbedFx>0){emitCombatFx(this,'guard',p,{amount:absorbedFx,absorbed:true,from:{x:e.x,y:e.y}});resourceAbsorbed(this,e,absorbedFx);}if(p.hp<hpBefore)emitCombatFx(this,'hurt',p,{amount:hpBefore-p.hp,from:{x:e.x,y:e.y}});if(n>0)p.hurt=.16;p.inCombat=7;if(p.hp>0&&p.hp/p.maxHp<.35)fireProcs(this,'lowHealth',cs);if(e.arena)(this.arenaStats||(this.arenaStats=freshArenaStats())).taken+=n;this.noteAttacker(e,n,fresh);if(!this.sct({area:'in',kind:'damage',value:n,text:e.name,iconKey:'claw'}))this.float(p.x,p.y-18,'−'+n,'#f09a81');this.emit('shake',{strength:1.7});this.emit('sound',{id:'hit'});if(p.hp===0)this.die(e.name,e.lastCast?.name||'',!!e.lastCast?.ground);
  }
  ...
  die(by,skill='',ground=false){if(this.dead)return;this.dead=true;finishMeterCombat(this);stopAuto(this,false);this.casting=null;this.keys.clear();this.moveTo=null;this.path=[];this.routeGoal=null;/* E-72 R4: ein angehaltener Laufweg lief nach dem Aufwachen weiter – zurück ins Rudel */this.autopilot=null;this.autopilotResume=null;this.memoryEvent({kind:'firstDeath'});/* Ursache für den Todesbildschirm (death-screen.js, Runde 5a) */this.emit('death',{by,skill,ground});}
