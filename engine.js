...
export class Game{
  ...
  navigate(point){if(this.dead||this.paused||!point||!Number.isFinite(point.x)||!Number.isFinite(point.y)||!tutorialAllowsTravel(this,point))return false;this.casting=null;this.keys.clear();this.routeGoal={x:point.x,y:point.y,...(typeof point.name==="string"&&point.name?{name:point.name}:{})};this.routeStuck=0;this.routeRetried=false;const walk=this.walkWorld();try{this.path=walk.findPath(this.player,point);}catch{this.path=[];}/* unerreichbar: so nah wie möglich heran (path-near.js) */if(!this.path.length){this.path=pathNear(walk,this.player,point);if(this.path.length)this.routeGoal={...this.path.at(-1)};}this.moveTo=this.path.shift()||null;if(!this.moveTo){this.routeGoal=null;this.toast(NO_PATH);return false;}/* Autopilot hält beim ersten neuen Angreifer (autopilot.js) */startAutopilot(this);return true;}
  ...
  die(by,skill='',ground=false){if(this.dead)return;this.dead=true;finishMeterCombat(this);stopAuto(this,false);this.casting=null;this.keys.clear();this.moveTo=null;this.path=[];this.routeGoal=null;this.memoryEvent({kind:'firstDeath'});/* Ursache für den Todesbildschirm (death-screen.js, Runde 5a) */this.emit('death',{by,skill,ground});}
  ...
  tick(dt){
    ...
    for(const e of this.enemies){
      ...
      if(!e.aggro&&e.ai!=='returning'&&e.behavior==='aggressive'&&e.spawnGrace<=0&&d<entryAggroRange(this,e)&&!inSanctuary(this.world,p)&&this.world.lineClear(e,p)&&(!e.dungeon||dungeonNotices(this,e))){e.aggro=true;e.ai='combat';e.attackTimer=COMBAT_RULES.firstSpecial;adoptAttacker(this,e);autopilotThreat(this,e,'aggro');}
