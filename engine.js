...
import {burstMultiplier,afterBurst,onGroundMech,damageMultiplier,onMarkExpire,onStrikeMech,onHitTakenMech,onKillMech,isMobile,quickGcd,skillCostMech,onHealMech} from './spec-mechanics.js';
...
export class Game {
  ...
  switchMember(id){if(this.dead||this.paused||!this.atHub()){this.toast(hubRule('Clanwechsel'));return false;}if(member(id).id!==id)return false;finishMeterCombat(this);this.member=member(id);this.rpg.talents=this.rpg.talentBuilds[id];this.player.classId=id;this.lastStrike=-100;this.resetClassState();this.refreshStats();this.target=null;this.emit('classChanged');this.emit('save');return true;}
  ...
  action(id,point=null,completing=false){
    ...
    if(s.castTime&&!completing){if(!isMobile(this,s)&&movingToCast(this)){this.toast(COMBAT_TEXT.moving);return false;}this.casting={id,name:s.name,point:point?{...point}:null,targetId:s.range&&!s.ground?e.id:null,remaining:s.castTime,total:s.castTime};if(!s.offGcd)this.gcd=quickGcd(this,id,e)?Math.min(cs.gcd,BALANCE.player.gcdQuick||1):cs.gcd;this.aiming=null;this.aimPoint=null;return true;}
    ...
    if(id==='strike'){const empowered=this.classState.empowered>0;if(empowered)this.classState.empowered--;this.damage(e,skillDamage(this,s,s.damage,ITEMS)*(empowered||this.classState.freeStrike?2:1)*pm,'Kelle');onStrikeMech(this,e,cs);const passives=this.member.passives||{},window=passives.beatWindow,beat=!!window&&this.time-this.lastStrike>=window[0]&&this.time-this.lastStrike<=window[1];p.runes=Math.min(3,p.runes+(beat?(passives.beatRunes??1):1));this.lastStrike=this.time;context.beat=beat;if(p.runes>=3&&!available(this,'burst')&&COMBAT_RULES.earlyEscalation){const extra=skillDamage(this,s,s.damage,ITEMS)*(COMBAT_RULES.earlyEscalation.factor-1);p.runes=0;this.damage(e,extra,'Eskalation');if(!this.sct({area:'note',kind:'momentum',text:COMBAT_TEXT.earlyEscalation,skill:'strike',color:'#ffe08a'}))this.float(e.x,e.y-34,COMBAT_TEXT.earlyEscalation,'#ffe08a');emitCombatFx(this,'burst',e,{radius:28});}if(beat)this.float(p.x,p.y-35,APEROL_TEXT.combo,this.member.color);p.energy=Math.min(100,p.energy+s.gain);}
    ...
    if(id==='mark'){e.mark=s.duration;e.dotTimer=1;e.dotDamage=s.dot;e.slow=s.slow||1;this.float(e.x,e.y-23,'MARKIERT!','#9fdacb');this.log(s.name+' · 10 s für deine nächste Eskalation.');}
    if(id==='burst'){const runes=p.runes,marked=e.mark>0;p.runes=0;let n=skillDamage(this,s,s.base+runes*s.perPoint,ITEMS,runes)*(marked?s.multiplier:1)*pm;if(runes===3)fireProcs(this,'burst3',cs);if(this.relic)n*=1.12;if(this.rpg.talents.spec==='dieter-brawl'&&this.classState.rage>=5)n*=1.25;if(this.rpg.talents.spec==='kevin-iron')n*=.7;n*=burstMultiplier(this,e,cs,context);this.damage(e,Math.round(n),marked&&runes===3?'RESONANZ':'Entladung');afterBurst(this,e,cs,Math.round(n),context);if(s.splash)for(const other of this.enemies){if(other!==e&&other.hp>0&&other.ai!=='returning'&&other.spawnGrace<=0&&distance(e,other)<s.splash&&this.world.lineClear(e,other))this.damage(other,Math.round(n*.55)*(1+(cs.aoe||0)),APEROL_TEXT.splash);}if(s.knockback&&e.hp>0){const d=distance(p,e)||1;for(let i=0;i<7;i++)this.move(e,(e.x-p.x)/d*(s.knockback+(cs.hunterFinish?35:0))/7,(e.y-p.y)/d*(s.knockback+(cs.hunterFinish?35:0))/7);}e.mark=0;e.slow=1;if(marked&&runes===3){this.toast('KOMPLETT ESKALIERT! Drei Punkte + Markierung.');this.emit('shake',{strength:4});}}
    if(id==='interrupt'){
      if(e.cast&&e.cast.interruptible){e.cast=null;e.stun=2;fireProcs(this,'interrupt',cs);e.attackTimer=3;e.vulnerable=4;this.stats.interrupts++;const passives=this.member.passives||{};p.runes=Math.min(3,p.runes+(passives.interruptRunes||0));this.cooldowns.burst=Math.max(0,this.cooldowns.burst-(passives.interruptBurstCd||0));p.energy=Math.min(100,p.energy+15+(cs.procs.includes('silence')?10:0));if(!this.sct({area:'note',kind:'avoid',text:'UNTERBROCHEN',skill:'interrupt',color:'#9bdce4'}))this.float(e.x,e.y-30,'UNTERBROCHEN','#9bdce4');this.log('Klare Ansage · unterbrochen! +35 % Schaden für 4 s.');}
    ...
    if(id==='buff'){this.buffs={...s,remaining:s.duration*(1+(base.buffDuration||0)),shield:s.shield?Math.round(s.shield*(1+cs.shieldPower+(cs.shieldBonus||0)+cs.mastery*.4)):0};this.log(s.name+' · '+s.duration+' s aktiv.');}
    if(id==='heal'){onHealMech(this,healPlayer(this,s.heal,cs,true),cs);fireProcs(this,'heal',cs);}
  ...
  kill(e){const wasMarked=e.mark>0,cs=combatStats(this);onKill(this,e,wasMarked,cs);onKillMech(this,e,wasMarked,cs);this.gainMomentum(cs);if(cs.procs.includes('thirst')){const before=this.player.energy;this.player.energy=Math.min(100,this.player.energy+PROCS.thirst.energy);if(this.player.energy>before)emitCombatFx(this,'proc',this.player,{procId:'item:thirst',signal:'resource',label:'RANDALE'});}e.hp=0;if(this.target===e)stopAuto(this,false);e.aggro=false;e.cast=null;e.dead=(e.respawn?.[0]||35)+this.random()*((e.respawn?.[1]||55)-(e.respawn?.[0]||35));e.respawnAt=this.time+e.dead;e.ai='dead';e.mark=0;e.roamGoal=null;this.stats.kills++;
    ...
    createDrop(this,e);const xp=killXp(e);this.gainXp(xp);if(e.type==='boss'&&BOSS_LINES[e.bossId])this.bark(e,BOSS_LINES[e.bossId].defeat,'boss');if(!this.sct({area:'note',kind:'xp',value:xp,unit:'EP',iconKey:'coins',color:'#bacd8b'}))this.float(e.x,e.y-40,'+'+xp+' EP','#bacd8b');this.effect('death',e.x,e.y,{life:1.5,max:1.5});this.log(e.name+' besiegt · +'+xp+' EP.');
  ...
  hitPlayer(e,n,avoidable=true){beginMeterCombat(this,e);const p=this.player,fresh=p.inCombat<=0;if(p.invulnerable>0&&avoidable){this.stats.dodges++;emitCombatFx(this,'dodge',p,{from:{x:e.x,y:e.y}});fireProcs(this,'dodge',combatStats(this));if(!this.sct({area:'in',kind:'avoid',text:'AUSGEWICHEN',skill:'dash'}))this.float(p.x,p.y-25,'AUSGEWICHEN','#b8e0d3');return;}
    if(p.parry>0&&avoidable){p.parryCharges=Math.max(0,(p.parryCharges||1)-1);if(!p.parryCharges)p.parry=0;fireProcs(this,'parry',combatStats(this));onParry(this,e,combatStats(this));p.runes=Math.min(3,p.runes+1);p.energy=Math.min(100,p.energy+20);this.stats.parries++;this.damage(e,this.skills.find(s=>s.id==='parry').reflect*(1+(combatStats(this).reflect||0))*(this.classState.m?.hausverbot>0?2:1),'Parade');{const back=restoreMeterHealth(this,this.member.passives?.parryHeal||0,'parryHeal');if(back>0)this.sct({area:'in',kind:'heal',value:back,text:'Parade',skill:'parry'});}if(!this.sct({area:'in',kind:'avoid',text:'PARIERT',skill:'parry'}))this.float(p.x,p.y-25,'PARIERT','#f2da92');emitCombatFx(this,'parry',p,{from:{x:e.x,y:e.y}});this.log('Perfekte Parade · +1 Punkt, +20 Randale.');return;}
