...
export class Game {
  ...
  hitPlayer(e,n,avoidable=true){beginMeterCombat(this,e);const p=this.player,fresh=p.inCombat<=0;if(p.invulnerable>0&&avoidable){this.stats.dodges++;fireProcs(this,'dodge',combatStats(this));if(!this.sct({area:'in',kind:'avoid',text:'AUSGEWICHEN',skill:'dash'}))this.float(p.x,p.y-25,'AUSGEWICHEN','#b8e0d3');return;}
    if(p.parry>0&&avoidable){p.parryCharges=Math.max(0,(p.parryCharges||1)-1);if(!p.parryCharges)p.parry=0;fireProcs(this,'parry',combatStats(this));onParry(this,e,combatStats(this));p.runes=Math.min(3,p.runes+1);p.energy=Math.min(100,p.energy+20);this.stats.parries++;this.damage(e,this.skills.find(s=>s.id==='parry').reflect*(1+(combatStats(this).reflect||0)),'Parade');{const back=restoreMeterHealth(this,this.member.passives?.parryHeal||0,'parryHeal');if(back>0)this.sct({area:'in',kind:'heal',value:back,text:'Parade',skill:'parry'});}if(!this.sct({area:'in',kind:'avoid',text:'PARIERT',skill:'parry'}))this.float(p.x,p.y-25,'PARIERT','#f2da92');this.effect('interrupt',p.x,p.y);this.log('Perfekte Parade · +1 Punkt, +20 Randale.');return;}
