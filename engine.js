...
export class Game{
  ...
  startCast(e){if(e.dungeon&&e.bossId)dungeonBossCast(this,e);const p=this.player,set=CAST_SETS[e.castSet]||DUNGEON_CASTS[e.castSet]||CAST_SETS[e.type==='boss'?'horst':e.type]||CAST_SETS.wolf,type=set.cycle[e.cycle%set.cycle.length];
    ...
    const d={...set.casts[type]};if(!available(this,'parry'))d.name=d.name.replace('Parade','Abstand halten');if(!available(this,'interrupt'))d.name=d.name.replace('Q unterbricht','Sichtlinie verlassen');e.cast={...d,type,remaining:d.total,x:d.ground?p.x:e.x,y:d.ground?p.y:e.y,angle:Math.atan2(p.y-e.y,p.x-e.x)};resourceEnemyCast(this,e);
  }
