...
import {freshProcState,fireProcs,procFree,consumeProc,tickProcs,lowHealthReady} from './procs.js';
...
import {startAutopilot,autopilotThreat,tickAutopilot} from './autopilot.js';
import {tryQueue,tickQueue,clearQueue,blocker,alignCooldowns} from './spell-queue.js';
...
import {bossOutOfReach,concealed,roomAt,bossHeld,addressBoss,pullBoss} from './dungeon.js';
import {DUNGEON_CASTS,ENEMY_AUTOS,DEATH_UI} from './content/index.js';
...
import {BALANCE,SPEC_MECHANICS,CAST_SETS,killXp,PROCS,SYSTEM_LINES,BOSS_LINES,BOSSES,ENEMY_BARKS,VILLAGERS,COMBAT_RULES,COMBAT_TEXT,DODGE_UI,STORY_CHAPTERS,triggeredMemories,BUILDINGS,BUILDING_IDS,nextStage,buildingEffects,hubLine} from './content/index.js';
...
/** Dungeon-Fix 4: Treffer-Protokoll des Helden für den Todesrückblick (death-screen.js). Hält HIT_LOG s bzw. höchstens 24 Einträge. */
const HIT_LOG=12;
function noteHit(g,h){const log=g.hitLog||(g.hitLog=[]);log.push({...h,t:g.time});while(log.length>24||log.length&&g.time-log[0].t>HIT_LOG)log.shift();}
/** Treffer der letzten DEATH_UI.recap.window s, der letzte zuletzt; t = Sekunden vor jetzt. */
export function recentHits(g,window=DEATH_UI.recap.window){return (g.hitLog||[]).filter(h=>g.time-h.t<=window).map(h=>({...h,ago:+(g.time-h.t).toFixed(2)}));}
export class Game{
  ...
  resetClassState(){this.res=this.player?freshResource(this):null;this.autoAttack.enabled=false;this.casting=null;this.touchMove=null;this.classState=freshClassState();this.procState=freshProcState();this.fields=[];this.zones=[];this.aiming=null;this.aimPoint=null;this.player.parry=0;this.player.hurt=0;this.player.dash=0;this.player.castPose=0;this.player.parryCharges=0;this.buffs={};}
  ...
  actionCore(id,point=null,completing=false,friend=this.friend,{queued=false,quiet=false}={}){
    ...
    // E-72 Runde 4 (Kenner-Befund Bodenziel): ein anderer Kniff schließt den Zielmodus und läuft selbst; die ausgelöste Vormerkung nicht.
    if(this.aiming&&!completing&&!queued&&id!==this.aiming){this.aiming=null;this.aimPoint=null;}
    ...
    if(!offGcd&&!completing)alignCooldowns(this);/* E-72 R4: Knopfzahl = echte Sperre (spell-queue.js) */
    ...
    if(!handled&&id==='parry'){p.parry=s.window+(cs.parryWindow||0)+resourceParryBonus(this);p.parryCharges=cs.doubleParry?2:1;}
    if(!handled&&id==='dash'){
      p.dash=.22;
  ...
  gainMomentum(cs=combatStats(this)){const M=BALANCE.momentum,m=this.momentum,p=this.player;m.stacks=Math.min(M.maxStacks,m.stacks+1);m.until=this.time+M.duration;m.restUntil=this.time+M.restSeconds;grantResource(this,M.energyOnKill+(this.baseEffects().energyOnKill||0),'kill');this.cooldowns.mark=0;fireProcs(this,'kill',cs);if(!this.sct({area:'note',kind:'momentum',text:'SCHWUNG '+'▲'.repeat(m.stacks),iconKey:'boots',color:'#f4c06a'}))this.float(p.x,p.y-52,'SCHWUNG '+'▲'.repeat(m.stacks),'#f4c06a');}
  gainXp(n){if(!Number.isFinite(n)||n<=0)return;n=Math.round(n*(1+(this.baseEffects().xpBonus||0)));const oldSkills=this.skills.filter(s=>available(this,s.id)).map(s=>s.id),p=this.player;this.trainingXp+=n;p.xp+=n;while(p.level<30&&p.xp>=xpToNext(p.level)){p.xp-=xpToNext(p.level);const hpBefore=p.maxHp;p.level++;this.refreshStats();if(!this.dead)p.hp=p.maxHp;/* Dungeon-Fix 3 (Big-B-Abnahme #721): Aufstieg als Geist füllt kein Leben – sonst zeigte der Rahmen volles Leben bei offenem Todesbildschirm *//* Stufenaufstieg: die UI zeigt die große Einblendung (milestone-ui.js); text bleibt als Rückfall */this.emit('levelUp',{level:p.level,hpGain:p.maxHp-hpBefore,text:SYSTEM_LINES.levelUp(p.level)});if(p.level===BALANCE.player.specLevel&&!this.rpg.talents?.spec){this.emit('specUnlocked');this.log('Stufe '+p.level+': Wähle im Clanbuch unter Talente deinen Hauptbaum – deine gesparten Punkte warten.');}this.memoryEvent({kind:'level',level:p.level});this.emit('rpgChanged');if(!this.dead)this.effect('heal',p.x,p.y,{life:1.5,max:1.5});}if(p.level===30)p.xp=Math.min(p.xp,xpToNext(p.level));const learned=this.skills.filter(s=>available(this,s.id)&&!oldSkills.includes(s.id)).map(s=>s.id);if(learned.length){/* Klassen-Buffs (30 min) landen nur auf freien Plätzen ab Leiste 2 und verdrängen nie Kampfkniffe (rpg.js unlockOnBar) */unlockOnBar(this,learned);this.emit('skillsUnlocked',{ids:learned});}this.emit('save');}
  ...
  /** Endtruhe in der Schatzkammer (Dungeon Etappe 3): Beutel mit Wahl oder null. */
  dungeonChest(){return openDungeonChest(this);}
  ...
  lootReachable(bag){if(!bag||this.dead)return false;if(bag.leaveAsk&&dungeonRun(this))return true;/* Dungeon-Fix 4: Rückfrage der Endtruhe am Ausgang – die Wahl gilt im ganzen Dungeon */if(distance(this.player,bag)<=(bag.reach||COMBAT_RULES.lootRange))return true;const run=dungeonRun(this);return !!(bag.room&&run&&roomAt(run.def,this.player.x,this.player.y)?.id===bag.room);}
  ...
  claimQuest(choice){
    ...
    this.emit('questDone',{title:chapter.title,xp:reward.xp||0,coins:reward.coins||0});this.emit('chapterChanged',{chapter:this.quest.chapter,claimed:chapter.id,actDone:this.quest.actDone});
    this.emit('save');return true;
  }
  ...
  navigate(point){if(this.dead||this.paused||!point||!Number.isFinite(point.x)||!Number.isFinite(point.y)||!tutorialAllowsTravel(this,point))return false;this.casting=null;this.keys.clear();this.routeGoal={x:point.x,y:point.y,...(typeof point.name==="string"&&point.name?{name:point.name}:{})};this.routeStuck=0;this.routeRetried=false;const walk=this.walkWorld();/* Wegkosten: bekannte Gegnerreviere meiden, wenn es einen Umweg gibt (safe-route.js) */this.path=safeRoute(this,walk,this.player,point);/* unerreichbar: so nah wie möglich heran (path-near.js) */if(!this.path.length){this.path=pathNear(walk,this.player,point);if(this.path.length)this.routeGoal={...this.path.at(-1)};}this.moveTo=this.path.shift()||null;if(!this.moveTo){this.routeGoal=null;this.toast(NO_PATH);return false;}/* Autopilot hält beim ersten neuen Angreifer (autopilot.js) */startAutopilot(this);return true;}
  ...
  repath(){const goal=this.routeGoal;if(!goal)return false;this.path=safeRoute(this,this.walkWorld(),this.player,goal);this.moveTo=this.path.shift()||null;if(!this.moveTo){this.path=[];this.routeGoal=null;return false;}return true;}
  ...
  hitPlayer(e,n,avoidable=true,share=0){if(this.dead)return;dismount(this);if(robbiTaunt(this,e,n))return;beginMeterCombat(this,e);if(this.adminGod)return;/* Admin-Konsole: god */const p=this.player,fresh=p.inCombat<=0;if(p.invulnerable>0&&avoidable){this.stats.dodges++;emitCombatFx(this,'dodge',p,{from:{x:e.x,y:e.y}});fireProcs(this,'dodge',combatStats(this));if(!this.sct({area:'in',kind:'avoid',text:'AUSGEWICHEN',skill:'dash'}))this.float(p.x,p.y-25,'AUSGEWICHEN','#b8e0d3');return;}
    if(p.parry>0&&avoidable){p.parryCharges=Math.max(0,(p.parryCharges||1)-1);if(!p.parryCharges)p.parry=0;fireProcs(this,'parry',combatStats(this));onParry(this,e,combatStats(this));resourceParry(this,e,combatStats(this));this.stats.parries++;this.damage(e,this.skills.find(s=>s.id==='parry').reflect*(1+(combatStats(this).reflect||0))*(this.classState.m?.hausverbot>0?(SPEC_MECHANICS[this.rpg.talents.spec]?.hausverbot?.reflect||2):1),'Parade');{const back=restoreMeterHealth(this,this.member.passives?.parryHeal||0,'parryHeal');if(back>0)this.sct({area:'in',kind:'heal',value:back,text:'Parade',skill:'parry'});}if(!this.sct({area:'in',kind:'avoid',text:'PARIERT',skill:'parry'}))this.float(p.x,p.y-25,'PARIERT','#f2da92');emitCombatFx(this,'parry',p,{from:{x:e.x,y:e.y}});this.log('Perfekte Parade.');return;}
    const guardBefore=(this.classState.guard||0)+(this.buffs.remaining>0?this.buffs.shield||0:0),hpBefore=p.hp,cs=combatStats(this);n=share>0?Math.round(p.maxHp*share):Math.round(n*(e.damage||1)*resourceIncoming(this,e)*gapDamage(this,e)/* Stufenabstand (foe-rules.js) */*(1-cs.armor)*(p.hp/p.maxHp<.35?1-(cs.lastStand||0)-(cs.procs.includes('stout')?.08:0):1));if(this.buffs.remaining>0){n=Math.round(n*(1-(this.buffs.reduction||0)));const absorbed=Math.min(n,this.buffs.shield||0);this.buffs.shield=Math.max(0,(this.buffs.shield||0)-absorbed);n-=absorbed;}if(this.partyBuff?.remaining>0){n=Math.round(n*(1-(this.partyBuff.reduction||0)));const soaked=Math.min(n,this.partyBuff.shield||0);this.partyBuff.shield-=soaked;n-=soaked;}n=Math.round(resourceHit(this,Math.round(modifyHit(this,n,cs)*(this.baseEffects().damageTaken??1)*(p.cert?.until>this.time?1+p.cert.stacks*p.cert.taken:1)/* Dungeon Etappe 3: Zertifikat (Siegelring) */),e,cs,{noTab:share>0}));/* E-72: Zeche; Anteil-Treffer der Dungeons kommen voll an */onHitTakenMech(this,n,cs);p.hp=Math.max(0,p.hp-n);/* Dungeon-Fix 4 (Nachprüfung #726): Todesrückblick wie in WoW – die letzten Treffer mit Quelle und Schaden */if(n>0)noteHit(this,{by:e.name||'',skill:e.lastCast?.title||e.lastCast?.name||(share>0?'':e.autoAttack?.name||ENEMY_AUTOS[e.type]?.name||''),auto:!e.lastCast&&!(share>0),ground:!!e.lastCast?.ground,amount:n,hp:p.hp});const absorbedFx=guardBefore-(this.classState.guard||0)-(this.buffs.remaining>0?this.buffs.shield||0:0);if(absorbedFx>0){emitCombatFx(this,'guard',p,{amount:absorbedFx,absorbed:true,from:{x:e.x,y:e.y}});resourceAbsorbed(this,e,absorbedFx);}if(p.hp<hpBefore)emitCombatFx(this,'hurt',p,{amount:hpBefore-p.hp,from:{x:e.x,y:e.y}});if(n>0)p.hurt=.16;p.inCombat=7;if(p.hp>0&&lowHealthReady(this))fireProcs(this,'lowHealth',cs);/* E-72 R4: einmal beim Unterschreiten, nicht bei jedem Treffer darunter (procs.js) */if(e.arena)(this.arenaStats||(this.arenaStats=freshArenaStats())).taken+=n;this.noteAttacker(e,n,fresh);if(!this.sct({area:'in',kind:'damage',value:n,text:e.lastCast?.name||e.lastCast?.title||e.name/* Dungeon-Fix 6 (Prüferin #741: „−955 Big B“ las sich wie Schaden an Big B): bei Zaubern steht die Fähigkeit dabei */,iconKey:'claw'}))this.float(p.x,p.y-18,'−'+n,'#f09a81');this.emit('shake',{strength:1.7});this.emit('sound',{id:'hit'});if(p.hp===0)this.die(e.name,e.lastCast?.title||e.lastCast?.name||(share>0?'':e.autoAttack?.name||ENEMY_AUTOS[e.type]?.name||'')/* Dungeon-Fix 3: der Todesschlag hat immer einen Namen – auch ein Autoangriff */,!!e.lastCast?.ground,n);
  }
  ...
  /** Tod des Helden – aus Treffern, aber auch aus der Zeche oder der Stichflamme (class-resources.js). */
  die(by,skill='',ground=false,amount=0){if(this.dead)return;const recent=recentHits(this);this.dead=true;finishMeterCombat(this);stopAuto(this,false);this.casting=null;this.keys.clear();this.moveTo=null;this.path=[];this.routeGoal=null;/* E-72 R4: ein angehaltener Laufweg lief nach dem Aufwachen weiter – zurück ins Rudel */this.autopilot=null;this.autopilotResume=null;this.memoryEvent({kind:'firstDeath'});/* Ursache für den Todesbildschirm (death-screen.js, Runde 5a); Dungeon-Fix 4: dazu die letzten Treffer */this.emit('death',{by,skill,ground,amount,recent});}
  respawn(){dismount(this);this.floor=0;if(inKiosk(this))leaveKiosk(this,true);finishMeterCombat(this);const p=this.player;this.attackers?.clear();Object.assign(p,this.world.spawn,{hp:p.maxHp,energy:100,inCombat:0,parry:0,attack:0,hurt:0,dash:0,castPose:0,invulnerable:2,vx:0,vy:0,moving:false});startWakeGuard(this);/* „kurzer Schutz“ (foe-rules.js) */this.dead=false;this.resetClassState();this.target=null;this.enemies.forEach(e=>{if(e.aggro)this.resetEnemy(e);});resetCompanions(this);if(inDungeon(this))dungeonRespawn(this);
  ...
  tick(dt){
    if(this.professionCommit)return;tickProfession(this,dt);if(this.professionCommit)return;if(this.paused||this.dead&&!inDungeon(this)/* Dungeon (E-71): Held als Geist, Söldner kämpfen weiter */)return;tickClassBuffs(this,Math.min(dt,.05));if(inKiosk(this)){tickKiosk(this,Math.min(dt,.05));return;}dt=Math.min(dt,.05);this.time+=dt;
    ...
    for(const key in this.cooldowns)this.cooldowns[key]=Math.max(0,this.cooldowns[key]-dt);this.gcd=Math.max(0,this.gcd-dt);tickQueue(this);
    for(const key of ['parry','invulnerable','attack','inCombat','dash','hurt','castPose'])p[key]=Math.max(0,(p[key]||0)-dt);
    if(p.inCombat<=0&&this.attackers?.size)this.attackers.clear();tickWakeGuard(this,dt);
    const tickStats=combatStats(this);if(!resourceKind(this))p.energy=Math.min(100,p.energy+dt*((p.inCombat>0?BALANCE.momentum.combatEnergyRegen:BALANCE.player.energyRegen)+(tickStats.energyRegen||0)));else tickResource(this,dt,tickStats);
    ...
    if(!this.dead&&(p.inCombat===0||resting))p.hp=Math.min(p.maxHp,p.hp+dt*(resting?BALANCE.momentum.restRegen:BALANCE.player.outOfCombatRegen)*(tickStats.procs.includes('hops')?PROCS.hops.regen:1)*(1+(this.baseEffects().restRegen||0)));
    let dx=(this.keys.has('d')||this.keys.has('arrowright')?1:0)-(this.keys.has('a')||this.keys.has('arrowleft')?1:0),dy=(this.keys.has('s')||this.keys.has('arrowdown')?1:0)-(this.keys.has('w')||this.keys.has('arrowup')?1:0);
    ...
    if(this.dead){dx=dy=0;this.moveTo=null;this.path=[];}/* Geist im Dungeon (E-71): der Körper bleibt liegen */const routed=!!this.moveTo;tickMount(this,dt);stepPlayer(this,dx,dy,dt);this.walkStairs(dx,dy,routed,dt);tickTutorial(this,dt);tickCasting(this,dt);tickAuto(this,dt);tickApproach(this,dt);tickAutopilot(this,dt);tickCompanions(this,dt);
    for(const e of this.enemies){
      ...
      // Dungeon (E-71, Kenner-Befund 7): soziale Aggro nur im eigenen Pack, sofort und vollständig – statt der Kette „Kumpel kommt".
      ...
      e.attackTimer=Math.max(0,e.attackTimer-dt);enemyAuto(this,e,dt);if(this.dead)break;if(distance(e,p)<=reach&&e.attackTimer<=0&&this.world.lineClear(e,p)&&specialSlot(this,e)/* Rudel ohne Gleichschlag */)this.startCast(e);
    }
