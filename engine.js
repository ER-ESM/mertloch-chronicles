...
export class Game{
  ...
  action(id,point=null,completing=false,aidId=this.companionAidId){
    ...
    if(id==='buff'){this.buffs={...s,remaining:s.duration*(1+(base.buffDuration||0)),shield:s.shield?Math.round(s.shield*(1+cs.shieldPower+(cs.shieldBonus||0)+cs.mastery*.4)):0};this.log(s.name+' · '+s.duration+' s aktiv.');if(aid)buffCompanionByPlayer(this,aid,this.buffs);if(!aid)this.netParty?.buffFriend?.({name:s.name,icon:s.icon,duration:this.buffs.remaining,reduction:s.reduction||0,hot:s.hot||0,shield:this.buffs.shield||0});this.netParty?.buff?.({name:s.name,icon:s.icon,duration:this.buffs.remaining,reduction:s.reduction||0,hot:s.hot||0,shield:this.buffs.shield||0});}
    if(id==='heal'){const amount=Math.round(s.heal*(1+cs.healPower+(cs.healBonus||0)+cs.mastery*.4)),own=healPlayer(this,s.heal,cs,true),help=aid?healCompanionByPlayer(this,aid,amount):0;if(aid&&(cs.hotHeal||this.rpg.talents.spec==='baerbel-care')&&this.classState.hot>0)aid.aidHot={remaining:this.classState.hot,power:Math.round(this.classState.hotPower*(1+cs.healPower+(cs.healBonus||0)+cs.mastery*.4)),tick:1};onHealMech(this,own+help,cs);if(!aid)this.netParty?.aidHeal?.(amount,s.name);fireProcs(this,'heal',cs);}
