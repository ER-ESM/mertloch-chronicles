...
export class Game{
  constructor(world,saved={},options={}){
    ...
    this.settings={autoLoot:saved.settings?.autoLoot!==false,prerender:saved.settings?.prerender===true,sct:saved.settings?.sct!==false,light:saved.settings?.light!==false,fps:saved.settings?.fps===true,fullRes:saved.settings?.fullRes===true};
  ...
  action(id,point=null,completing=false){
    ...
    if(id==='buff'){this.buffs={...s,remaining:s.duration*(1+(base.buffDuration||0)),shield:s.shield?Math.round(s.shield*(1+cs.shieldPower+(cs.shieldBonus||0)+cs.mastery*.4)):0};this.log(s.name+' · '+s.duration+' s aktiv.');this.netParty?.buffFriend?.({name:s.name,icon:s.icon,duration:this.buffs.remaining,reduction:s.reduction||0,hot:s.hot||0,shield:this.buffs.shield||0});this.netParty?.buff?.({name:s.name,icon:s.icon,duration:this.buffs.remaining,reduction:s.reduction||0,hot:s.hot||0,shield:this.buffs.shield||0});}
    if(id==='heal'){onHealMech(this,healPlayer(this,s.heal,cs,true),cs);this.netParty?.aidHeal?.(Math.round(s.heal*(1+cs.healPower+(cs.healBonus||0)+cs.mastery*.4)),s.name);fireProcs(this,'heal',cs);}
  ...
  respawn(){if(inKiosk(this))leaveKiosk(this,true);finishMeterCombat(this);const p=this.player;this.attackers?.clear();Object.assign(p,this.world.spawn,{hp:p.maxHp,energy:100,runes:0,inCombat:0,parry:0,attack:0,hurt:0,dash:0,castPose:0,invulnerable:2,vx:0,vy:0,moving:false});this.dead=false;this.resetClassState();this.target=null;this.enemies.forEach(e=>{if(e.aggro)this.resetEnemy(e);});resetCompanions(this);
