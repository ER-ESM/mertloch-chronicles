...
import {BALANCE,CAST_SETS,killXp,PROCS,SYSTEM_LINES,BOSS_LINES,BOSSES,COMBAT_RULES,COMBAT_TEXT} from './content/index.js';
...
export class Game {
  ...
  action(id,point=null,completing=false){
    ...
    const p=this.player,cs=combatStats(this),cost=skillCost(this,s,cs),context={runes:p.runes,interrupted:!!this.target?.cast?.interruptible};const failure=beforeSkill(this,s,cs);if(failure){this.toast(failure);return false;}if(this.cooldowns[id]>.01){this.toast(`${s.name} ist noch nicht bereit.`);return false;}
    ...
    this.cooldowns[id]=s.cd*(id==='dash'?(1-(cs.dashCd||0))*(cs.procs.includes('fleet')?.85:1):id==='interrupt'?1-(cs.interruptCd||0):1-cs.haste);p.energy-=cost;
    if(!s.offGcd&&!completing)this.gcd=cs.gcd;
    if(s.range&&!s.ground){this.autoAttack.enabled=true;e.aggro=true;e.ai='combat';p.inCombat=7;p.facing=e.x>p.x?1:-1;p.attack=.25;}
    if(s.talent){performTalent(this,s,point,cs);if(s.ground){this.aiming=null;this.aimPoint=null;}}
    if(id==='strike'){const empowered=this.classState.empowered>0;if(empowered)this.classState.empowered--;this.damage(e,skillDamage(this,s,s.damage,ITEMS)*(empowered||this.classState.freeStrike?2:1),'Kelle');const beat=this.member.id==='baerbel'&&this.time-this.lastStrike>=.85&&this.time-this.lastStrike<=1.5;p.runes=Math.min(3,p.runes+(beat?2:1));this.lastStrike=this.time;context.beat=beat;if(beat)this.float(p.x,p.y-35,APEROL_TEXT.combo,this.member.color);p.energy=Math.min(100,p.energy+s.gain);this.effect(s.range>60?'projectile':'slash',e.x,e.y,{from:{x:p.x,y:p.y},classId:this.member.id,life:.3,max:.3});}
    if(id==='throw'){this.damage(e,skillDamage(this,s,s.damage,ITEMS),'Pfandwurf');this.effect('projectile',e.x,e.y,{from:{x:p.x,y:p.y},life:.4,max:.4,classId:this.member.id});}
    ...
    if(id==='burst'){const runes=p.runes,marked=e.mark>0;let n=skillDamage(this,s,s.base+runes*s.perPoint,ITEMS,runes)*(marked?s.multiplier:1);if(this.relic)n*=1.12;if(this.rpg.talents.spec==='dieter-brawl'&&this.classState.rage>=5)n*=1.25;if(this.rpg.talents.spec==='kevin-iron')n*=.7;this.damage(e,Math.round(n),marked&&runes===3?'RESONANZ':'Entladung');if(s.splash)for(const other of this.enemies){if(other!==e&&other.hp>0&&other.ai!=='returning'&&other.spawnGrace<=0&&distance(e,other)<s.splash&&this.world.lineClear(e,other))this.damage(other,Math.round(n*.55)*(1+(cs.aoe||0)),APEROL_TEXT.splash);}if(s.knockback&&e.hp>0){const d=distance(p,e)||1;for(let i=0;i<7;i++)this.move(e,(e.x-p.x)/d*(s.knockback+(cs.hunterFinish?35:0))/7,(e.y-p.y)/d*(s.knockback+(cs.hunterFinish?35:0))/7);}p.runes=0;e.mark=0;e.slow=1;this.effect('burst',e.x,e.y,{life:.7,max:.7,strong:marked&&runes===3,radius:s.splash||45,classId:this.member.id});if(marked&&runes===3){this.toast('KOMPLETT ESKALIERT! Drei Punkte + Markierung.');this.emit('shake',{strength:4});}}
    if(id==='interrupt'){
    ...
    if(id==='heal'){healPlayer(this,s.heal,cs,true);this.effect('heal',p.x,p.y,{life:1,max:1});}
    afterSkill(this,id,e,cs,context);tutorialSignal(this,id);this.emit('sound',{id});return true;
  }
  damage(e,n,label){if(e.tutorial)return tutorialDamage(this,e,n,label);if(tutorialActive(this))return 0;if(e.hp<=0||e.ai==='returning')return;e.aggro=true;e.ai='combat';this.player.inCombat=7;const cs=combatStats(this),critical=this.random()<cs.crit,mark=e.mark>0,bonus=(label==='Markierung'?(cs.markBonus||0)+cs.mastery:label==='RESONANZ'||label==='Entladung'?(cs.burstBonus||0)+cs.mastery:0)+(cs.execute&&e.hp/e.maxHp<.3?cs.execute:0)+(mark&&cs.procs.includes('verdict')?.1:0);const actual=Math.round(n*(1+cs.power+(['Autoangriff','Kelle','Pfandwurf','Parade','Sprung','Deckelwelle'].includes(label)?cs.physicalPower:cs.technicalPower))*(1+bonus)*(critical?1.6+(cs.critDamage||0):1)*(e.vulnerable>0?1.35:1)*(this.buffs.remaining>0?this.buffs.power||1:1));const dealt=Math.min(e.hp,actual);e.hp=Math.max(0,e.hp-actual);this.stats.damage+=dealt;if(critical&&cs.procs.includes('rage'))this.player.energy=Math.min(100,this.player.energy+PROCS.rage.energy);if(cs.leech)this.player.hp=Math.min(this.player.maxHp,this.player.hp+Math.round(dealt*cs.leech));afterDamage(this,e,dealt,cs);e.attack=.15;this.float(e.x+(this.random()-.5)*14,e.y-27,String(actual)+(critical?'!':''),critical?'#ffdf78':label==='RESONANZ'?'#ecc3fc':'#fff0bf');if(e.hp<=0)this.kill(e);return dealt;}
  kill(e){const wasMarked=e.mark>0,cs=combatStats(this);onKill(this,e,wasMarked,cs);if(cs.procs.includes('thirst'))this.player.energy=Math.min(100,this.player.energy+PROCS.thirst.energy);e.hp=0;e.aggro=false;e.cast=null;e.dead=(e.respawn?.[0]||35)+this.random()*((e.respawn?.[1]||55)-(e.respawn?.[0]||35));e.respawnAt=this.time+e.dead;e.ai='dead';e.mark=0;e.roamGoal=null;this.stats.kills++;createDrop(this,e);const xp=killXp(e);this.gainXp(xp);if(e.type==='boss'&&BOSS_LINES[e.bossId])this.log(e.name+': „'+BOSS_LINES[e.bossId].defeat+'“');this.float(e.x,e.y-40,'+'+xp+' EP','#bacd8b');this.effect('death',e.x,e.y,{life:1.5,max:1.5});this.log(e.name+' besiegt · +'+xp+' EP.');
    ...
    if(this.quest.accepted&&!this.quest.claimed&&!e.ambient&&!e.questId){if(e.type==='wolf')this.quest.wolves=Math.min(3,this.quest.wolves+1);if(e.type==='cultist')this.quest.cultists=Math.min(2,this.quest.cultists+1);if(e.type==='boss')this.quest.boss=true;if(this.questReady())this.toast(SYSTEM_LINES.mainReady);}
    if(e.type==='boss'){this.emit('bossVictory');}this.emit('save');
  }
  gainXp(n){if(!Number.isFinite(n)||n<=0)return;const oldSkills=this.skills.filter(s=>available(this,s.id)).map(s=>s.id),p=this.player;this.trainingXp+=n;p.xp+=n;while(p.level<30&&p.xp>=xpToNext(p.level)){p.xp-=xpToNext(p.level);p.level++;this.refreshStats();p.hp=p.maxHp;this.toast(SYSTEM_LINES.levelUp(p.level));this.emit('rpgChanged');this.effect('heal',p.x,p.y,{life:1.5,max:1.5});}if(p.level===30)p.xp=Math.min(p.xp,xpToNext(p.level));const learned=this.skills.filter(s=>available(this,s.id)&&!oldSkills.includes(s.id)).map(s=>s.id);if(learned.length){unlockOnBar(this,learned);this.emit('skillsUnlocked');}this.emit('save');}
  questReady(){return this.quest.wolves>=3&&this.quest.cultists>=2&&this.quest.boss;}
  acceptQuest(){if(tutorialActive(this))return false;this.quest.accepted=true;this.log('Auftrag angenommen: Die letzte Kiste.');this.emit('save');}
  claimQuest(choice){if(this.paused||!this.questReady()||this.quest.claimed||this.dead||distance(this.player,this.world.npc)>50)return false;if(!chooseReward(this,'main',choice))return false;this.quest.claimed=true;this.relic=true;this.rpg.coins+=25;this.gainXp(600);this.emit('save');return true;}
  ...
  respawn(){const p=this.player;Object.assign(p,this.world.spawn,{hp:p.maxHp,energy:100,runes:0,inCombat:0,parry:0,invulnerable:2,vx:0,vy:0,moving:false});this.dead=false;this.resetClassState();this.target=null;this.enemies.forEach(e=>{if(e.aggro)this.resetEnemy(e);});this.toast(SYSTEM_LINES.respawn);}
  tick(dt){if(this.paused||this.dead)return;dt=Math.min(dt,.05);this.time+=dt;tickActivity(this);const p=this.player;tickClass(this,dt,combatStats(this));for(const z of this.zones){z.remaining-=dt;if(z.remaining<=0){const victims=this.enemies.filter(e=>e.hp>0&&e.ai!=='returning'&&!e.spawnGrace&&distance(e,z)<z.radius&&this.world.lineClear(z,e)).sort((a,b)=>distance(a,z)-distance(b,z)).slice(0,5);for(const e of victims)this.damage(e,z.damage*(1+(combatStats(this).aoe||0)),'Böller');if(this.rpg.talents.spec==='kevin-fuse'||combatStats(this).burnGround)this.fields.push({...z,kind:'burn',remaining:combatStats(this).burnGround?6:2,tick:1,power:0});this.effect('burst',z.x,z.y,{life:.7,max:.7,radius:z.radius});}}this.zones=this.zones.filter(z=>z.remaining>0);this.life.tick(dt,p);if(!tutorialActive(this))this.ecology.tick(dt);if(this.buffs.remaining>0)this.buffs.remaining=Math.max(0,this.buffs.remaining-dt);
    ...
    for(const e of this.enemies){
      if(tutorialActive(this)||e.tutorial)continue;
