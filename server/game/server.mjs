...
export function cleanCompanionWire(list){if(!Array.isArray(list))return undefined;const out=[];for(const w of list.slice(0,4)){const i=String(w?.i||'');if(!/^merc-[a-z-]{2,40}$/.test(i)||out.some(o=>o.i===i))continue;out.push({i,x:Math.round(num(w.x,-1e6,1e6)),y:Math.round(num(w.y,-1e6,1e6)),f:Number(w.f)<0?-1:1,s:['idle','walk','combat','down'].includes(w.s)?w.s:'idle',h:Math.round(num(w.h,0,100,100)),l:Math.round(num(w.l,1,60,1)),...(w.a===1||w.a===2?{a:w.a}:{}),...(w.r===1?{r:1}:{})});}return out.length?out:undefined;}
