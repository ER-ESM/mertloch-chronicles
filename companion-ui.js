...
export function mountCompanionHud(shell,getGame,open){
 ...
 return {update(){const g=getGame(),list=g.companions||[];el.hidden=!list.length;if(!list.length)return;
  const key=list.map(c=>c.id+':'+c.level).join('|');if(key!==signature){signature=key;el.innerHTML=`<header>${button('manage','',UI.team)}</header>`+list.map(c=>`<button type="button" class="companion-frame unit-frame" data-companion-select="${c.id}" data-companion-row="${c.id}" data-role="${c.def.role}" title="${UI.select}" aria-pressed="false">${unitPortrait(c.def.look,c.level)}<span class="unit-content"><strong>${esc(c.name)} <small>${COMPANION_ROLES[c.def.role].name}</small></strong><span class="companion-life" role="progressbar" aria-label="${esc(c.name)}"><i></i><span></span></span><span class="companion-frame-meta"><span data-companion-state></span><small data-companion-contract></small></span></span></button>`).join('');paintUnitPortraits(el);}
  const pick=selectedCompanion(g),failure=pick&&helpFailure(g,helpTarget(g));
  for(const c of list){const row=el.querySelector(`[data-companion-row="${c.id}"]`),selected=pick===c;updateRow(row,g,c);row.setAttribute('aria-pressed',String(selected));row.classList.toggle('is-selected',selected);row.title=selected?(failure||UI.selected):UI.select;if(selected)row.querySelector('[data-companion-state]').textContent=UI.selected+' \u00b7 '+status(g,c);row.classList.toggle('target-unavailable',selected&&!!failure);}
 }};
}
