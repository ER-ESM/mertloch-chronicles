...
export function companionPanel(g,{tab='offers',selected=''}={}){
 ...
 <header class="companion-intro"><span class="eyebrow">${T.title}</span><h2>${tab==='offers'?T.board:UI.team}</h2><p>${UI.intro}</p><p>${UI.aidHint}</p><div class="companion-summary"><strong data-companion-wallet>${UI.coins(g.rpg.coins)}</strong><span data-companion-slots>${UI.slots(1+(g.partyHumans||0)+list.length,R.maxActive+1)}</span></div></header>
