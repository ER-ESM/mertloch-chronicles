...
export class WorldFx{
 ...
 guard(ms,now=performance.now()){this.cost+=(ms-this.cost)*.05;this.frames++;const gap=now-(this.lastFrame??now);this.lastFrame=now;if(gap>0&&gap<250)this.pace=this.pace?this.pace+(gap-this.pace)*.05:gap;/* Pausen (Tab im Hintergrund) zählen nicht */
  if(this.forced||this.mode!=='voll'||this.frames<F.guard.window)return;const slow=this.pace>F.guard.slowMs;if(this.cost<=F.guard.budgetMs&&!slow)return;this.mode='leicht';this.reason=this.cost>F.guard.budgetMs?`zu teuer (${this.cost.toFixed(1)} ms), leichte Stufe`:`Bildtempo zu niedrig (${Math.round(1000/this.pace)} FPS), leichte Stufe`;this.upload=0;this.cost=0;if(this.debug)console.warn('[world-fx] '+this.reason);}
}
