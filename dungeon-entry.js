...
export function entryCard(g,id='schloss-bigb',{talents=true}={}){
 ...
 const hire=offers.length?`<div class="dg-hire" aria-label="${esc(E.hire)}">${offers.map(o=>`<button type="button" class="dg-offer" data-dg-hire="${esc(o.def.id)}" data-role="${o.def.role}" ${o.affordable?'':'aria-disabled="true"'} ${tip(E.hire+': '+o.def.name,E.hireNote(o.cost,COMPANION_ROLES[o.def.role].name)+(o.affordable?'':' · '+E.noMoney))}>${unitPortrait(o.def.look,null)}${dicon('role-'+o.def.role,16,'dg-role')}</button>`).join('')}</div>`:'';
