...
function lantern(c,x,y,t){line(c,'#594d35',[[x,y],[x,y-32],[x+9,y-32]],2);if(drawMaifeld(c,'lantern',x+9,y-12,21))return;framed(c,'#dac293',x+4,y-25,8,12,P.ink,.8);r(c,'#ffe6a1',x+6,y-23,4,8);}
function board(c,x,y,text){if(drawMaifeld(c,'board',x,y+7,40))return;framed(c,'#8a665a',x-2,y-7,3,15,P.ink,.5);framed(c,'#554655',x-22,y-28,44,22,'#d0ad77',1);c.font='bold 5px Georgia';c.textAlign='center';c.fillStyle='#f2d7a1';c.fillText(text,x,y-19);for(const xx of [-14,-3,8]){r(c,'#e8d6ad',x+xx,y-15,8,7);r(c,'#ac8b91',x+xx+2,y-13,4,.5);r(c,'#a08e87',x+xx+2,y-11,3,.5);}}
function drawSiteProps(c,site,time){if(!site.dressing||!maifeld.tent)return false;for(const p of site.dressing){if(p.type==='board')board(c,p.x,p.y,'AUFTRÄGE');else if(p.type==='lantern')lantern(c,p.x,p.y,time);else drawMaifeld(c,p.type,p.x,p.y,p.height);}return true;}
export function drawHub(c,hub,time){if(drawSiteProps(c,hub,time))return;if(hub.id==='kirchplatz'){board(c,hub.x-72,hub.y+4,'CLAN-BRETT');return;}const x=hub.x,y=hub.y;c.save();
 if(maifeld.tent){drawMaifeld(c,'tent',x,y-20,69);drawMaifeld(c,'supplies',x-64,y+3,29);board(c,x+80,y+4,'AUFTRÄGE');lantern(c,x-87,y-6,time);c.restore();return;}
...
export function drawOccupiedCamp(c,camp,time,clear){if(drawSiteProps(c,camp,time))return;const x=camp.x,y=camp.y;c.save();
 if(maifeld.tent){drawMaifeld(c,'tent',x-70,y-68,62);drawMaifeld(c,'supplies',x+73,y-30,32);lantern(c,x-112,y+12,time);board(c,x+101,y+18,clear?'FREIGERÄUMT':camp.type==='boss'?'RUHE 22:01':'BESETZT');c.restore();return;}
...
export function drawEstateDetail(c,p,time){const x=p.x,y=p.y;
 if((p.kind===0||p.kind===3)&&drawMaifeld(c,'supplies',x,y+3,23))return;
 if(p.kind===1&&drawMaifeld(c,'bench',x,y+3,18))return;
