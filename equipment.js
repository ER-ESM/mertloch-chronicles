...
export const itemSlotName=d=>d?.weapon?d.weapon.label||WEAPON_TYPES[d.weapon.type]?.name||'Waffe':d?.slot==='ring'?'Ring':d?.slot==='trinket'||d?.slot==='charm'?'Glücksbringer':d?.shield?'Schild · Nebenhand':EQUIPMENT_SLOTS[d?.slot]||'';
