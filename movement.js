...
export function stepPlayer(game,dx,dy,dt){const p=game.player,requested=!!(dx||dy),len=Math.hypot(dx,dy)||1,speed=(p.mount&&Object.hasOwn(MOUNTS,p.mount)?WALK_SPEED*mountSpeed(game):(p.inCombat>0?COMBAT_SPEED:WALK_SPEED)*(1+classBuffValue(game,'speed')))*(game.touchMove?Math.min(1,len):1)*(p.slowed>0?1-p.slowed:1)/* Dungeon Etappe 4 Teil A: nasser Boden (Kurts Sprinkler) */;
