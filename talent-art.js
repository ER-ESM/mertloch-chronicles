...
import {CLASS_SPECS,TALENT_ART} from './content/index.js';
import {contentAsset,drawContentIcon,loadContentArt} from './content-art.js';
const sheets=new Map();let pending;
export function talentIconCell(id){for(const [member,specs] of Object.entries(CLASS_SPECS))for(let s=0;s<specs.length;s++){const prefix=specs[s]+'-';if(id.startsWith(prefix)){const n=Number(id.slice(prefix.length));if(Number.isInteger(n)&&n>=0&&n<10)return {member,index:s*10+n};}}return null;}
export function loadTalentArt(){return pending||=Promise.all([loadContentArt(),...Object.keys(CLASS_SPECS).map(member=>member==='baerbel'?loadAperolArt():new Promise(resolve=>{const img=new Image();img.onload=()=>{sheets.set(member,importSheet(img,member));resolve();};img.onerror=()=>resolve();img.src=TALENT_ART.path+member+'.png';}))]);}
...
export function paintTalentIcon(canvas,id){
