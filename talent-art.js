...
export function loadTalentArt(){return pending||=Promise.all([loadE32Art(),loadContentArt(),loadDetailArt(),...Object.keys(CLASS_SPECS).filter(member=>member==='baerbel'||TALENT_ART.edges[member])/* alter Atlas nur für Klassen, die einen haben – sonst 404 bei Schorsch/Käthe */.map(member=>member==='baerbel'?loadAperolArt():new Promise(resolve=>{const img=new Image();img.onload=()=>{sheets.set(member,importSheet(img,member));resolve();};img.onerror=()=>resolve();img.src=TALENT_ART.path+member+'.png';}))]);}
