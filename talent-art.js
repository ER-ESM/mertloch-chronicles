...
import {drawDetailIcon,loadDetailArt} from './detail-art.js';
...
/** E-72 Runde 4 (Kenner-Befund 6): Nur Klassen mit Talentatlas (Schnittkanten in TALENT_ART.edges) laden einen Bogen – Schorsch und Käthe
 *  haben keinen (Vokabel-Talentbilder), ihr Abruf lieferte zwei 404-Fehler in der Konsole schon beim Seitenstart. Bärbel: Aperol-Atlas. */
export const talentSheetMembers=()=>Object.keys(CLASS_SPECS).filter(member=>member==='baerbel'||!!TALENT_ART.edges?.[member]);
export function loadTalentArt(){return pending||=Promise.all([loadE32Art(),loadContentArt(),loadDetailArt(),...talentSheetMembers().map(member=>member==='baerbel'?loadAperolArt():new Promise(resolve=>{const img=new Image();img.onload=()=>{sheets.set(member,importSheet(img,member));resolve();};img.onerror=()=>resolve();img.src=TALENT_ART.path+member+'.png';}))]);}
...
function paintVocabTalent(canvas,id,cell){
 ...
 const t=document.createElement('canvas');t.width=W;t.height=H;const tc=t.getContext('2d');if(!drawDetailIcon(tc,icon,0,0,W))paintItem(t,icon);fitMotif(t,.86);
