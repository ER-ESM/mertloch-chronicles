...
export function createNetParty({game,me,send,others,ui=null}){
 ...
 function receive(m){
  ...
  if(m.t==='roll')ui?.roll(m,verdict(m.item));
  else if(m.t==='rollpick')ui?.pick(m);
  else if(m.t==='rolled'){const mine=m.winner===me(),own=settle(m.item);if(mine)grantLoot(g,m.item,{name:own?.source||m.item.name,kind:'enemy'});ui?.result(m,mine);}
...
export function mountRollUi(shell,{choose,esc=s=>String(s).replace(/[&<>"']/g,c=>'&#'+c.charCodeAt(0)+';'),rarityName=r=>r}){
 ...
 return {
  roll(m,verdict){const el=document.createElement('section');el.className='roll-card';el.dataset.roll=m.id;
   el.innerHTML='<header><b class="rarity-'+esc(m.item.rarity)+'">'+esc(m.item.name)+'</b><small>'+esc(rarityName(m.item.rarity))+' · '+esc(PARTY_UI.from.replace('{n}',m.from))+(verdict?.verdict==='upgrade'||verdict?.verdict==='empty'?' · <em>'+esc(PARTY_UI.upgrade)+'</em>':'')+'</small></header>'+
   ...
   ensure().appendChild(el);cards.set(m.id,el);},
