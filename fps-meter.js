...
export function mountFpsMeter(root,{enabled=false}={}){
 ...
 const api={
  ...
  /** Einmal je Bild am Ende der Schleife: `now` = rAF-Zeit, `workMs` = Rechenzeit dieses Bildes. */
  frame(now,workMs){
   ...
   el.textContent=fps+' FPS · '+avg.toFixed(1).replace('.',',')+' ms';el.dataset.level=fps>=50?'ok':fps>=30?'warn':'bad';
