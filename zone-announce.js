...
export function mountZoneAnnounce({label=document.querySelector('.region-label'),minimap=document.querySelector('#miniButton'),name=document.querySelector('#zoneName')}={}){
 ...
 function announce(text=name.textContent.trim()){/* nacheinander (Runde 2b): nicht zugleich mit einer großen Einblendung an derselben Stelle */if(document.querySelector('.milestone.show')){clearTimeout(settle);settle=setTimeout(()=>{if(name.textContent.trim()===text)announce(text);},400);return;}shown=text;set(true);clearTimeout(hide);hide=setTimeout(()=>set(false),ZONE_SHOW_MS);document.body.dataset.zoneAnnounced=text;}
