...
// damit ein Weg entlang zweier Straßen nicht flackert.
...
export function mountZoneAnnounce({label=document.querySelector('.region-label'),minimap=document.querySelector('#miniButton'),name=document.querySelector('#zoneName')}={}){
 ...
 const set=on=>{label.classList.toggle('zone-show',on);minimap?.classList.toggle('zone-show',on);};
 function announce(text=name.textContent.trim()){shown=text;set(true);clearTimeout(hide);hide=setTimeout(()=>set(false),ZONE_SHOW_MS);document.body.dataset.zoneAnnounced=text;}
