...
import {iconStep} from './icon-steps.js';
...
export function passiveBook(g){
 const main=g.rpg?.talents?.spec,cls=g.member.id,tiles=[tile({describe:'passive:'+cls,icon:iconMarkup({set:'clan',member:cls},undefined,iconStep('book')),name:T.classTitle(g.member.name),origin:T.classScope,on:true})];
 for(const spec of CLASS_SPECS[cls]||[]){const h=mechanicHelp(g,spec);if(!h)continue;const on=main===spec,origin=on?T.active(SPECS[spec].name):T.withMain(SPECS[spec].name),art=`<canvas width="${iconStep('book')}" height="${iconStep('book')}" data-spec-art="${esc(spec)}" aria-hidden="true"></canvas>`;
