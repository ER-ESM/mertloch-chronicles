...
export function tutorialDialogue(g,touch=false){const t=g.tutorial,s=D.steps[t.step],dressed=['body','legs','feet'].some(slot=>g.rpg.equipment[slot]),welcome=(dressed?D.welcomeDressed:D.welcome)+' '+D.welcomeTools;return `${conversationHeader('ida')}<span class="eyebrow">${D.title} · ${t.step+1}/${D.steps.length}</span><h2>${s.title}</h2><p>${t.step===0?welcome:s.text}</p>${t.step===0||t.step===7?'':`<p class="dialog-keyhint">${touch?s.touch:s.desktop}</p>`}<div class="dialog-actions">${t.step===0?`<button class="outline-button" data-shell="clan">${D.clan}</button>`:''}${t.step===0||t.step===7?`<button class="gold-button" data-tutorial-next>${t.step===0?D.start:D.finish}</button>`:('<button class="outline-button" data-close>'+D.continue+'</button>')}</div>`;}
