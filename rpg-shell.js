...
export function mountShell(api){const root=document.querySelector('#gameShell');
 ...
 /* Runde 5b (Spielerbericht R5): Tastenkappen der Menüleiste aus der wirksamen Tastenbelegung – nach jeder Änderung neu */
 const liveWin=w=>{const k=keysOf(liveKeymap(),w[0]).filter(Boolean).map(b=>bindingLabel(b));return k.length?[w[0],w[1],w[2],k[0],w[4],k[1]]:w;};
 const rail=document.createElement('nav');rail.className='game-menu-rail';rail.setAttribute('aria-label',WINDOW_UI.rail);rail.innerHTML=WINDOW_UI.windows.map(w0=>{const w=liveWin(w0),[id,name,icon,key]=w;return `<button data-panel="${id}" data-tooltip-label="${WINDOW_UI.railLabel(w)}" data-tooltip-note="${WINDOW_UI.notes[id]}" aria-label="${WINDOW_UI.open(name)}">${menuIcon(icon)}<span>${name}</span><kbd>${key}</kbd></button>`;}).join('');root.append(rail);rail.onclick=e=>{const b=e.target.closest('[data-panel]');if(b)api.panel(b.dataset.panel);};
 document.addEventListener('keymap-change',()=>{for(const w0 of WINDOW_UI.windows){const b=rail.querySelector(`[data-panel="${w0[0]}"]`);if(!b)continue;const w=liveWin(w0);b.querySelector('kbd').textContent=w[3];b.dataset.tooltipLabel=WINDOW_UI.railLabel(w);}});
