...
export class WorldLight{
 constructor(){this.dark=L.ambient.zones.rest;this.frame=0;this.silhouettes=new Map();this.glows=new Map();this.shadowLayer=canvas(1,1);this.lightLayer=canvas(1,1);this.glowLayer=canvas(1,1);this.clouds=null;this.vignette=null;}
 /** Hängt Licht- und Scheinebene als Überlagerung hinter die Weltfläche: gemischt wird im Compositor, nicht im Zeichenweg der Welt. */
 mount(world){if(this.mounted||!world?.parentNode)return;this.mounted=true;for(const [layer,mode] of [[this.lightLayer,'multiply'],[this.glowLayer,'screen']]){layer.className='world-light';layer.setAttribute('aria-hidden','true');Object.assign(layer.style,{position:'absolute',inset:'0',width:'100%',height:'100%',pointerEvents:'none',mixBlendMode:mode,display:'none'});world.after(layer);}}
 show(on){for(const layer of [this.lightLayer,this.glowLayer]){const value=on?'block':'none';if(layer.style.display!==value)layer.style.display=value;}}
 ...
 apply(view,game,world,time,elapsed){const {ox,oy,W,H}=view,dark=this.zoneDark(game,world,elapsed),visible=(o,pad)=>o.x>ox-pad&&o.x<ox+W+pad&&o.y>oy-pad&&o.y<oy+H+pad,lights=this.sources(game,world,visible,time),lw=Math.ceil(W/2),lh=Math.ceil(H/2),layer=this.lightLayer;
  ...
  const glow=this.glowLayer;if(glow.width!==lw||glow.height!==lh){glow.width=lw;glow.height=lh;}const c=glow.getContext('2d');c.setTransform(.5,0,0,.5,0,0);c.globalCompositeOperation='source-over';c.globalAlpha=1;c.fillStyle='#000000';c.fillRect(0,0,W,H);
  c.globalCompositeOperation='lighter';for(const s of lights){if(s.s===L.sources.hero)continue;const r=s.s.radius*s.scale*s.flicker*.8;c.globalAlpha=Math.min(1,L.glow.day+L.glow.night*dark)*s.flicker;c.drawImage(this.glow(s.s.color),s.x-ox-r,s.y-oy-r,r*2,r*2);}
  ...
  l.globalCompositeOperation='source-over';l.globalAlpha=Math.min(1,L.grade.vignette+dark*.25);l.drawImage(this.vignette,0,0,W,H);l.globalAlpha=1;}
}
