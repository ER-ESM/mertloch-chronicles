...
/** NPC in der Welt: Anziehpuppe aus content/figuren.js zuerst (nicht für Porträts und nicht, wenn der Aufrufer eigene Ausrüstung mitgibt = Held). */
export function drawWorldPerson(c,id,x,y,time=0,scale=1,pose={}){if(!pose.portrait&&!pose.visualEquipment&&drawFigure(c,id,x,y,scale,pose))return;if(drawLivePerson(c,id,x,y,time,pose,scale))return;drawTinyPerson(c,x,y,time,{...pose,classId:PERSON_APPEARANCE[id]||id,facing:pose.facing||1},false,scale);}
...
export function paintPersonPortrait(canvas,id,pose={}){
 ...
 if(hasContentAsset('portrait-'+id)&&!lookKey(pose.tint)){drawContentIcon(c,'portrait-'+id,0,0,96);if(tintKey(pose.tint)){/* festes Porträtbild: gleiche Farbregeln, der ganze Ausschnitt zählt als Kopfbereich */c.restore();try{const img=c.getImageData(0,0,canvas.width,canvas.height);tintPixels(img.data,canvas.width,canvas.height,{x:canvas.width/2,y:canvas.height*.42,brow:canvas.height*.4,side:canvas.width*.2},canvas.width,pose.tint,/^(anni|baerbel)/.test(id));c.putImageData(img,0,0);}catch{}c.save();}}else if(lookKey(pose.tint))drawWorldPerson(c,id,48,PORTRAIT_FEET[id]||236,0,9,{...pose,portrait:true});else drawWorldPerson(c,id,48,130,0,4,{...pose,portrait:true});c.restore();canvas.dataset.personSprite=PERSON_APPEARANCE[id]||id;
}
