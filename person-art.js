...
import {tintKey,tintPixels} from './hero-tint.js';
...
export function paintPersonPortrait(canvas,id,pose={}){
 ...
 if(hasContentAsset('portrait-'+id)){drawContentIcon(c,'portrait-'+id,0,0,96);if(tintKey(pose.tint)){/* festes Porträtbild: gleiche Farbregeln, der ganze Ausschnitt zählt als Kopfbereich */c.restore();try{const img=c.getImageData(0,0,canvas.width,canvas.height);tintPixels(img.data,canvas.width,canvas.height,{x:canvas.width/2,y:canvas.height*.42,brow:canvas.height*.4,side:canvas.width*.2},canvas.width,pose.tint,/^(anni|baerbel)/.test(id));c.putImageData(img,0,0);}catch{}c.save();}}else drawWorldPerson(c,id,48,130,0,4,pose);c.restore();canvas.dataset.personSprite=PERSON_APPEARANCE[id]||id;
}
