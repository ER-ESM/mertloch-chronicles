...
if(process.argv[1]?.endsWith('bude-house-guide.mjs')){
 ...
 try{
  ...
  for(const [name,g] of Object.entries(GUIDE)){
   const png=await b.evaluate(`(async()=>{const {BUDE_HOUSE}=await import('./content/index.js');const {buildHouse}=await import('./world-house.js');const art=await import('./bude-house-art.js');
    ...
    ${name==='innen'?`x.fillStyle='#6f8f55';x.fillRect(0,0,c.width,c.height);`:''}
    x.setTransform(${g.scale},0,0,${g.scale},${g.origin.x},${g.origin.y});
    ${name==='innen'?`art.drawHouseFloor(x,h,1);const walls=[...h.walls].sort((a,b)=>a.maxY-b.maxY);for(const w of walls)art.drawHouseWall(x,w,h,1);`:`art.drawHouseExterior(x,h,1,h.doors.find(d=>d.id==='eingang'));`}
