...
export function components(image){const {width:w,height:h,data}=image,seen=new Uint8Array(w*h),queue=new Int32Array(w*h),pieces=[];
...
export function segment(image,cols,rows,{ranked=false}={}){const {width:w,height:h}=image,pieces=components(image);
 if(ranked){const bands=Array.from({length:rows},()=>[]);for(const p of pieces.filter(p=>p.count>500))bands[Math.min(rows-1,Math.floor(p.cy/h*rows))].push(p);return bands.flatMap((parts,row)=>{if(parts.length!==cols)throw Error('Expected '+cols+' components in row '+row+', got '+parts.length);return parts.sort((a,b)=>a.cx-b.cx);});}
