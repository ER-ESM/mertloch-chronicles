...
const coats={villager7:{hipFraction:.83,kneeFraction:.91,stepReach:8,stepLift:3,groundDepth:4},horst:{hipFraction:.78,kneeFraction:.9,stepReach:10,stepLift:4}};
 ...
 for(const [id,a] of Object.entries(sourceCatalog.assets)){
  ...
  for(let row=0;row<4;row++){
   ...
   for(let phase=0;phase<8;phase++){
    ...
    if(rect){const [px,py,pw,ph]=rect;blit(preserved,r.image,{x:px,y:py,w:pw,h:ph},{x:px,y:py+Math.round(r.bodyOffset*size/192)});}
  ...
  catalog.assets[id+'-walk']={...a,path,source:a.path,sourceHash:hash(bytes),hash:hash(data),width:atlas.width,height:atlas.height,columns:Array.from({length:8},(_,i)=>'walk-'+i),frames,stride:WALK_RIG.stride*a.worldHeight/26,animation:'registered-two-leg-gait'};
 }
