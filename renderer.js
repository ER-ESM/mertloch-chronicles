...
import {dungeonSortables,drawDungeonProp,drawDungeonCeiling} from './dungeon-scenery-art.js';/* Räume als Orte: Requisiten tiefensortiert, Lichterketten oben */
import {dungeonScale,drawRoleMark,inIdleSwarm,swarmPlates} from './dungeon-actors.js';
import {dungeonPlateHeight} from './dungeon-figuren-art.js';
...
import {drawTargetRings,noteDrawn} from './target-ui.js';
...
import {alertShown,alertProgress} from './foe-rules.js';
...
/** Ankündigung eines Gegners (foe-rules.js, E-72 R5): gepixeltes rotes „!“ direkt auf der Weltleinwand – keine Schrift, damit die
 *  Sprechblase des Rufs es nicht verdrängt (Schilder unter einer Blase entfallen, flushLabels). x/y = Oberkante Mitte. */
function alertMark(c,x,y){rect(c,'#1d1410',x-2,y-1,4,10);rect(c,'#1d1410',x-2,y+10,4,4);rect(c,'#ff7a4a',x-1,y,2,8);rect(c,'#ffd2a0',x-1,y,2,2);rect(c,'#ff7a4a',x-1,y+11,2,2);}
...
let heroGhost=null,ghostLayer=null,outlineLayer=null,lastCorpse=null,heroTree=false;
/** Grauschleier des Geists (Dungeon-Fix 2): entsättigt und dunkelt die Welt in 0,6 s weich ab – zwei Flächen, kein Filter, keine Mischmodi
 *  (E-46). Grau nimmt Farbe, Dunkel nimmt Licht; Namensschilder und HUD liegen darüber und bleiben farbig. Werte: LIGHTING.deathVeil. */
export function drawDeathVeil(c,W,H,since){const V=LIGHTING.deathVeil,k=Math.max(0,Math.min(1,since/V.fade));if(!(k>0))return;c.save();c.globalAlpha=V.gray*k;c.fillStyle=V.grayColor;c.fillRect(0,0,W,H);c.globalAlpha=V.dark*k;c.fillStyle=V.darkColor;c.fillRect(0,0,W,H);c.restore();}
...
export class Renderer {
  ...
  drawScene(){heroGhost=null;heroTree=false;const lit=this.game.settings?.light!==false,kiosk=inKiosk(this.game),dungeon=inDungeon(this.game)/* Etappe 2: im Keller keine Außenwelt-Licht- und Effektschicht, eigene Lichtstimmung je Ebene */;/* Ohne Grafikkarte: Farbabstimmung eingebacken statt CSS-Filter (E-50) */this.software??=softwareRendering();{const baked=this.software&&lit?gradeFilter():'';if(bakedGrade.filter!==baked){bakedGrade.filter=baked;this.ground?.invalidate();}}applyGrade(this.canvas,lit&&!this.gradeOff&&!this.software);this.light.mount(this.canvas);this.light.show(lit&&!kiosk&&!dungeon);const effects=this.game.settings?.fx!==false&&!kiosk&&!dungeon;fxQuality.rich=fxQuality.force??(effects&&!this.software);this.fx.mount(this.canvas);this.fx.show(effects);if(kiosk){drawKioskRoom(this);return;}const c=this.ctx,w=this.world,g=this.game,p=g.player,time=g.time,bubbles=this.bossSpeech.update(g);labelBoxes=[clanSignBounds(c,w)];this.frame++;const elapsed=Math.min(.1,Math.max(.001,time-(this.lastDrawTime??time-.016)));this.lastDrawTime=time;/* cameraFocus: Einführungsfilm fährt die Kamera weich an Orte, ohne den Helden zu bewegen *//* Runde 4b: heroShift (hero-reveal.js/hero-frame.js) legt den Helden weich in die Lücke zwischen offenen Fenstern */const hs=this.heroShift,cs=this.heroShiftNow||={x:0,y:0},ks=1-Math.exp(-5*elapsed);cs.x+=((hs?.x||0)-cs.x)*ks;cs.y+=((hs?.y||0)-cs.y)*ks;const focus=this.cameraFocus||(Math.abs(cs.x)+Math.abs(cs.y)>.05?{x:p.x-cs.x,y:p.y-cs.y,pinned:true}:p),follow=focus.pinned?1/* in der Lücke: Kamera hängt fest am Helden, sonst rutscht er beim Laufen aus der Lücke; weich ist der Versatz selbst */:1-Math.exp(-(this.cameraFocus?this.cameraFocus.speed||1.6:10)*elapsed);this.camera.x+=(focus.x-this.camera.x)*follow;this.camera.y+=(focus.y-this.camera.y)*follow;const W=this.viewWidth,H=this.viewHeight;c.setTransform(this.density,0,0,this.density,0,0);this.shake*=.87;
    ...
    /* Dungeon-Räume (docs/DUNGEON-RAEUME-2026-09-25.md): Requisiten laufen mit den Figuren; draußen steht die Garage als Gebäude */if(dungeon)sorted.push(...dungeonSortables(g,visible));else if(!g.instance){const door=dungeonEntrance(g);if(door&&visible(door,170))sorted.push({type:'dungeonGarage',obj:door,y:door.y});}
    ...
    const plateTop=e=>{/* Dungeon-Figuren (2026-09-26): Schild über der gezeichneten Figur statt über dem Platzhalter */if(e.dungeon){const fh=dungeonPlateHeight(e);if(fh)return e.y-fh*dungeonScale(e);}const dummy=(e.tutorial||e.dummy)&&hasContentAsset('ui-arena-dummy')?37:0;const art=e.tutorial||e.dummy?0:liveActorHeight(e.variant||e.bossId||e.skin,e.variant);return e.y-(dummy||(art?art+11:e.tutorial?58:e.type==='boss'?(e.variant==='automat'?56:41):e.type==='cultist'?36:e.elite?37:29))*dungeonScale(e);};
    const plates=[];{const view={x:this.viewOrigin.x,y:this.viewOrigin.y,w:W,h:H};c.save();c.font="800 7px Nunito,'Trebuchet MS',sans-serif";for(const e of g.enemies){if(!visible(e)||e.hp<=0||dungeon&&(concealed(g,e)||inIdleSwarm(g,e))||!(e===g.target||e.aggro||distance(e,p)<160||g.hoverUnit?.ref===e))continue;/* Neutrale Tiere (Runde 5a, Grafikbefund 10, WoW): Schild nur unter der Maus, als Ziel oder nach dem ersten Treffer */if(isNeutralUnit(e)&&e!==g.target&&g.hoverUnit?.ref!==e&&!(e.hp<e.maxHp))continue;const y=plateTop(e),named=(g.settings?.namesEnemy!==false||e===g.target)&&!(e.dungeonBoss&&e.aggro&&e.ai==='combat')/* Dungeon Etappe 4 Teil B: im Bosskampf trägt der Bossrahmen den Namen, über dem Boss nur der Balken */,w=Math.max(44,named?c.measureText(e.name).width+6:0),at=liftOffHero(g,e.x,y,20),cl=clampPlate(at.x,y,w,view);plates.push({id:e.id,e,x:cl.x,y:cl.y,w,named,prio:e===g.target?0:e.aggro?1:2});}c.restore();}
    const stacked=stackPlates(plates),plateOf=new Map(plates.map(q=>[q.e,q])),plateBoxes=[];
    for(const e of g.enemies){if(!visible(e)||e.hp<=0||dungeon&&concealed(g,e))continue;const y=plateTop(e);e.spriteTop=y+6;/* Spur der gezeichneten Lagen (Runde 5a): Rechtsklick auf einen laufenden Gegner trifft auch dort, wo er eben noch gezeichnet war */noteDrawn(e,y+6);/* Oberkante des Bilds für den Treffertest (target-ui.js) */const pl=plateOf.get(e),dy=pl?stacked.get(e.id):null;if(pl&&dy!=null){/* Schild über dem Gegner, nie auf der eigenen Figur (Runde 2b, Kenner-Befund 9) */const ax=pl.x,ay=pl.y+dy;/* Trefferfläche des Namensschilds für die Maus (target-ui.js, Runde 3a) */e.plateAt={x:ax,y:ay,t:performance.now()};plateBoxes.push({l:ax-pl.w/2-(questMob(g,e)?30:0),r:ax+pl.w/2,t:ay-10,b:ay+8});if(pl.named)label(c,e.name,ax,ay,REACTION_COLORS[reactionOf(e)].name,7,true);nameplate(c,e,ay+4,isNeutralUnit(e),ax);/* Ankündigung (foe-rules.js, E-72 R5): rotes „!“ rechts am Lebensbalken – links sitzt die Auftragsmarke, darüber die Sprechblase */if(alertShown(g,e)&&alertProgress(g,e)==null)alertMark(c,ax+25,ay+2);/* gegenüber der Auftragsmarke (ax−25) */if(dungeon)drawRoleMark(c,e,ax,ay+6);if(questMob(g,e))questMobMark(c,ax-25,ay+6);if(e.elite)drawContentIcon(c,'ui-elite-badge',Math.round(ax-29),Math.round(ay-9),16);}else e.plateAt=null;/* Zielmarkierung der Gruppe über dem Kopf, auch aus der Ferne (target-marks.js) */if(e.groupMark){const fresh=e.plateAt&&performance.now()-e.plateAt.t<200;drawMark(c,e.groupMark,fresh?e.plateAt.x:e.x,(fresh?e.plateAt.y:y)-15,6);}
      ...
      /* Ankündigung (foe-rules.js, E-72 R5): rotes „!“, wer dich von sich aus bemerkt; Fernkämpfer zücken den Block – roter Balken wie ein Zauber */{const k=alertProgress(g,e);if(k!=null){const yy=y+11;rect(c,'#282b23',e.x-23,yy,46,4);rect(c,'#e2432f',e.x-22,yy+1,44*k,2);alertMark(c,e.x+28,yy-2);/* neben dem Balken: über dem Schild liegt oft die Sprechblase (Schrift darunter entfällt) */}else if(alertShown(g,e)&&!e.plateAt&&!(e.mark>0))alertMark(c,e.x,y-16);}
    ...
    /* Dungeon-Fix 2: Grauschleier beim Tod im Bild statt CSS-Filter auf der Weltfläche (der Filterwechsel zeigte ohne Grafikkarte 1–2 s Schwarz) */this.deadAt=g.dead?(this.deadAt??performance.now()):null;if(g.dead)drawDeathVeil(c,W,H,(performance.now()-this.deadAt)/1000);
    ...
    const bounds=this.canvas.getBoundingClientRect(),obstacles=bubbles.length?[...document.querySelectorAll('.hud,.region-label,.action-area,.game-popup,.attack-warning:not(.hidden),.touch-topline,#touchMenu,#touchContext,#touchStick,#touchActions,#touchUtility,#buffStrip,#touchCancelAim,#tutorialGuide,.boss-frame:not([hidden]),.boss-announce:not([hidden])')].filter(el=>getComputedStyle(el).visibility!=='hidden'/* Dungeon Etappe 4 Teil B: ein ausgeblendeter Zonentitel drückt die Blase nicht mehr bis unter den Bossrahmen; Bossrahmen und Ansage sind Hindernis */).map(el=>el.getBoundingClientRect()).filter(b=>b.width&&b.height).map(b=>({x:(b.left-bounds.left)/this.zoom,y:(b.top-bounds.top)/this.zoom,w:b.width/this.zoom,h:b.height/this.zoom})):[];
