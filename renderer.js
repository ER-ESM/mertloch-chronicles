...
import {hoverNear,hoverInBox,liftOffHero,labelHudFree,labelYield,stackPlates,clampPlate,waypointOrbit,WAYPOINT} from './world-labels.js';
...
let heroGhost=null,ghostLayer=null,lastCorpse=null,heroTree=false;
function ghost(c,p,draw,alpha=.68){if(typeof document==='undefined'||typeof c.getTransform!=='function')return;const S=96,ax=48,ay=84,d=Math.max(1,Math.min(4,Math.abs(c.getTransform().a)||1)),L=ghostLayer||=document.createElement('canvas');if(L.width!==Math.ceil(S*d)){L.width=L.height=Math.ceil(S*d);}
 ...
 f.setTransform(1,0,0,1,0,0);f.globalCompositeOperation='source-in';f.fillStyle='#f3e2b0';f.fillRect(0,0,L.width,L.height);f.globalCompositeOperation='source-over';c.save();c.globalAlpha=alpha;c.drawImage(L,p.x-ax,p.y-ay,S,S);c.restore();}
...
export class Renderer {
  ...
  drawScene(){heroGhost=null;heroTree=false;const lit=this.game.settings?.light!==false,kiosk=inKiosk(this.game);/* Ohne Grafikkarte: Farbabstimmung eingebacken statt CSS-Filter (E-50) */this.software??=softwareRendering();{const baked=this.software&&lit?gradeFilter():'';if(bakedGrade.filter!==baked){bakedGrade.filter=baked;this.ground?.invalidate();}}applyGrade(this.canvas,lit&&!this.gradeOff&&!this.software);this.light.mount(this.canvas);this.light.show(lit&&!kiosk);const effects=this.game.settings?.fx!==false&&!kiosk;this.fx.mount(this.canvas);this.fx.show(effects);if(kiosk){drawKioskRoom(this);return;}const c=this.ctx,w=this.world,g=this.game,p=g.player,time=g.time,bubbles=this.bossSpeech.update(g);labelBoxes=[clanSignBounds(c,w)];this.frame++;const elapsed=Math.min(.1,Math.max(.001,time-(this.lastDrawTime??time-.016)));this.lastDrawTime=time;/* cameraFocus: Einführungsfilm fährt die Kamera weich an Orte, ohne den Helden zu bewegen *//* Runde 4b: heroShift (hero-reveal.js/hero-frame.js) legt den Helden weich in die Lücke zwischen offenen Fenstern */const hs=this.heroShift,cs=this.heroShiftNow||={x:0,y:0},ks=1-Math.exp(-5*elapsed);cs.x+=((hs?.x||0)-cs.x)*ks;cs.y+=((hs?.y||0)-cs.y)*ks;const focus=this.cameraFocus||(Math.abs(cs.x)+Math.abs(cs.y)>.05?{x:p.x-cs.x,y:p.y-cs.y,pinned:true}:p),follow=focus.pinned?1/* in der Lücke: Kamera hängt fest am Helden, sonst rutscht er beim Laufen aus der Lücke; weich ist der Versatz selbst */:1-Math.exp(-(this.cameraFocus?this.cameraFocus.speed||1.6:10)*elapsed);this.camera.x+=(focus.x-this.camera.x)*follow;this.camera.y+=(focus.y-this.camera.y)*follow;const W=this.viewWidth,H=this.viewHeight;c.setTransform(this.density,0,0,this.density,0,0);this.shake*=.87;
    ...
    this.speakers=[];for(const item of sorted){const e=item.obj,at=e?.giver||e;if(['npc','hotspotgiver','questgiver','mentor','merchant'].includes(item.type)&&Number.isFinite(at?.x))this.speakers.push({x:at.x,y:at.y,name:at.name||e.name||w.npc?.name||''});hideLabels=!!(house&&houseFade<.5&&item.type!=='houseShell'&&Number.isFinite(at?.x)&&insideHouse(house,at.x,at.y));c.save();if(this.actorRenderer?.(c,item,time)){c.restore();continue;}if(item.type==='corpse'){drawCorpse(c,e,g.time-e.corpseAt,p,time);}else if(item.type==='classField'){drawClassField(c,e,time);}else if(item.type==='houseWall'){drawHouseWall(c,e,house,houseFade);}else if(item.type==='kitItem'){drawHouseItem(c,e,houseFade);}else if(item.type==='houseShell'){drawHouseExterior(c,e,1-houseFade,e.doors.find(d=>d.id==='eingang'));}else if(item.type==='building'){if([p,...(g.target?.hp>0?[g.target]:[])].some(u=>buildingOccludesActor(e,u)))c.globalAlpha=.38;this.building(c,e);}
      ...
      else if(item.type==='hotspotgiver'){const n=e.giver,glyph=giverGlyph(g,e.id);drawWorldPerson(c,n.npc,n.x,n.y,time,PERSON_SCALE,{facing:-1});const named=nearestSpeaker(g,n);if(named)(g.settings?.namesFriendly!==false)&&label(c,n.name,n.x,n.y-32,NPC_NAME,8);if(glyph){c.save();if(glyph==='low')c.globalAlpha=.45;questBadge(c,n.x,n.y-(named?43:36),glyph==='low'?'!':glyph,false,time);c.restore();}}
      ...
      else if(item.type==='questgiver'){const n=e.giver,s=g.sideQuests[e.id];drawWorldPerson(c,n.npc,n.x,n.y,time,PERSON_SCALE,{facing:-1});const named=nearestSpeaker(g,n);if(named)(g.settings?.namesFriendly!==false)&&label(c,n.name,n.x,n.y-32,NPC_NAME,8);if(!s.claimed)questBadge(c,n.x,n.y-(named?43:36),s.progress>=e.required?'?':s.accepted?'…':'!',false,time);}
    ...
    const plateTop=e=>{const dummy=(e.tutorial||e.dummy)&&hasContentAsset('ui-arena-dummy')?37:0;const art=e.tutorial||e.dummy?0:liveActorHeight(e.variant||e.bossId||e.skin,e.variant);return e.y-(dummy||(art?art+11:e.tutorial?58:e.type==='boss'?(e.variant==='automat'?56:41):e.type==='cultist'?36:e.elite?37:29));};
    const plates=[];{const view={x:this.viewOrigin.x,y:this.viewOrigin.y,w:W,h:H};c.save();c.font="800 7px Nunito,'Trebuchet MS',sans-serif";for(const e of g.enemies){if(!visible(e)||e.hp<=0||!(e===g.target||e.aggro||distance(e,p)<160))continue;const y=plateTop(e),named=g.settings?.namesEnemy!==false||e===g.target,w=Math.max(44,named?c.measureText(e.name).width+6:0),at=liftOffHero(g,e.x,y,20),cl=clampPlate(at.x,y,w,view);plates.push({id:e.id,e,x:cl.x,y:cl.y,w,named,prio:e===g.target?0:e.aggro?1:2});}c.restore();}
    const stacked=stackPlates(plates),plateOf=new Map(plates.map(q=>[q.e,q]));
    for(const e of g.enemies){if(!visible(e)||e.hp<=0)continue;const y=plateTop(e);e.spriteTop=y+6;/* Oberkante des Bilds für den Treffertest (target-ui.js) */const pl=plateOf.get(e),dy=pl?stacked.get(e.id):null;if(pl&&dy!=null){/* Schild über dem Gegner, nie auf der eigenen Figur (Runde 2b, Kenner-Befund 9) */const ax=pl.x,ay=pl.y+dy;/* Trefferfläche des Namensschilds für die Maus (target-ui.js, Runde 3a) */e.plateAt={x:ax,y:ay,t:performance.now()};if(pl.named)label(c,e.name,ax,ay,REACTION_COLORS[reactionOf(e)].name,7,true);nameplate(c,e,ay+4,isNeutralUnit(e),ax);if(questMob(g,e))questMobMark(c,ax-25,ay+6);if(e.elite)drawContentIcon(c,'ui-elite-badge',Math.round(ax-29),Math.round(ay-9),16);}else e.plateAt=null;/* Zielmarkierung der Gruppe über dem Kopf, auch aus der Ferne (target-marks.js) */if(e.groupMark){const fresh=e.plateAt&&performance.now()-e.plateAt.t<200;drawMark(c,e.groupMark,fresh?e.plateAt.x:e.x,(fresh?e.plateAt.y:y)-15,6);}
    ...
    const destination=inDungeon(g)?null:g.destination();if(destination&&distance(p,destination.point)>145){/* Runde 4b (Grafikbefund 10): goldener Pfeil mit dunkler Kontur auf einer Kreisbahn um den Helden (WoW-Supertracking), Entfernung darunter */const hy=p.y-17-(g.stairLift?.()||0),d=destination.point,dx=d.x-p.x,dy=d.y-hy,n=Math.hypot(dx,dy),o=waypointOrbit(dx,dy,WAYPOINT.radius);waypointArrow(c,p.x+o.x,hy+o.y,o.angle);label(c,Math.round(Math.hypot(d.x-p.x,d.y-p.y)/SCALE)+' m',p.x+o.x,hy+o.y+(o.y>-4?15:-8),'#ffe6a8',6,true);}
