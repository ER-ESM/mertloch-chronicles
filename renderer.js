...
function drawCorpse(c,e,age,p,time){const k=Math.max(0,age)/CORPSE_TIME,fall=Math.min(1,age/.28),ease=1-(1-fall)**3,side=e.x>=p.x?1:-1;c.save();
 c.globalAlpha=k<.55?1:Math.max(0,1-(k-.55)/.45);c.translate(e.x,e.y);c.rotate(side*ease*Math.PI*.46);c.translate(-e.x,-e.y+ease*3);if('filter' in c)c.filter='grayscale(.55) brightness(.82)';
