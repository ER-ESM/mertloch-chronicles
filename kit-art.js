...
const INK='#293b44',BASE='./assets/precision/runtime/kit/',KIT={meta:null,images:new Map(),patterns:new Map(),off:false};let requested=false;
/** Nur Platzhalter (Bildvorlagen der Pipeline). */
export function kitArtOff(){KIT.off=true;}
function meta(){
 if(KIT.off)return null;
 if(!requested&&typeof fetch!=='undefined'&&typeof Image!=='undefined'){requested=true;fetch(BASE+'kit.json').then(r=>r.ok?r.json():null).then(m=>{if(m)KIT.meta=m;}).catch(()=>{});}
 return KIT.meta;
}
/** Geladenes Bild einer Sprite-Art samt Registrierung, sonst null. */
function sprite(id){const m=meta()?.sprites?.[id];if(!m)return null;let img=KIT.images.get(id);if(!img){img=new Image();img.src=BASE+m.file;KIT.images.set(id,img);}return img.complete&&img.naturalWidth?{img,m,k:KIT.meta.pxPerUnit}:null;}
...
export function drawWallDecor(c,it,wall){
 ...
 if(s){const hh=s.img.height/s.img.width*it.w;c.drawImage(s.img,it.minX,top+(it.height-hh),it.w,hh);return;}
...
export function drawKitItem(c,it){
 ...
 if(s){const scale=it.w/(s.img.width/s.k),hh=s.img.height/s.k*scale;c.drawImage(s.img,it.minX,it.maxY-lift-hh,it.w,hh);return;}
