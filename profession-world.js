import {PROFESSION_STATIONS} from './content/index.js';
...
export function professionWorld(world){
 ...
 function place(dx,dy,size){const base={x:world.spawn.x+dx,y:world.spawn.y+dy};for(let r=0;r<=400;r+=25)for(let i=0;i<(r?16:1);i++){const p={x:base.x+Math.cos(i*Math.PI/8)*r,y:base.y+Math.sin(i*Math.PI/8)*r};if(world.blocked(p.x,p.y,size)||used.some(a=>Math.hypot(a.x-p.x,a.y-p.y)<size+55)||(world.trees||[]).some(t=>Math.abs(t.x-p.x)<size+90*(t.size||1)&&t.y>p.y-45&&t.y<p.y+140*(t.size||1))||!world.findPath(world.spawn,p).length)continue;used.push(p);return p;}return null;}
 ...
 const value={stations,nodes};cache.set(world,value);return value;
}
