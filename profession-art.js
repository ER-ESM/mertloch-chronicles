...
function label(c,text,x,y,color='#eed39a'){c.font='8px Georgia';c.textAlign='center';c.lineWidth=3;c.strokeStyle='#13201ded';c.strokeText(text,x,y);c.fillStyle=color;c.fillText(text,x,y);}
...
export function drawProfession(c,e,g,time){const near=Math.hypot(g.player.x-e.x,g.player.y-e.y)<100;c.save();
 if(e.type==='professionStation'){const smith=e.id==='werkhof';drawProp(c,{kind:smith?'bude-werkstatt':'bude-grill',x:e.x-16,y:e.y-4,w:38,h:22});drawWorldPerson(c,smith?'sigi':'gisela',e.x+22,e.y,time,.9,{facing:-1});if(!smith){herb(c,e.x-30,e.y+8,true);c.fillStyle='#92b386';c.fillRect(e.x-10,e.y-28,5,12);c.fillStyle='#c3ad7f';c.fillRect(e.x-9,e.y-31,3,4);}label(c,ST[e.id].name,e.x,e.y-61);if(near)label(c,'F · '+T.teachers,e.x,e.y+23);}
