...
export function presenceMessage(game,worldKey){const p=game.player,privateRoom=!!game.instance;return {t:'pos',w:privateRoom?'':worldKey,x:privateRoom?0:Math.round(p.x),y:privateRoom?0:Math.round(p.y),f:p.facing||1,c:game.member?.id,l:p.level,sp:game.rpg?.talents?.spec,s:privateRoom?'idle':game.dead?'dead':p.inCombat>0?'combat':p.moving?'walk':'idle',h:Math.max(0,Math.min(100,Math.round(100*p.hp/(p.maxHp||1)))),fl:!privateRoom&&game.floor?1:undefined,k:p.look||undefined,kt:lookKey(p.tint)||undefined,mt:privateRoom||game.dead?null:p.mount||null,md:p.direction,eq:!privateRoom?equipmentAppearance(game.rpg?.equipment,ITEMS).map(({slot,id,asset,rarity,hands})=>({slot,id,asset,rarity,hands})):undefined,cp:privateRoom?undefined:companionWire(game),tg:!privateRoom&&game.target?.hp>0&&game.target.netId||undefined,a:poseCode(p),r:p.usingRanged?1:undefined};}
/** Kampfpose fürs Netz: 1 = Schlag, 2 = Zauber, sonst nichts (Mitspieler sehen Angriffe statt reglosem Stehen). */
export const poseCode=u=>u?.attack>0?1:u?.castPose>0?2:undefined;
