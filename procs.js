...
export function fireProcs(g,trigger,cs,info={}){const st=g.procState||(g.procState=freshProcState());st.counts||(st.counts={});let fired=0;const p=g.player;
 for(const id of procIds(cs)){const r=PROC_RULES[id];if(!r||r.trigger!==trigger)continue;
  ...
  if(r.skill&&r.skill!==info.skill)continue;if(r.zone&&![].concat(r.zone).some(z=>info.zones?.includes(z)))continue;/* zone: eine Art oder eine Liste (E-60) */
  ...
  if(ef.free)st.free[ef.free]=until;if(ef.empower)st.empower[ef.empower]=until;if(ef.reset)g.cooldowns[ef.reset]=0;if(ef.energy)p.energy=Math.min(100,p.energy+ef.energy);if(ef.shield)addGuard(g,ef.shield,cs);if(ef.heal)healPlayer(g,typeof ef.heal==='number'?ef.heal:(info.damage||0)*ef.heal.damage,cs,false,{id:'proc:'+id,name:Object.values(TALENT_ROWS).flat().find(t=>t.effects?.['proc:'+id])?.name||METER_TEXT.proc},typeof ef.heal!=='number');if(ef.cdReduce)for(const c of [].concat(ef.cdReduce))if(c?.skill in g.cooldowns)g.cooldowns[c.skill]=Math.max(0,g.cooldowns[c.skill]-(c.seconds||0));if(ef.haste){st.haste=Math.max(st.haste,ef.haste);st.hasteUntil=until;}if(ef.supply){const mm=mechanic(g);if(mm?.supply)M(g).supply=Math.min(mm.supply.max+(cs.supplyMax||0),M(g).supply+ef.supply);}if(ef.clean){const mm=mechanic(g);if(mm?.supply)M(g).clean=Math.max(M(g).clean,ef.clean);}
