...
export const freshProcState=()=>({free:{},empower:{},glow:{},haste:0,hasteUntil:0,fired:0});
...
export function tickProcs(g){const st=g.procState;if(!st)return;for(const kind of ['free','empower','glow'])for(const k of Object.keys(st[kind]))if(st[kind][k]<=g.time)delete st[kind][k];if(st.hasteUntil<=g.time)st.haste=0;}
