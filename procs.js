...
export function fireProcs(g,trigger,cs,info={}){const st=g.procState||(g.procState=freshProcState());st.counts||(st.counts={});let fired=0;const p=g.player;
 for(const id of procIds(cs)){const r=PROC_RULES[id];if(!r||r.trigger!==trigger)continue;
  ...
  const skill=r.glow&&g.skills.find(s=>s.id===r.glow);{const line=skill?skill.name.toUpperCase()+(ef.cdReduce&&g.cooldowns[r.glow]>0?' · '+g.cooldowns[r.glow].toFixed(1)+' s':' BEREIT'):'PROC';if(!g.sct?.({area:'note',kind:'proc',text:line,icon:{set:'talents',spec:(id.match(/^(.*)-\d+$/)||[])[1]||'',index:Number((id.match(/-(\d+)$/)||[])[1]||0)},iconKey:'burst',color:'#ffd77a',procId:id}))g.float(p.x,p.y-44,line,'#ffd77a');}g.log('Proc · '+r.text);g.emit('proc',{id});}
 return fired;}
