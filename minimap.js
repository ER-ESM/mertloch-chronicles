...
const AREA_FILL=a=>{const t=a.tags||{},use=t.landuse||t.natural||t.leisure;return use==='farmland'?'#8a8454':use==='residential'?'#6a7059':['forest','wood'].includes(use)?'#35553e':['meadow','grass','park','village_green'].includes(use)?'#5f7c4e':'#58724b';};
let stripe=null;
function farmPattern(c){if(stripe)return c.createPattern(stripe,'repeat');stripe=document.createElement('canvas');stripe.width=stripe.height=8;const s=stripe.getContext('2d');s.fillStyle='#8a8454';s.fillRect(0,0,8,8);s.fillStyle='#7c774a';s.fillRect(0,0,8,2);s.fillRect(0,4,8,1);return c.createPattern(stripe,'repeat');}
function paintBase(cv,w,ox,oy,cs,zoom){
 ...
 for(const b of w.buildings){if(b.maxX<ox||b.minX>x1||b.maxY<oy||b.minY>y1)continue;c.save();c.translate(1,1.4);path(b.points);c.closePath();c.fillStyle='#1f2a2066';c.fill();c.restore();path(b.points);c.closePath();c.fillStyle=b.church?'#dcb970':'#a45a43';c.fill();c.strokeStyle=b.church?'#f4dc9c':'#c8785a';c.lineWidth=1;c.stroke();const bw=(b.maxX-b.minX)*cs,bh=(b.maxY-b.minY)*cs;if(bw>5&&bh>5){c.strokeStyle=b.church?'#b8944e':'#7c3f2e';c.beginPath();if(bw>=bh){const y=((b.minY+b.maxY)/2-oy)*cs;c.moveTo((b.minX-ox)*cs+1.5,y);c.lineTo((b.maxX-ox)*cs-1.5,y);}else{const x=((b.minX+b.maxX)/2-ox)*cs;c.moveTo(x,(b.minY-oy)*cs+1.5);c.lineTo(x,(b.maxY-oy)*cs-1.5);}c.stroke();}}
